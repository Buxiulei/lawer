 ...
 */
const FORCED_CAUSE_SRC = String.raw`(?:拖欠|欠薪|克扣|未(?:足额|及时|按时)?(?:支付|发放)|未(?:依法)?(?:缴纳?|交)(?:社保|社会保险|五险)|工资[^，。；！？、]{0,6}[没未不][发给结付]|[没未][发给结](?:我)?工资|欠着[^，。；！？、]{0,4}工资|社保[^，。；！？、]{0,4}[没未不][交缴]|[没未](?:给我)?[交缴]社保)`;
 ...
 */
const COMPANY_ACTOR_SRC = String.raw`(?<![向给对])${COMPANY_SIDE_SRC}(?:(?!我|本人)[^，。；！？、]){0,4}(?:[向给对]我)?`;
/**
 * **我方发出**的形态，三条：
...
const COMPANY_TERMINATED = new RegExp(
  String.raw`(?<![向给对])${COMPANY_SIDE_SRC}.{0,10}(?:通知我|把我|将我|对我)?(?:解除|辞退|开除|终止|解聘)`,
);
...
/**
 * **还没定**的那一类：风声、传闻、放话、打算、可能、考虑、准备、计划……
 * 它们与已作出的决定同形，只差一个"到底发生了没有"。
 *
 * 【为什么这些词必须紧挨着那个决定动作，而不是整条一命中就排掉】
 * 「准备 / 计划 / 考虑」这些词在**已经解除之后**同样常见：
 *「公司送达《解除劳动合同通知书》，让我准备交接」。整条排的形态是——
 * 一个手上正拿着解除通知书的人，三条要件写着「缺失」，清单让他去补那份他已经收到的纸。
 * 所以只排"这个词就压在那个决定动作上"的写法（中间隔不过 4 个字，且不跨标点）。
 */
const COMPANY_DECISION_UNSETTLED = new RegExp(
  String.raw`(?:说要|打算|可能|也许|或许|风声|传闻|听说|据说|放话|考虑|准备|计划|将要|有意|意向|威胁|暗示|拟)[^，。；！？、]{0,4}${COMPANY_DECISION_ACT_SRC}`,
);
/**
 * **协商类**：协商 / 和解，以及"催我签、让我签"那几种提议形态。整条一命中就不算。
 ...
 */
const COMPANY_DECISION_NEGOTIATED = /协商|和解|催签|让我签|要我签/;
 ...
 * 老板放话可能要解除一批人；以及**协商那一类**（协商不是单方决定）——
...
function companyTerminationDecision(raw: string): boolean {
  const s = raw.replace(/\s+/g, '');
  if (s === '') return false;
  // ① 还没定的、以及协商那一类，**先排掉**：它们里面也带着「解除 / 裁员」这些词
  //    （「说要裁员」「催签协商解除协议」），不先排就会被②当成已经作出的决定。
  if (COMPANY_DECISION_UNSETTLED.test(s) || COMPANY_DECISION_NEGOTIATED.test(s)) return false;
  // ② 公司已经作出的那个决定。剩下的（调岗、降薪、约谈、警告、考核、搬迁、待岗、年会）
  //    一个都不带这些词，自然落不进来——不另列黑名单，是因为黑名单挡不住
  //    「约谈时通知我解除劳动合同」这种两样都写着的记录，而那一条确实是决定。
  return COMPANY_DECISION_ACT.test(s);
}
