 ...
 */
const FORCED_CAUSE_SRC = String.raw`(?:拖欠|欠薪|克扣|未(?:足额|及时|按时)?(?:支付|发放)|未(?:依法)?(?:缴纳?|交)(?:社保|社会保险|五险))`;
...
/** 公司一侧的主语。三处共用（下面两条方向判定的排除项 + `COMPANY_TERMINATED`）。 */
const COMPANY_SIDE_SRC = String.raw`(?:公司|HR|单位|老板|人事)`;
/**
 * 公司一侧作**收件方**时的称谓：上面那几个词，外加那份通知书上写的法定称谓「用人单位」。
 *「用人单位」只落在这里、不进 `COMPANY_SIDE_SRC`——那份词表同时管着「公司作出解除」的主语，
 * 两份词表合成一份是另一票的事（本票只修收件与格）。
 */
const COMPANY_RECIPIENT_SRC = String.raw`(?:用人)?${COMPANY_SIDE_SRC}`;
 ...
 *     上面那道与格例外看的是紧挨着的那两个字，「向用人单位」中间还隔着「用人」，它看不见。
 *     收件方与发出动词之间同样隔得住一小段（「向公司**的 HR**发出」「给用人单位**的人事**
 *     发了」），按主语短语那边的同一个尺寸放行。
 *     **发出动词是这一条的必要部分**：只认那个介词短语的形态是——「我对公司解除劳动合同
 *     有异议」被判成我发出了通知，而那句话说的是公司解除、方向正相反。
 */
const SENT_BY_ME = new RegExp(
  String.raw`(?<!${COMPANY_ACTOR_SRC})${SEND_ACT_SRC}|(?:通知|告知)${COMPANY_RECIPIENT_SRC}|[向给对]${COMPANY_RECIPIENT_SRC}[^，。；！？、]{0,4}${SEND_ACT_SRC}`,
);
/**
...
const LABOR_ELEMENT_CARDS: readonly ElementCard[] = [
  ...
  },
  {
    ...
    // 这一条当场写着「成立」；它又恰好是本包声明的 counterpartyDecisionSlot，
