...
function noWrittenContract(raw: string): boolean {
  ...
  if (s === '') return false;
  // ①「一次都没签」「一份也没有」——这一类自己带着"一次/一份"，要先认，否则会被②判成签过
...
const LABOR_ELEMENT_CARDS: readonly ElementCard[] = [
  ...
  },
  {
    ...
    // 被迫解除通知书与它的送达回执落在证据库的「沟通记录」类别下——这个行当的类别表里
    // 没有「我方文书」那一格（lib/evidence/categories.ts 八类）。
    satisfiedBy: ['evidence:沟通记录'],
    typicalEvidence: [
