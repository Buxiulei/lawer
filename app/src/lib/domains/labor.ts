...
function forcedTerminationNotice(raw: string): boolean {
  ...
  //   不排掉它，⑥ 会把「因公司拖欠工资，双方协商解除劳动合同」判成被迫解除。
  if (/协商(一致|解除)|和解协议|离职协议/.test(s)) return false;
  // ④「被迫解除 / 被迫离职 / 被迫辞职」是这件事在本行当里的固定说法，认它。
  if (/被迫(解除|离职|辞职|终止)/.test(s)) return true;
  // ⑤ 点了第三十八条的条号 + 一个解除动作。
  if (/(三十八|卅八|38)条/.test(s) && /解除|离职|终止/.test(s)) return true;
  // ⑥ 没说「被迫」、也没点条号时：**事由与解除动作必须同时出现**。
  //   事由只取第三十八条第一款里可机械认的那两项（欠薪、未缴社保）——它们是本行当
  //   被迫解除最常见的两个由头，且措辞收敛。只有事由（「催问工资什么时候发」）
  //   或只有动作（「拒绝签署解除协议」）都不算。
  const cause = /拖欠|欠薪|克扣|未(足额|及时|按时)?(支付|发放)|未(依法)?(缴纳?|交)(社保|社会保险|五险)/.test(s);
  const act = /(解除|终止)(劳动)?(合同|关系|通知书?)/.test(s);
  if (cause && act) return true;
  // ⑦ 剩下的一律认不准 ⇒ 不算（异议函、加班申请、催薪函、整理证据、申请仲裁……）。
  //    落「缺失」，清单里由 missingAs 点名「被迫解除通知」——那正是他该补的东西。
  return false;
}
...
const LABOR_ELEMENT_CARDS: readonly ElementCard[] = [
  ...
  },
  {
    ...
    // 理由与词表样本写在那个函数上）。判不过时用 missingAs 点名「被迫解除通知」，
    ...
    slotChecks: {
      'timeline:我方动作': { accepts: forcedTerminationNotice, missingAs: '被迫解除通知' },
    },
    typicalEvidence: [
