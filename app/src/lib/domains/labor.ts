...
/**
 * 一条时间线记录的原字 ⇒ 判定用的那一串。**换行是小句边界**，其余空白去掉。
 *（第五轮复审第四条，2026-09-10；`companyTerminationDecision` 与 `forcedTerminationNotice`
 * 共用，两处各写一遍的形态是改了一处、另一处静默沿用旧口径。）
 *
 * 【为什么换行不能直接抹掉】`lib/cases/elements.ts` 的 checkableEntries 传进来的是
 *「标题\n详情」——它特意放了那个换行，好让判定自己决定要不要把两段字当一句连续的话读。
 * 上一版 `\s+ ⇒ ''` 等于把两段接在一起，于是标题末尾与详情开头凑出一句谁都没写过的话：
 *「领导暗示」+「公司送达了《解除劳动合同通知书》」被读成一句「领导暗示公司送达了…」，
 * 未定态那条把整句排掉，一份已经拿到手的解除通知书判成「缺失」。
 * 换行按逗号处理，与下游每一处小句切分、每一处「不跨标点」的量程自然一致。
 */
function recordText(raw: string): string {
  return raw.replace(/[\r\n]+/g, '，').replace(/\s+/g, '');
}
 ...
 *
 * 【否定词后面还得放行一个「有」】（第五轮复审第三条，2026-09-10）上一版的否定词
 * 与那个动词之间只放行「给 / 给我」，于是口语里最常见的「没有」整类取不到事由：
 *「工资没有发」「工资一直没有发」「社保没有交」「公司没有给我交社保」——四条都真是欠薪
 * 与未缴社保，却因为多了一个"有"字判不出来，一份真发过被迫解除通知的档案落「缺失」。
 * 加的是一格可选的 `(?:有)?`，**不动双重否定那道守卫**：「工资不会不发」照旧不认
 *（那道守卫拦的是事由词与否定词**之间**，与这一格不在同一处）。
 */
const FORCED_CAUSE_SRC = String.raw`(?:拖欠|欠薪|克扣|未(?:足额|及时|按时)?(?:支付|发放)|未(?:依法)?(?:缴纳?|交)(?:社保|社会保险|五险)|工资(?:(?![没未不])[^，。；！？、]){0,6}[没未不](?:有)?(?:给我?)?[发给结付]|[没未不](?:有)?(?:给我?)?[发给结付](?:我)?(?:的)?工资|欠着[^，。；！？、]{0,4}工资|社保(?:(?![没未不])[^，。；！？、]){0,4}[没未不](?:有)?(?:给我?)?[交缴]|[没未不](?:有)?(?:给我?)?[交缴](?:我)?(?:的)?(?:社保|社会保险|五险))`;
...
/**
 * 「向 / 给 / 对 + 公司一侧 +（的）」这个**收件人前缀**。它自己不匹配任何内容词，
 * 只用来说明"紧跟在它后面的那个公司词还是收件人的一部分"，见 `NOT_COMPANY_SUBJECT` 第四条。
 */
const COMPANY_RECIPIENT_SRC = String.raw`[向给对]${COMPANY_SIDE_SRC}的?`;
/**
 * 公司一侧那个词**不是这句话的主语**时的四种前缀。`COMPANY_ACTOR_SRC` 与
 ...
 *   ·「向/给/对 + 公司一侧 +（的）」⇒ **紧跟着的那个部门名 / 职务名还是同一个收件人**
 *     （第五轮复审第一条，2026-09-10）。上一条只看紧挨着的那一个字，于是收件人写全称的
 *     那一整类反过来被判成公司作出的解除：「向公司**人力资源部**发出被迫解除劳动合同通知书」
 *    「给公司的**人力资源部**发了被迫解除通知书」「已向用人单位**人力资源部**邮寄…」——
 *     介词管的是前面那个「公司 / 用人单位」，而部门名自己前面挨着的是「司 / 的 / 位」，
 *     于是它落进公司一侧的主语短语：一边把发出动词挡在 `COMPANY_ACTOR_SRC` 后面
 *    （我方发出认不出来），一边让 `COMPANY_TERMINATED` 把「人力资源部发出被迫解除」
 *     读成公司解除，两个方向同时判反。收件人是**一整个短语**，不是一个词。
 */
const NOT_COMPANY_SUBJECT = String.raw`(?<!(?:[向给对]|作为))(?<!(?:我|本人)作为[^，。；！？、]{0,4})(?<!${COMPANY_RECIPIENT_SRC})`;
 ...
 *
 * 【「被迫解除」只能是我方那一份】（第五轮复审第一条，2026-09-10）「被迫解除 / 被迫终止」
 * 是第三十八条那件事在本行当的固定说法——**没有**"公司被迫解除"这回事。不摘出去的形态是：
 *「公司人力资源部拒收被迫解除劳动合同通知书」里公司词 + 十字内的「解除」凑成一条公司解除，
 * 于是 `forcedTerminationNotice` 的④把一份真寄出去的通知判成"公司作出的解除"、整条落「缺失」。
 * 所以那个解除动作前面紧挨着「被迫」时，这一条不成立。
 */
const COMPANY_TERMINATED = new RegExp(
  String.raw`${NOT_COMPANY_SUBJECT}${COMPANY_SIDE_SRC}.{0,10}(?:通知我|把我|将我|对我)?(?<!被迫)(?:解除|辞退|开除|终止|解聘)`,
);
 ...
 *
 * 【裁员 / 末位淘汰只认落到本人头上的那一形（第五轮复审第二条，2026-09-10）】
 * 上一版把光杆的「裁员」「末位淘汰」当决定动作，而这两个词说的是**公司的一项动作或制度**，
 * 不是"这个人已经被解除了"：「公司宣布裁员 20%，还没找我谈」「公司在裁员，但我不在名单里」
 *「公司启动裁员，暂时没轮到我」「HR 约谈说末位淘汰，让我自己考虑」——四条全部命中，
 * 而这四条正是核心客户窗口最典型的档案（风声起、还没轮到自己）。抬起来的后果是三层：
 * 三张卡一起写「成立·待证」⇒ 争点表规则三让他去固定「公司那份书面决定」⇒ 而那份纸不存在。
 * 所以这两个词收成**带本人宾语或已发生**的形态：被裁 / 裁掉 / 把我裁 / 裁了我 /
 * 通知我被裁 / 通知我被末位淘汰 / 裁员名单里有我。收窄同时作用于未定态那两条
 *（它们拼的是同一份源串），方向一致：光杆裁员本来就不该被当成"那个决定"。
 */
const COMPANY_DECISION_ACT_SRC = String.raw`(?:解除|终止|辞退|开除|解聘|除名|不再?续签|不予续签|到期不续|被裁|裁掉|裁了我|[把将]我[^，。；！？、]{0,4}裁|裁员名单[^，。；！？、]{0,4}有我|被末位淘汰)`;
const COMPANY_DECISION_ACT = new RegExp(COMPANY_DECISION_ACT_SRC);
/**
 * 那几份**文书名**本身。（第五轮复审第五条，2026-09-10）文书名递到手上就是那个决定，
 * 不必另有一个决定动词——决定动词那份表是会收窄的，收窄时文书名不该跟着一起丢。
 * 要与下面那格"已经送到了"同现才算数：只认文书名的形态是
 *「HR 说下周寄解除劳动合同通知书」也被当成已经作出的决定。
 *
 * 【「裁员通知书」是这一条唯一压得住的那一格】前四份文书名里都嵌着一个决定动词
 *（解除 / 终止 / 辞退 / 解聘），所以它们有没有这一条都认得出。而上面那份表这一轮收掉了
 * 光杆「裁员」（理由见那里），于是「公司送达了裁员通知书」变成一份**没有决定动词**的文书——
 * 一张已经送到手上的纸，三条要件却写着「缺失」。它正是这一条要接住的那一格。
 */
const COMPANY_DECISION_DOC = /(?:解除|终止)劳动合同通知书|辞退通知书?|解聘通知书?|裁员通知书?/;
/**
 * 那份文书**已经送到**的几种说法（与 `COMPANY_DECISION_DOC` 同现才算）。
 *
 * 【「给我」要摘出「给我们」（经理裁决，2026-09-10 台账「公司动作」票）】
 *「公司发了裁员通知给我们全体」「公司群发裁员通知给我们」说的是一份发到全员的通知，
 * 收件人是"我们"、不是这个人——与上面【④ 面向全员的方案】同一件事，
 * 只是它走的是文书名那条路（裁员通知书里没有决定动词，靠这一格才算数）。
 */
const COMPANY_DECISION_DELIVERED = /送达|收到|寄来|给我(?!们)/;
/**
 ...
 * 【⑦ 这一小句说的是别人身上的事】（第五轮复审第二条，2026-09-10；
 *   收窄经经理裁决，2026-09-10 台账「公司动作」票）
 *  「同事被辞退了，我很担心」——决定动作是真的，只是它没落在这个人头上。裁员这一轮里
 *   这类记录最多（同事一个个走、自己还在），而它与"我被辞退了"在词面上只差一个主语。
 *
 *   【为什么不是光那几个裸词】上一版一见「同事 / 其他人 / 他人」就把那一小句作废，
 *   于是**这些词做事由、做旁人、做同被裁的人**的那一整类记录被判没：
 *  「公司以我和同事打架为由开除了我」「公司以我辱骂同事为由辞退我」
 *  「公司以泄露他人信息为由解除劳动合同」——决定已经作出、理由里写着别人；
 *  「公司通知我和其他人一起被裁」「公司辞退了我和另外两个同事」——这个人就在名单里；
 *  「HR 当着其他同事的面通知我被辞退」——别人只是在场。七条全落「缺失」，
 *   清单让他们去记一条刚记下的事件。所以这一格收成「**那个人是被动作的对象**」的形态：
 *   人称词后隔不过 4 个字就是「被」（「同事被辞退」「别人被裁」），且不跨标点。
 *
 *   【两道摘除，为的是"我也在里面"那几条】人称词与「被」之间夹着我（「同事和我一起被裁」）、
 *   或人称词本身挂在我后面（「我和其他人一起被裁」），说的都是这个人也被裁了：
 *   前者靠中间那段不许出现「我」摘出去，后者靠「我+和与跟及」的前置断言摘出去。
 *  「其他人」里嵌着「他人」，会绕开那个前置断言，所以「他人」这一支另摘掉前面的「其」。
 */
const COMPANY_DECISION_UNSETTLED = new RegExp(
  [
    String.raw`说要|打算|可能|也许|或许|风声|传闻|听说|据说|放话|将要|有意|意向`,
    String.raw`(?<!我[和与跟及])(?:同事|同组|其他人|别人|(?<!其)他人)[^，。；！？、我]{0,4}被`,
 ...
 * 不是对方作出了那个决定。
 * 【这张表管的只剩"没选过类型的那些记录"】（2026-09-11 台账「协商一致解除」票）协商一致
 * 解除这一档现在由 N-2a 按**事件类型**认下来（NEGOTIATED_OR_DECISION_CHECKS），
 * 而这张表是**正则兜底**那条路上的，它**不跟着放宽**：从一段自由文本里认"这份协商解除
 * 协议签没签字"认不准，而认错的代价是把一个还在被催签的人判成"这一项已经立住"。
 * 选过类型的记录根本跑不到这里（类型优先，谓词不再跑）。
 *
 ...
 * **别人只是写在事由或名单里的那几条**（见排除项⑦的收窄）——
 * 公司以我和同事打架为由开除了我／公司以泄露他人信息为由解除劳动合同／
 * 公司通知我和其他人一起被裁／HR 当着其他同事的面通知我被辞退。
 ...
 * 公司群发裁员通知给我们（收件人是"我们"，不是这个人）；
 * **落在别人头上的那一类**——同事被辞退了，我很担心／别人被裁了；
...
function companyTerminationDecision(raw: string): boolean {
  ...
  return s
    ...
    .some(
      (c) =>
        c !== '' &&
        // 决定动词，或者"那份文书已经送到手上"——后者不必另有一个动词，见 COMPANY_DECISION_DOC。
        (COMPANY_DECISION_ACT.test(c) ||
          (COMPANY_DECISION_DOC.test(c) && COMPANY_DECISION_DELIVERED.test(c))) &&
...
const LABOR_TIMELINE_EVENT_TYPES = {
  公司动作: [
    ...
    },
    {
      ...
      label: '协商解除提议或协议',
      hint: '公司递来《协商解除协议》、催你签字、谈补偿方案。协商不是公司单方作出的决定，所以它与上一格分开记——违法解除赔偿金那一项不认这一格。已经协商一致签完字的，把那份已签协议按「公司文件」登记为证据，经济补偿那一项认它；只是递过来、还没签的那一份是提议，不算。',
    },
    {
      id: 'company_notice',
 ...
 *（2N-2 / 2N-3 / counterpartyDecision 三处共用；N-2a 多认一种类型，它那一行见
 *  `NEGOTIATED_OR_DECISION_MISSING_AS`。） */
const COMPANY_DECISION_MISSING_AS =
  '公司的解除/终止决定（请把公司作出这个决定的那一刻记成一条时间线事件，' +
  '并把类型选成「公司作出的解除/终止决定」——选了就不用靠那段字去猜）';

/**
 * N-2a 那个槽判不过时，缺口清单里那一行的名字。上面那一行**再加协商一致解除那一档**。
 *
 * 【为什么它与上面那行不能共用】N-2a 覆盖的是第四十六条第二至七项，第二项就是
 *「用人单位提出协商一致解除」——它由 `company_negotiation` 承载（见下面那份判定）。
 * 沿用上面那行的形态是：一个已经协商一致签完字的人读到缺口，那行字从头到尾只让他去选
 *「公司作出的解除/终止决定」，而他手上那份已签协议对应的是另一格——他要么选错类型、
 * 要么以为系统不认他这件事。禁令配出路（设计稿 §7.7）要求这一行说得出他该怎么做。
 *
 * 【为什么还要写一句"没签的不算"】两格在登记面上长得一样：递来还没签的那一份与
 * 已经签完字的那一份都选「协商解除提议或协议」。签没签由**证据那一格**分辨
 *（已签协议按材料登记进「公司文件」），而这句话是登记时唯一说得清这件事的地方。
 */
const NEGOTIATED_OR_DECISION_MISSING_AS =
  '公司的解除/终止决定（请把公司作出这个决定的那一刻记成一条时间线事件：' +
...
/**
 * 「公司确实作出过那个解除 / 终止决定」认哪两个槽。
 * **四处共用这一份槽**：要件卡 2N-2、2N-3、N-2a，以及本包声明的 counterpartyDecision
 *（争点表规则三据它判「对方的书面决定是否在档」）。
 *
 * 【槽四处共用，判定分两份】（2026-09-11 台账「协商一致解除」票）取值判定此前也是四处共用，
 * 而这一票让 N-2a **多认一种事件类型**（协商解除），其余三处不许跟着动：详见下面两份判定
 * 各自的抬头。拆成「槽」与「判定」两个常量，是为了让"哪一处不一样、为什么"只有一个地方可读；
 * 合成一份再在 N-2a 处内联一份字面量的形态，正是第四轮复审点名的那种"独立写 N 次"。
 *
 ...
 */
const COMPANY_DECISION_SLOTS = ['evidence:公司文件', 'timeline:公司动作'] as const;

/**
 * 判定：那条「公司动作」记的是不是**公司单方作出的**那个解除 / 终止决定。
 * **三处共用**：2N-2、2N-3（违法解除赔偿金那一项）与本包的 counterpartyDecision（规则三）。
 *
 * 【类型优先，谓词兜底】（2026-09-10/11 台账「结构化决定」票）登记时选过类型的记录
 * **只按类型判**：选了这一格就是公司作出的那个决定，选了别的（协商提议、其他通知）
 * 就不是，谓词不再跑。没选过的（存量行、以及文件提取一类不猜的写入）才走谓词。
 *
 * 【协商解除为什么不在这里】（沿用经理裁定，2026-09-10）协商一致解除不是用人单位
 * **单方**作出的决定：2N（违法解除赔偿金）不因协商解除成立，司法解释（一）第四十四条
 * 那句「因用人单位作出的……决定」的举证倒置也不该因它触发。所以这三处只认
 * `company_termination`——N-2a 多认协商那一格，见下面那份判定。
 */
const COMPANY_UNILATERAL_DECISION_CHECKS = {
  'timeline:公司动作': {
...
/**
 * 判定：那条「公司动作」记的是不是**第四十六条第二至七项那一类"由用人单位一侧发生"的
 * 解除 / 终止**。**只有 N-2a 用这一份**。
 *
 * 【它与上面那份差在哪，为什么】多认一个 `company_negotiation`。第四十六条第二项
 *（用人单位提出、双方协商一致解除）本来就是经济补偿的一条独立路径，而那一档在时间线上
 * 落的正是「协商解除提议或协议」这一格。不认它的形态是——一个已经协商一致签完字的人，
 * N-2a 写着「缺失」：N 这一项的两条路径（N-2a / N-2b）双双立不住，风险档位落到「依据不足」，
 * 争点表让他去补一份他永远不会有的《解除通知书》，而他手上那份已签协议一个字都没被提到。
 *
 * 【为什么 2N-2 / 2N-3 与规则三不跟着认】协商解除成立时 2N（违法解除赔偿金）本就不成立，
 * 举证倒置也不该被一份"双方谈成的协议"触发（2026-09-11 台账「协商一致解除」票）。
 * 于是本包里 N-2a 与 counterpartyDecision 的取值判定**不再完全同值**——这是刻意的，
 * 不是第四轮复审那种"漏改其中一处"：差别只在 acceptsType 多的那一项，谓词与槽仍然同源。
 *
 * 【正则兜底不跟着放宽】没选类型的老记录仍然只走 `companyTerminationDecision`，
 * 协商类文本照旧不认（`COMPANY_DECISION_NEGOTIATED`）。理由是误差方向：
 * 从一段自由文本里认"这是不是一份已经签了字的协商解除协议"认不准，
 * 而认错的代价是把一个还在被催签的人判成"这一项已经立住"。只有**显式选过类型**才走这条路。
 */
const NEGOTIATED_OR_DECISION_CHECKS = {
  'timeline:公司动作': {
    acceptsType: ['company_termination', 'company_negotiation'],
    accepts: companyTerminationDecision,
    missingAs: NEGOTIATED_OR_DECISION_MISSING_AS,
  },
} as const;

const COUNTERPARTY_DECISION = {
  slots: COMPANY_DECISION_SLOTS,
  slotChecks: COMPANY_UNILATERAL_DECISION_CHECKS,
...
const LABOR_ELEMENT_CARDS: readonly ElementCard[] = [
  ...
  // 一个拿着《解除通知》的用户读到「依据不足」，还被指去准备一份他从没发过的被迫解除通知。
  {
    ...
    // 事件由《解除通知》一类材料提取写入（书证档）⇒ 「成立」。
    // 【协商一致解除这一档落在哪】（2026-09-11 台账「协商一致解除」票）时间线上那条
    // 「协商解除提议或协议」（company_negotiation）**本卡认，其余三处不认**：第四十六条
    // 第二项的协商一致解除本来就是经济补偿的一条独立路径，而 2N（违法解除赔偿金）
    // 与举证倒置都不因协商解除成立。所以这里挂的是 NEGOTIATED_OR_DECISION_CHECKS，
    // 与 2N-2 / 2N-3 / counterpartyDecision 那份**槽相同、判定多认一种类型**，
    // 两份判定各自的抬头写着为什么。不认它的形态是——一个已经协商一致签完字的人，
    // 这一条写着「缺失」，同组那条被迫解除路径也立不住，于是 N 整项落到「依据不足」，
    // 清单从头到尾只在讲《解除通知书》，他手上那份已签协议一个字都没被提到。
    satisfiedBy: COMPANY_DECISION_SLOTS,
    slotChecks: NEGOTIATED_OR_DECISION_CHECKS,
    typicalEvidence: [
      '《解除劳动合同通知书》或协商解除协议原件',
      '离职证明',
      '合同期满不再续签的书面通知',
      '把公司作出解除/终止的那一刻记进时间线（哪天、谁通知的、怎么通知的）——员工手册、规章制度、工资结构表也归在「公司文件」类别下，但它们都不是公司的解除决定',
      '协商一致解除的：把那份**已签**的协商解除协议按「公司文件」登记为证据，并把那条事件的类型选成「协商解除提议或协议」——这一项认它（第四十六条第二项本来就是经济补偿的一条路径）；只是递过来、还没签的那一份是提议，不算',
    ],
  },
  {
    id: 'N-2b',
...
export const LABOR: DomainPack = {
  ...
  copy: {
    ...
    pages: {
       ...
       * 卷宗栏时间线头部那个登记入口上的字。
       *
       * 【为什么它也在领域包里，哪怕两个行当写的是同一句】这是**按钮上的动词**，
       * 今天两个行当恰好都说「记一件事」。写死在组件里的形态是：第三个行当接进来、
       * 那个行当管这件事叫别的说法时，没有一处会报错——只有那个行当的用户
       * 读到一句不属于他们的话。同字面不等于同一份字（见 draftsIntroAfter 那一键）。
       */
      timelineAddLabel: '记一件事',
      /**
       * 卷宗栏时间线的空态：**一条都还没有的时候，告诉他从哪儿记起**。
       * 逐个行当不一样的是「都有谁在做事、做的是哪一类事」——这一句要举得出例子，
       * 举一个另一个行当的例子等于让他照着别人的剧本回忆自己的事。
       */
      timelineEmpty:
        '这一栏按日子记下这个案子发生过什么：公司哪天找你谈、哪天发了通知，' +
        '你哪天回了话、递了函。现在一条都还没有——从你印象最深的那一天记起就行，记错了补一条新的。',
