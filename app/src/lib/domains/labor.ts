 ...
 * 说的就是我寄的）。发出动词紧跟在公司一侧的主语后面时不算——「公司提出解除劳动合同」
 * 里的「提出」是公司做的。
 */
const SENT_BY_ME = /(?<!公司|HR|单位|老板|人事)(?:发出|发了|发过|发函|发送|寄出|寄送|邮寄|送达|递交|提交|提出|EMS)|(?:通知|告知)公司|向公司/;
/**
 * **我方发出**的第二种形态：「以〈第三十八条的事由〉为由 + 解除」。主语同样省了，但事由
 * 绑在「为由」上，说的是我据此解除（「以公司未缴纳社会保险为由解除劳动合同」）。
 * 前面紧挨着公司一侧的主语时不算：「公司以拖欠工资期间旷工为由解除劳动合同」——
 * 同一个「以…为由」，那是公司给的理由。
 */
const CITED_BY_ME = new RegExp(
  String.raw`(?<!公司|HR|单位|老板|人事)[以因].{0,6}${FORCED_CAUSE_SRC}.{0,8}(?:为由|为理由).{0,6}(?:解除|终止)`,
);
/** **公司一侧作出**的解除（含辞退、开除、解聘）。 */
const COMPANY_TERMINATED = /(?:公司|HR|单位|老板|人事).{0,10}(?:通知我|把我|将我|对我)?(?:解除|辞退|开除|终止|解聘)/;
