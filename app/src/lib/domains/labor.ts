...
const LABOR_ELEMENT_CARDS: readonly ElementCard[] = [
  ...
  },
  {
    ...
    // 它是用户自述档，所以两个槽都在档时这一条的天花板是「成立·待证」——这句话是实话：
    // 在回执进档之前，"我发过那份通知"就是只有你自己说。
    //
    // 【这仍然不是精确判定，记在这里】这个行当的证据类别表里没有「我方文书」那一格
    //（lib/evidence/categories.ts 八类），时间线也只有四种 kind（cases.TIMELINE_KINDS）：
    // 一条别的「我方动作」（比如发过异议函）配上聊天截图，仍然会把这一条抬到「成立·待证」。
    // 要根治得给证据库加一类、或给事实槽加"取值判定"这一层，那是另一票（复审 2026-09-10）。
