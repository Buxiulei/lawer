 ...
 */
const COMPANY_ACTOR_SRC = String.raw`${COMPANY_SIDE_SRC}(?:(?!我|本人)[^，。；！？、]){0,4}(?:[向给对]我)?`;
/**
 * **我方发出**的形态：第一人称或省略主语的发出动词（中文里主语常省，「寄出解除通知」
 * 说的就是我寄的）。发出动词落在公司一侧的主语短语后面时不算——「公司提出解除劳动合同」
 *「人事经理提出解除劳动合同」「公司给我发了解除通知」里的那个动作都是公司做的。
 */
const SENT_BY_ME = new RegExp(
  String.raw`(?<!${COMPANY_ACTOR_SRC})(?:发出|发了|发过|发函|发送|寄出|寄送|邮寄|送达|递交|提交|提出|EMS)|(?:通知|告知)公司|向公司`,
);
/**
...
/** **公司一侧作出**的解除（含辞退、开除、解聘）。 */
const COMPANY_TERMINATED = new RegExp(
  String.raw`${COMPANY_SIDE_SRC}.{0,10}(?:通知我|把我|将我|对我)?(?:解除|辞退|开除|终止|解聘)`,
);
 ...
 * 哪怕同一段里提到欠薪）；**被逼着自己走**（HR 逼我签字后被迫离职／被公司逼迫辞职）——
 * 那是胁迫辞职，走的不是第三十八条。
