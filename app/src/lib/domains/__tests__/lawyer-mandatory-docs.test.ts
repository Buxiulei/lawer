...
// 反而钉不住。件数是四份文档里唯一一处"抄错了就一定是错的"的地方。
//（例外是 lib/paste/guide.ts：它是代码不是文档，措辞不必迁就人读，
// 所以那一份连**两件事的枚举本身**也钉住——见下面最后一条。）
...
describe('抄了"只有 N 件"的每一处都跟着 DomainPack.lawyerMandatory 走', () => {
  ...
  /**
   * 【为什么只给无工具模式那一份再加一条"枚举本身"】上面几条一律只数**件数**。
   * 只数件数的形态是：有人把 guide.ts 第 4 条那句枚举删掉、只留一句"只有两件"，
   * 件数照样对得上，判据全绿——而那条路径上的模型**读不到 DomainPack.lawyerMandatory**，
   * 于是它知道有两件却不知道是哪两件，只能自己补一个（"金额较大的""案情复杂的"），
   * 补出来的那一条读起来完全负责任。别的四份是写给人读的文档，措辞各写各的、不该逐字钉；
   * 这一份是**代码里唯一的清单副本**，钉得起，也必须钉。
   */
  it('lib/paste/guide.ts 把那两件事逐条列了出来（变异：删掉枚举只留「只有两件」→ 红）', () => {
    const text = fs.readFileSync(path.join(REPO, 'app/src/lib/paste/guide.ts'), 'utf8');
    for (const item of ['诉讼代理与辩护', '以律师事务所名义出具律师函']) {
      expect(
        text.includes(item),
        `缺什么：无工具模式那份指南（NO_TOOL_GUIDE 第 4 条）里找不到「${item}」。\n` +
          '为什么缺：上面几条只数件数，枚举被删成一句"只有两件"也照样绿。\n' +
          '怎么办：把这一件按原样补回第 4 条。那条路径上的模型读不到清单，' +
          '只告诉它件数，它会自己编出是哪几件。',
      ).toBe(true);
    }
  });
...
describe('诉讼代理那一条同时给出非律师的代理途径（民诉法 §61）', () => {
  ...
  it.each(withAgency.map((x) => [x.key, x.item] as const))(
    ...
    (_key, item) => {
      ...
      /**
       * 【为什么还要一条负判据】上面三条只问"§61 那几类**写了没有**"。
       * 只问写没写的形态，正是 2026-09-07 第三次复审逮到的那一条：前半句断言
       *「替你出庭的那个人只能是执业律师」，后半句又把 §61 那几类列了出来——
       * 三条正判据全绿，而这一条**自己和自己打架**，用户读到的是前半句。
       * 所以把那个断言本身钉成不许出现：改口时把整句改掉，不是在后面补一句了事。
       */
      expect(
        why,
        '缺什么：这一条里仍然写着"（替你出庭的）那个人只能是执业律师"，' +
          '而同一条后面又列出了 §61 允许的非律师代理人——前后自相矛盾。\n' +
          '为什么缺：补全比改写省力，于是新说法被追加在旧断言后面，旧断言没人删。\n' +
          '怎么办：按《民事诉讼法》第六十一条一次列全可被委托的那几类，' +
          '再说明我们不属于其中任何一类，不要保留"只能是执业律师"这半句。',
      ).not.toMatch(/只能是执业律师/);
    },
  );
});
