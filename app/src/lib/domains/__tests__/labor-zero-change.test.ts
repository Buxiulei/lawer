...
//
// ── 基线换过的第三次（S6 第四轮复审整改，2026-09-10）──
// `caseFacts` 里**只有一行变了**：要件表那一节 2N-2 那行的「需补」列末尾多了一项
//「把公司作出解除的那一刻记进时间线（哪天、谁通知的、怎么通知的）……」。
// 【为什么这是承诺真的变了】2N-2「公司单方解除或终止了劳动合同」此前只认「公司文件」
// 一个槽，这一票把它收窄成与 2N-3 / N-2a / counterpartyDecisionSlot 同一对槽
//（COUNTERPARTY_DECISION_SLOTS：公司文件 + 时间线上的「公司动作」）。缺口多了一格，
// 「需补」就得把那一格该怎么补告诉用户——不加的形态是：要件表点名缺 timeline:公司动作，
// 而下面那句「需补」从头到尾只在讲文件，用户传了一份又一份，那一项一个字都不变。
// 这一节其余各行、以及这份夹具的三态计数（成立·待证 2、缺失 3）**一个字都没动**：
// 该夹具本来就没有「公司文件」类材料，2N-2 收窄前后都是「缺失」。
//
