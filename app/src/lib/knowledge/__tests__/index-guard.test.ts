...
describe('🔴 manager 2026-08-29 裁定新加的四道（此前全部放行）', () => {
   ...
   *
   * 【口径由谁定】三支在这里分叉（ws/p4-w1 未注册即抛、ws/p4-w2 排除），
   * 按裁决统一成排除 + console.error 点名。本条钉的就是这个裁决。
   *
   ...
   * 所以这条同时验**三件事**：那条真的没进来、别的条目一条不少、日志点了名。
   */
  test('⑨ domain 是注册表不认识的 → 排除该条并点名，其余卡照常可用', () => {
    ...
    expect(packs.some((p) => p.id === victim), '未注册域的那条卡还是被加载进来了').toBe(false);
    // ② 出声了，且点了名：哪条卡、哪个 domain、怎么办（缺什么/为什么缺/怎么办）
    expect(err, 'console.error 没出声 ⇒ 知识库少了一批卡而没人知道').toContain(victim);
