 ...
 * ALTER TABLE 放行按位置给（addColumnIfMissing 函数体内），靠它划范围。
 * 找不到函数、或花括号不配对一律返回 null——调用方 fail closed（定位不到就全拦）。
 */
...
// 头部注释此前写死「runMigrations() 的 N 个 db.exec()」并由本文件机检 N==实测。
// 那个 N 是每加一条迁移（本仓一表一 db.exec）就得手改的常数，合并时五支各停在旧数、
// 合并后无一正确——正是要消除的那类写死计数。已从顶部注释去掉该数字（迁移步数不再入注释），
// 对应的 execCountClaim / 计数断言一并撤除；「无事务、纯加法才安全」的告警本身仍在。
