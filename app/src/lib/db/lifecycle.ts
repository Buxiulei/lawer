...
// ───────────────────────────── 同意台账 ─────────────────────────────
// 同意台账的读写**不在本文件**：那张表只有 lib/db/consents.ts 一个入口（合入 P5-C1 时
// 按裁决合表并入，判据 lib/__tests__/consent-single-writer.test.ts 会点名任何第二处）。
// lib/lifecycle/consents.ts 从那里取 findConsent / listConsents / revokeConsent。
