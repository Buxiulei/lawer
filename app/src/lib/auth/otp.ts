...
//     ⚠️ 这一条的口径已裁为 A 案，见 docs/adr/003。下面所有关于它的注释都是
//     A 案的实现说明，口径依据见 ADR-003（已接受 A 案）。
 ...
 *
 * 【状态：已裁 A 案（ADR-003）】工单只写了「允许保留一个 ≤5 秒的防连击闸，若已有就复用」，而基座并无短闸；
 * 新建它同时把工单判据「失败后立即重发放行」改写成了「5 秒后放行」。这一步是 ADR-003 的 A 案，
 * 已获裁为 A 案（ADR-003）；B 案仅历史备案。
 *
...
function checkSendQuota(
  ...
  if (store.countSendAttemptsSince(db, target, burstFrom) > 0) {
    ...
    return fail(
      429,
      'SEND_TOO_FAST',
      `刚才刚点过一次发送，请 ${SEND_BURST_SECONDS} 秒后再点一次。` +
...
export function provisionOnRegistered(
   ...
   * 省略即缺省领域——**注册页只有在开关开了不止一个领域时才摆那个选择控件**，
   * 摆不出来时这里就该落缺省，而不是拿一个页面没问过的值去建案。
   */
  domain?: string,
