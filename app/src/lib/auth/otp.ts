...
//     ⚠️ 这一条的**口径尚未裁决**，见 docs/adr/003（提案 A 案）。下面所有关于它的注释都是
//     A 案的实现说明，不是口径依据；合并前经理须在 ADR-003 的 A/B 间二选一。
 ...
 *
 * 【状态：未裁决】工单只写了「允许保留一个 ≤5 秒的防连击闸，若已有就复用」，而基座并无短闸；
 * 新建它同时把工单判据「失败后立即重发放行」改写成了「5 秒后放行」。这一步是 ADR-003 的 A 案，
 * **尚未获裁**。B 案（撤掉整块、维持原判据）的可应用补丁与实测见 ADR-003。
 *
