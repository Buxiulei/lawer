...
/** 排队用例专用：把闸占满，返回一组释放函数（用例结束必须全放，否则污染同 provider 的后续用例）。 */
async function fillGate(provider: 'openai' | 'anthropic' | 'dashscope' | 'deepseek') {
  const held: (() => void)[] = [];
  for (let i = 0; i < MAX_CONCURRENT_PER_PROVIDER; i++) held.push(await acquireSlot(provider));
