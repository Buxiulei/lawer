 ...
 */
export const PII_PATTERNS: readonly { kind: PiiKind; re: RegExp }[] = [
  {
...
/**
 * 本文件内部沿用的短名。**导出的那份是 PII_PATTERNS**：识别规则有第二个消费方了
 *（敏感级出口的不可逆脱敏，lib/sensitive.ts）。让第二处照抄一份正则的形态是——
 * 某次给手机号补了一种写法只补了一处，另一处从此漏掉那种写法，而两处都不报错。
 */
const PATTERNS = PII_PATTERNS;
