...
// 定额服务价目与阈值的**唯一读入口**：读 pricing_config 表，缺行回落本文件的代码常量。
//
...
// 所以服务定额走本文件这张独立表；billing/__tests__/pricing-config.test.ts 有结构守卫
// 遍历 skus 全部 name，出现服务名即报红。
//
// ── 常量是兜底，不是事实源 ──
// 表里有行即以表为准（改价改表、不改代码、不重启进程）；缺行才回落 FALLBACK。
...
export function readPrice(db: Database.Database, key: PriceKey): number {
  const row = db.prepare('SELECT value_int FROM pricing_config WHERE key=?').get(key) as
    | { value_int: number }
    | undefined;
  if (row === undefined) return PRICE_FALLBACK[key];
  if (!Number.isInteger(row.value_int) || row.value_int < 0) {
    throw new Error(
      `价目配置非法：pricing_config 里「${key}」的 value_int = ${row.value_int}。` +
