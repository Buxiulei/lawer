...
export const PRICE_FALLBACK = {
  ...
  'brief.per_item': 3,
  /**
   * 文书导出 PDF 每份（设计稿 §6 定为 0）。
   *
   * 【为什么 0 也要有键、也要走报价流】导出是纯本地渲染、边际成本近零，今天免费；
   * 但「免费」与「不计价」是两件事：不计价的形态是导出这条路上根本没有报价与账本这两步，
   * 哪天要收费就得回去把整条流程重接一遍，而那时它已经在线上跑着。
   * 现在按 0 走完同一条报价→确认→落账，改价只是往 pricing_config 写一行。
   */
  'draft_export.per_pdf': 0,
