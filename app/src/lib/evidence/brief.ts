...
// ───────────────────────── 生成器插座（出证收尾用） ─────────────────────────
// 出证那条线要在存证成功之后顺手补一份简报，但它不该知道简报是怎么写出来的：
// 它只调 ensureBrief，由这里决定"有没有生成器、要不要写、写不成怎么办"。
// 插座里没插东西时回 'no_generator'，安静地什么都不做——调用方（出证）不受影响。
// 生产上没人调 setBriefGenerator，attest 那边会退回上面的 generateBrief（真模型）。
