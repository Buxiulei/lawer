...
// 上传路径的两道进程级闸门：单次体积上限 + 并发上传槽位。
//
// 【谁在用】两条：POST /api/v1/evidence（证据）与 POST /api/v1/realname/passport（护照实名，
// 一次两份材料）。它们**共用**这里的常量和这一个信号量，不各开一个池——两条路把字节复制进的
// 是同一块进程内存，各算各的预算等于把 1280M 当 2560M 花，两边同时占满就直接 OOM。
//
