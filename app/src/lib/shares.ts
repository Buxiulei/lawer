...
export function readShare(db: Database, token: string): ShareReadResult {
  ...
  // 而两条路都返回 200、页面上什么都不缺。所以在这里取一次脱敏器，两条路共用
  //（要洗的还有本案登记的来访化名，那份清单也在这个脱敏器里，见 lib/sensitive）。
  const redactor = shareRedactorFor(db, row.case_id);
  if (row.target_kind === 'draft' && row.target_id !== null) {
    ...
    const redacted = redactor.text(cases.stripConfirmationFooter(draft.content ?? ''));
    return {
      state: 'ok',
      view: {
        kind: 'draft',
  ...
  if (row.target_kind === 'evidence' && row.target_id !== null) {
    ...
    return {
      ...
      view: {
        ...
        title: redactor.text(ev.name).text ?? ev.name,
        expires_at: row.expires_at,
        body: null,
        meta: redacted.meta,
        redact_notice: redactor.notice,
