...
export const CLIENT_MATRIX: readonly ClientEntry[] = [
  {
    ...
    status: 'ready',
    steps: (vars) => [
      'ChatGPT 的自定义连接器只接受 OAuth，不接受在界面里填 Bearer 密钥——走 OAuth 这条路，密钥你一个字都不用填。',
      `设置 → 连接器 → 添加，地址粘下面这个：${vars.mcpUrl}。鉴权那栏留给它自己发现（它会自己去注册一个客户端 ID）。`,
      '它会跳到本站的授权页。没登录先登录（手机号或邮箱验证码），登完自动跳回；授权页上写着是哪个客户端在申请、要哪些权限，点「同意并接入」就接完了。',
      '接完在「设置 → API key」里会多出一行「来自 <客户端名> 的授权」，吊销那一行即断开它。',
    ],
    snippetLabel: '连接器地址',
  ...
  },
  {
    ...
    status: 'ready',
    steps: (vars) => [
      `设置 → 连接器 → 添加自定义连接器，地址填下面这个：${vars.mcpUrl}。标准配置里只有 OAuth 字段，而 OAuth 正是这条路要走的，不用你填任何密钥。`,
      '点添加后它会跳到本站的授权页。没登录先登录（手机号或邮箱验证码），登完自动跳回；在授权页上确认申请方与权限，点「同意并接入」。',
      '接完在「设置 → API key」里会多出一行「来自 <客户端名> 的授权」，吊销那一行即断开它。',
      '桌面版与 Claude Code 接的是同一个服务，能力完全一样，接哪个都行。',
    ],
    snippetLabel: '连接器地址',
    snippet: (vars) => vars.mcpUrl,
  },
  ...
  },
  {
    ...
    status: 'ready',
    steps: (vars) => [
      `网页端：在连接器里填地址 ${vars.mcpUrl}，鉴权走 OAuth（它自己发现、自己注册），跳到本站授权页点「同意并接入」即可，不用填密钥。`,
      `命令行版（Kimi CLI）能填自定义请求头，把地址 ${vars.mcpUrl} 与 Authorization 头按下面这段写进它的 MCP 配置即可。`,
      '两端接的是同一个服务，档案与能力完全一样。',
    ],
  ...
  },
  {
    ...
    steps: (vars) => [
      ...
      `服务地址：${vars.mcpUrl}。鉴权那一栏能自定义请求头就填 Authorization: Bearer <你的密钥>；只给 OAuth 选项也行，点下去会跳到本站授权页，同意即接上。`,
      '两种鉴权都接不上的话，用最后一档「无工具网页对话」在豆包里照样能陪跑，只是每次要重贴开场白。',
    ],
  ...
  },
  {
    ...
    steps: () => [
      ...
      '对方回复末尾会带一个 ```tubashu 代码块，列出这一轮的新增。把整段回复复制回本页下面那张「接不了工具的客户端」卡的「粘贴回填」框，先出预览，逐条勾完再确认写入档案。',
