...
describe('共用层不许写死领域内容（设计稿 §13-6）', () => {
  ...
  const SHARED_FILES = [
    ...
    ...[
      ...
      // ↓ 出路话术（2026-09-10）新落的一处共用层：五个闸标记的字面、它们各自的一句话解释，
      //   与「用户回一句什么就能推进」的那几句。**新写的共用层文件必须当场进这份名单**——
      //   不进的形态是守卫全绿，因为它没在看那个文件。它的敞口比 statute-guard 更直接：
      //   这里每一个字都会到用户眼前（正文标记的悬停解释、提示行、一键回复的按钮文字），
      //   写死行当名词就等于对第二个行当的用户讲上一个行当的话，而三处照常渲染、照常可点。
      ...
      // ↓ 结构化「决定」标记（2026-09-10/11）从 lib/cases/index.ts 搬出来的零依赖词表。
      //   **它与 stages.ts / milestones.ts 不同：这四个值里没有任何行当名词**，所以它进的是
      //   这份名单（"往里加一个领域词即红"），不是下面那份"现存领域词逐行钉住"的待清理清单。
      //   不进任何一份名单的形态是：往它里面加一格带行当名词的事件类别，两道闸都不响
      //   （共用层守卫按文件点名，页面守卫只扫 app/ 与 components/）。
      'lib/cases/timeline-kinds.ts',
