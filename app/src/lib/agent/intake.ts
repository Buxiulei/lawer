...
export function intakeDirective(stage: IntakeStage): string {
  ...
  const lines = [
    ...
    // 【为什么把配额说成"数问号"】"1-3 个问题"是个语义概念，模型会把
    // 「报税 16,500 是税前应发吧？年终奖是每年都有还是只有去年？」当成"一个话题"，
    // 于是一轮问出五个问号。实测 S14 就是这么超的。改成可数的硬约束：
    // 发出前数一遍问号，超了就删到只剩最关键的那几个——留下的必须是"不知道就没法往下算"的。
    '**发出前数一遍你写了几个问号（？和?都算），最多 3 个。** 超了就删，删到只剩「不知道这个就没法往下走」的那几个；',
    '其余的问题留到下一轮，或者先按最常见的情况给出结论再让用户纠正——**问得少但答得准，比问全了再答有用**。',
