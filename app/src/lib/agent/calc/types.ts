...
export const CALC_FLAG = {
  ...
  /** 恒发：合同定性未定，只能给区间不给单一数字——给单一数字就是把一个存疑的问题当成定论。 */
  tuifeiRangeNotSingle: '退费给区间不给单一数字（合同定性未定）',
  /**
   * 恒发：委托合同 vs 服务合同的定性直接决定用哪条公式，而这个定性**现行法律解释存疑**。
   * 【措辞 2026-09-07 改过】原话是"未经律师书面确认"；主理人裁决没有律师签字这回事，
   * 这一项等的是法律解释有定论，不是等谁签一份意见——照旧措辞会把用户支出去。
   */
  tuifeiContractNatureUndecided: '合同定性（委托/服务）现行法律解释存疑',
