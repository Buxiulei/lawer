 ...
 * 一句话里「这件事已经办完了」的完成标记。
 *
 * 「了」单列是因为中文最常见的完成态就靠它收尾——复审探针「我把上面三件事记到档案里了」
 * 一个「已」字都没有，短语表和只认「已/帮你」的判定**同时漏网**。
 *（「好了」被「了」覆盖，不再单列。）
 */
const CLAIM_DONE = /已|帮你|为你|给你|完成|了/;
/** 「把它弄进去」的动作词。缺这一组，「档案里已经有你上传的材料清单」这类陈述会被误判成承诺。 */
const CLAIM_VERB = /挂|建|创建|写进|记进|记到|记入|加进|加到|存进|录入|生成|列进/;
