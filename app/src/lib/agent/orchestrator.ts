...
/**
 * 一句话里「这件事已经办完了」的完成标记（**不含裸的「了」**）。
 *
 * 【为什么「了」不能单列】(复审 2026-09-02 RV3-1) 裸「了」是中文里最没有信息量的字，
 * 一句纯建议也照样以它收尾：「建议你把材料清单准备好**了**再去社保中心。」
 * 配上同样判宽的动作词，这句如实话被判成承诺，系统当场给自己追加一段
 * 「补一句实话：这一轮我没能把行动卡挂进你的档案」——**因为说了实话而自我指控**。
 * 基线 d203752 对同一句不触发，是本支新增的回归。
 */
const CLAIM_DONE = /已|帮你|为你|给你|完成/;
/**
 * 「把它弄进去」的动作词。缺这一组，「档案里已经有你上传的材料清单」这类陈述会被误判成承诺。
 *
 * 【不收裸的「建」】(RV3-1)「建」命中「建**议**」——中文里最常见的**给建议**的动词。
 * 「你可以自己建一份行动卡」「建议先建个清单」都是如实话，收裸「建」就全判成谎话。
 * 只收已完成形（建好/建了/建完）与「创建」。
 * 【「进」收进来】复审新造的漏网谎话「这几件事已经进你的待办了」的动作词就是光杆的「进」；
 * 它同时覆盖写进/记进/加进/存进/列进/落进，不再逐条列（排除「进行」「进一步」这两个非产出义）。
 */
const CLAIM_VERB = /挂|进(?!行|一步)|建好|建了|建完|创建|记到|记入|加到|录入|生成|安排/;
/**
 ...
 * 这正是裸「了」判宽与这一支判准的分界。
 */
const CLAIM_DONE_TRAILING = /(挂|进(?!行|一步)|建好|建了|建完|创建|记到|记入|加到|录入|生成|安排)[^。！？；\n]{0,10}(行动卡|待办|档案|清单)[^。！？；\n]{0,4}了/;
/** 承诺的**对象**。缺这一组，「我已经帮你把邮件模板写进回复里了」也会被判成挂了卡。 */
const CLAIM_OBJECT = /行动卡|待办|档案|清单/;
 ...
 *  「行动卡我已经建好了」、「我把上面三件事记到档案里了」……
 * 一张穷举表永远追不上模型的说法，而**漏一条就是用户读到一句谎话、没有任何地方纠正它**。
 *
 * 【判定形状：同一句里三样齐了才算】完成标记 + 动作词 + 对象词。
 * 三者缺一都不成其为"卡已经在档案里了"这个断言——三个正交的条件，
 * 比一张越写越长的短语表稳，也比"含『行动卡』就算"松紧得当。
 *
 * 【为什么按句切分，而不是整段找三个词】整段找就成了"这一段里某处提到已经、
 * 某处提到挂、某处提到行动卡"——一段正常的正文很容易凑齐这三样。
 * 断言是**在一句话里**做出的，判定就该在一句话里做。
 *
 * 【完成标记不收裸的「了」、动作词不收裸的「建」】(复审 2026-09-02 RV3-1) 两个字都判太宽：
 * 「建」命中「建议」、「了」是任何中文句子都可能有的句末助词，凑上一个「清单」就成了
 * 「承诺」——「建议你把材料清单准备好了再去社保中心。」被判 HIT，系统对着一句如实的建议
 * 追加一段自我指控。完成态靠「了」的那一支改由 CLAIM_DONE_TRAILING 认（「了」须紧跟动作词+对象词）。
 *
 * 【否定必须排除，且排在最前】纠正段自己那句「我没能把行动卡挂进你的档案」、
 * 模型如实报告的「我无法直接生成行动卡」，三个条件都能凑齐——不排除就变成
 * **系统因为自己说了实话而给自己追加一段道歉**。
 * 口径偏保守：一句里带否定就整句不算，宁可漏判一次谎，也不凭空自我指控一次。
 */
export function claimsActionCardExists(body: string): boolean {
  return body.split(/[。！？!?；;\n]+/).some((sentence) => {
    ...
    if (CLAIM_DONE_TRAILING.test(sentence)) return true;
    return CLAIM_DONE.test(sentence) && CLAIM_VERB.test(sentence) && CLAIM_OBJECT.test(sentence);
  });
}
