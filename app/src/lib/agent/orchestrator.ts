...
/**
 * prompt.ts 那三句禁令的**字面形**，逐条收下。
 *
 * 【为什么在语义判定之外还留着这张表】outputDiscipline 明写不许说的是
 *「行动卡已挂上」「已记进档案」「系统会按截止时间提醒你」——**提示词禁什么，这里就认什么**。
 * 两边各写各的，就会出现"提示词禁了、纠正认不出"，或反过来"纠正在纠正一句我们从没禁过的话"。
 * 这条对应关系有判据钉着（action-card-promise.test.ts「提示词禁的那三句话一句都不许漏认」）。
 *
 * 【它补的正是语义判定够不着的那几种】「已挂上」「已挂进」没有宾语词，
 *「按截止时间提醒」根本不含产出动作——那是**声称系统会替你记着**，同样是一句不作数的承诺。
 * 语义判定管同义变体，这张表管禁令原文，两者是并集不是替代（砍表即红）。
 *
 * 【只收正向形】故意不收裸的「挂进」「记进档案」：模型说「我没能把行动卡挂进档案」
 * 是**如实报告**而不是承诺，收进来就把老实话也判成谎话了。
 */
...
/**
 * 第二人称施事 / 疑问：这一句在说**用户**做了什么、或者在问用户，不是系统在承诺。
 *
 * 【为什么单立一条】(复核 2026-09-02 RV4-1) 判定原来只认「完成 + 动作 + 对象」三样齐，
 * **完全不管这三样的主语是谁**。于是整整一族如实句被判成谎话：
 *  「你把工资流水传进档案了吗？」「你已经把解除通知传进档案了，很好。」
 *  「等你把材料上传进档案了，我再帮你逐项核对。」
 * 都是系统在**问用户 / 确认用户做过的事**，系统一个字都没承诺，却当场被追加一段
 * 「补一句实话：这一轮我没能把行动卡挂进你的档案」——用户读到的是一条自相矛盾的回复。
 * 承诺只可能是**系统自己**做出的：主语是你（用户）、或者整句是个问句，就一律不算。
 *
 * 【「？」不在表里是因为切句时已经被吃掉】句子按 [。！？；\n] 切开，问号不会留在句内，
 * 疑问只能靠「了吗」「吗」这两个句内标记认。
 */
const CLAIM_SECOND_PERSON = /你把|你已经|你先|你上次|你自己|请你|了吗|吗/;
/**
 * 系统一侧「这件事已经办完了」的完成标记（**不含裸的「了」**）。
 *
 * 【为什么「了」不能单列】(复审 2026-09-02 RV3-1) 裸「了」是中文里最没有信息量的字，
 * 一句纯建议也照样以它收尾：「建议你把材料清单准备好**了**再去社保中心。」
 * 配上同样判宽的动作词，这句如实话被判成承诺，系统当场给自己追加一段自我指控。
 *
 * 【为什么不收裸的「我」】(RV4-1) 本轮口径写的是「系统/第一人称完成态」，但「我」本身
 * **不是完成标记**，把它收进来，纠正段自己的最后一句「你回我一句『把上面几件事记进档案』，
 * 我就补上」当场判 HIT——纠正段会给自己再追加一段纠正。第一人称那一半由
 * 帮你/替你/为你/给你 与 CLAIM_DONE_TRAILING 承担，够用且不误伤。
 */
const CLAIM_DONE = /已|帮你|替你|为你|给你|完成/;
/**
 * 「把它**搬进**档案」的动作词。对象可以是行动卡/待办/档案——搬进档案是系统这一轮的动作。
 *
 * 【只收系统侧的「进」，光杆「进」去掉】(RV4-1) 光杆「进」把**用户的动作**一起收了：
 * 传进 / 上传进 / 发进 都是用户把材料交上来，「材料已经进档案了」是在陈述用户的材料到位，
 * 不是系统声称挂了卡。改成白名单：写进/记进/加进/存进/列进/落进/录进，
 * 外加「进你的档案 / 进你的待办」这一种**方向明确指向系统→用户档案**的说法
 * （复审探针「这几件事已经进你的待办了」靠它认出来）。
 */
const CLAIM_VERB_MOVE = /挂|写进|记进|加进|存进|列进|落进|录进|进你的(?:档案|待办)|记到|记入|加到|录入/;
/**
 * 「把它**造出来**」的动作词。对象**只能是行动卡/待办**，不能是档案。
 *
 * 【为什么产出动作不许配「档案」】(RV4-1) 建档是产品自己的真事件，产线文案里到处都是：
 * 「手机号和邮箱都验证过了，你的档案已经建好」(app/welcome/page.tsx:24)、
 * 「档案已建好，正在打开驾驶舱」(intake/IntakeFlow.tsx:137)。
 * 这两句一个字都没提行动卡，收进来就是纯误伤。系统这一轮**造**出来的东西只有卡与待办，
 * 档案是早就在那儿的容器——所以「建好/创建/生成 + 档案」不算承诺，
 * 「写进/落进/记到 + 档案」才算。
 * 【「安排」只收两种形】裸「安排」命中「你先把这三件事安排好」这类给用户派活的如实句。
 */
const CLAIM_VERB_MAKE = /建好|建了|建完|创建|生成|安排妥当|安排进/;
/**
 * 完成态靠句末「了」收尾的那一支：**「了」必须挂在「动作词 + 对象词」之后**，才算完成标记。
 *
 * 复审探针「我把上面三件事记到档案里了」「我把这两件事加到你的待办清单里了」一个「已」字都没有，
 * 全靠这一支认出来；而「把这三件事列进去就好**了**」（动作词后面没有对象词）照旧 MISS——
 * 这正是裸「了」判宽与这一支判准的分界。搬运/产出两组各配各的对象词，理由同上。
 */
const CLAIM_DONE_TRAILING_MOVE =
  /(?:挂|写进|记进|加进|存进|列进|落进|录进|进你的(?:档案|待办)|记到|记入|加到|录入)[^。！？；\n]{0,10}(?:行动卡|待办|档案)[^。！？；\n]{0,4}了/;
const CLAIM_DONE_TRAILING_MAKE =
  /(?:建好|建了|建完|创建|生成|安排妥当|安排进)[^。！？；\n]{0,10}(?:行动卡|待办)[^。！？；\n]{0,4}了/;
/**
 * 承诺的**对象**。缺这一组，「我已经帮你把邮件模板写进回复里了」也会被判成挂了卡。
 *
 * 【为什么裸「清单」不在里面】(RV4-1) 本产品里的「清单」绝大多数指**用户自己要准备的那份纸**
 * ——证据清单、材料清单、工作清单、债权清单，产线语料里满地都是：
 * 「立案材料清单已按此生成」(_mock/workbench.ts:69)、
 * 「把在手工作清单、对接人数变化、日均处理量写进邮件回复」(cases/jujie-xinhuo-jing03-5083.md:59)。
 * 系统这一侧的那份东西叫**待办**（「待办清单」由「待办」认出来），不靠裸「清单」。
 */
const CLAIM_OBJECT = /行动卡|待办|档案/;
/** 产出动作的对象：档案不在其中，理由见 CLAIM_VERB_MAKE。 */
const CLAIM_OBJECT_MADE = /行动卡|待办/;
/** 否定：这一句是**如实报告**，不是承诺。纠正段自己那句「我没能把行动卡挂进你的档案」正在其中。 */
const CLAIM_NEGATED = /没能|没有|未能|无法|不能|没法|不了/;

/**
 * 这段正文里有没有**声称行动卡已经存在**的句子。没有就不该有纠正段。
 *
 * 【为什么从短语表换成语义判定】(复审 2026-09-02 RV2-③) 原来的 12 条短语是**字面比对**，
 * 复审拿 9 条同义谎话探针一打，**8 条漏网**：
 *  「我已经帮你挂上了三张行动卡」（多了「帮你」二字，`已经挂上` 就对不上）、
 *  「这三件事我已经写进你的档案了」、「已为你创建了两张行动卡」……
 * 一张穷举表永远追不上模型的说法，而**漏一条就是用户读到一句谎话、没有任何地方纠正它**。
 *
 * 【判定形状：同一句里，系统自己，把卡造出来或搬进档案】
 *  ① 不含否定 ② 主语不是用户、也不是问句 ③ 完成标记 ④ 动作词 + 与之配套的对象词。
 *
 * 【为什么按句切分，而不是整段找几个词】整段找就成了"这一段里某处提到已经、
 * 某处提到挂、某处提到行动卡"——一段正常的正文很容易凑齐。
 * 断言是**在一句话里**做出的，判定就该在一句话里做。
 *
 * 【口径：宁可漏判一次谎，也不凭空自我指控一次】(manager 2026-09-02 终局裁决)
 * 四轮收窄每一轮都把误伤搬到另一族如实句上（建议→列进去就好了→你把材料传进档案了）。
 * 所以本轮把天平钉死：**误伤（如实句被判承诺）是阻断级缺陷，漏判（谎话没抓到）可以接受**。
 * 判据表因此用**产线真实语料**当如实组（证据上传页 / 行动卡与工作台文案 / prompt.ts 输出纪律 /
 * scenarios.ts 期望回复 / knowledge SOP 句式，逐句抄来带出处），不再自己造如实句——
 * 自造的如实句只会长成"我已经想到的那种误伤"，产线语料才长成"我没想到的那种"。
 */
export function claimsActionCardExists(body: string): boolean {
  return body.split(/[。！？!?；;\n]+/).some((sentence) => {
    if (CLAIM_NEGATED.test(sentence)) return false;
    if (CLAIM_SECOND_PERSON.test(sentence)) return false;
    if (ACTION_CARD_PROMISE_PHRASES.some((p) => sentence.includes(p))) return true;
    if (CLAIM_DONE_TRAILING_MOVE.test(sentence) || CLAIM_DONE_TRAILING_MAKE.test(sentence)) return true;
    if (!CLAIM_DONE.test(sentence)) return false;
    return (
      (CLAIM_VERB_MOVE.test(sentence) && CLAIM_OBJECT.test(sentence)) ||
      (CLAIM_VERB_MAKE.test(sentence) && CLAIM_OBJECT_MADE.test(sentence))
    );
  });
}
