...
describe('D14 ③「法律问答过程中不插入推销」——"过程中"是位置词', () => {
  it('★推荐是**独立段落追加在正文之后**，绝不插进正文中间', async () => {
    ...
    // 【2026-09-08 基线随 ⑥ 条号闸上线换过一次 —— 是预期改动，不是有人动了推荐段】
    // 这一轮的夹具检索里没有 §47 的逐字原文，所以 ⑥ 在条号后缀了【条号待核验】。
    // **它是增不是删**：原句一个字都没少，这正是本条要守的东西。
    // 若哪天这个标记消失了，要么是夹具开始注入 §47 原文（正常），要么是 ⑥ 停了（要查）。
    expect(result.text.slice(0, idx)).toContain('按第四十七条【条号待核验】，经济补偿按工作年限算，每满一年一个月工资。');
