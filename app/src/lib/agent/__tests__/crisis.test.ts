...
describe('危机轮出口闸：剥掉模型段重复列出的热线清单', () => {
  ...
  /* ─────────────────────────────────────────────────────────────────────
   * 【本组第一条改过，来历必须留下】(2026-08-26)
   * 原断言：「模型把整张卡又列一遍 → **清单被剥掉**」——样本里整卡在 body 里只出现**一次**。
   * 那是旧口径（含号码行 ≥2 就把**所有**含号码的行全删）的忠实执行物，**不是写错了**。
   *
   * **而 manager 2026-08-25 撤回这道闸，撤的正是那个行为**：它把被指代物整个删光，
   * 留下「热线还是这三个，随时能打：」后面空一片——**悬空**。
   * 四处待修里那条「**保留第一处、只剥后续**」就是为它写的。
   *
   * 【所以新行为是】`cardOccurrences(body).slice(1)` —— body 里第一次列卡**保留**，
   * 只剥更后面的重复。于是「热线还是这三个：」后面永远有东西，悬空在结构上消失。
   * ⚠️ 代价说清楚：用户会在**首段 + 模型段各看到一次**号码。
   * manager 08-25 已就此定性：**危机轮刷屏的代价接近零，悬空的代价不可逆。**
   *
   * 【顺带暴露的一件事，值得单独记】本闸的**产线真实输入域是 `body`，不是整轮正文**
   *（orchestrator 只把模型段喂给它，首段不参与）。所以"第一处"指的是**模型段里的第一处**，
   * 而不是"用户看到的第一处"。评测官的 S7 是在整轮文本上算的——**两者的"第一处"不是同一个**。
   * 这正是 manager 08-25 列的四处待修之一「`cardShapeAgrees` 的产线真实输入域」。
   *
   * 【被它替换掉的那条断言，原文抄在这里】——**不留成 `it.skip`**：
   * 一条永不运行的测试会烂掉，而且它的 skip 计数会一直提醒人"这里有个没人管的东西"。
   * 留在注释里既保住"我们曾经这么想过"，又不留一个不跑的躯壳。
   *     const out = stripDuplicateHotlineList(body, P);          // body 里整卡只有一次
   *     for (const p of P) expect(out).not.toContain(p);         // ← 旧行为：全剥
   *     expect(out).toContain('我不会当成「就是想想」就翻过去');
   *     expect(out).toContain('我在这儿，你回我一句就行');
   * 注意它**没有**断言「热线还是这三个，随时能打：」这句是否还在——
   * **悬空正是从这个断言的盲区里长出来的**：它检查了正文两头，没检查被剥掉那段的引导句。
   * ───────────────────────────────────────────────────────────────────── */
  it('★body 里只列一次 → **不剥**（保留第一处；旧口径在这里会剥光并留下悬空）', () => {
    const body = [
  ...
  it('★body 里列两次 → 只剥后一次，第一次与它的引导句都留着', () => {
    ...
    const out = stripDuplicateHotlineList(body, P);
    expect(out).toContain('热线还是这三个，随时能打：');
    for (const p of P) expect(out).toContain(p);          // 第一处还在
    expect(out.split('12356').length - 1).toBe(1);        // 但只剩一处
    expect(out).toContain('我在这儿。');
  });
