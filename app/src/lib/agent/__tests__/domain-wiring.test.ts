...
describe('「只能由执业律师做」的闭合清单按 cases.domain 取（变异：把 prompt.ts 里那行 renderLawyerMandatory 删掉 → 红）', () => {
   ...
   */
  it('普通轮（非危机、非空包）也带这一段，且带着那三条通用纪律（第 4 条只给声明了解释存疑节的领域）', () => {
    const p = plainPrompt(DEFAULT_DOMAIN);
 ...
 * 「解释存疑」（DomainPack.interpretationDisputed）：那几条每轮随事实卡渲染，
 * 逐条写着"现行法律解释存疑"；而闭合清单那一段写着"清单以外的每一件事都由你做完"
 * 「不许用建议咨询律师收尾」。
 *
 * 两条都进了同一份 system prompt 而**没有写明谁优先**时，模型对着同一个问题
 * （比如"我们机构算不算强制报告主体"）两种做法都说得通，两种都不会报错：
 * 按清单那段办就给出是/否结论（正是解释存疑那一节要禁的），
 * 按解释存疑那一节办就把人指向了律师（正是清单那段要禁的）。
 * 所以先后必须**写在下发给模型的字里**，而不是留在设计稿或复审记录里。
 */
describe('解释存疑那一节与闭合清单的先后写在同一段里（变异：删掉 interpretationDisputedTiebreak 那几行 → 红）', () => {
  const plainPrompt = (domain: string) =>
    buildSystemPrompt({
      snapshot: snapshotOf(domain),
      mode: '陪跑',
  ...
  const REVIEW = {
    ...
    items: [
      {
        text: '假领域存疑的那一条——现行法律解释存疑。',
        precedents: [],
        searchKeywords: ['假领域 检索词'],
      },
    ],
    discipline: '假领域的纪律句：只给依据原文与分歧点，不下结论。',
  };

  it('声明了这一节的领域：清单段里点名它，并写死"它限制结论、不是转介理由"', () => {
    const seg = renderLawyerMandatory({ ...FAKE_PACK, interpretationDisputed: REVIEW });
    ...
    // 【这一条钉的是"存疑不许改写成等签字"】主理人 2026-09-07 裁决过没有律师签字这回事：
    // 少了这一句，模型把"解释存疑"顺势说成"等一份书面意见就有定论"，
    // 而它没有违反上面任何一条，读起来也完全负责任。
    expect(seg, '没写死"存疑不是等一份书面意见"，转介会从这个口子绕回来').toContain(
      '存疑的是法律解释本身',
    );
  });

  /**
   * 【这一条钉的是"存疑之后还得做一件事"】主理人 2026-09-07 另有一条裁决：
   * **现行法律解释存疑时应找过往相似判例**——存疑不是终点，先给官方相似案例的实际裁法
   *（标明个案不是规则），再给分歧点，仍不下结论。
   *
   * 只写"照第 1 条把能做的做完"的形态是：模型读到"存疑"就直接跳到"我不下结论"，
   * 一条案例都不去取，而那一轮回复**读起来完全负责任**——它没有违反上面任何一条，
   * 用户也看不出少了什么。所以三样都得出现在下发给模型的字里：
   * 顺序（先案例、后分歧点）、每个案例后面那句"个案不是规则"、
   * 以及**库里没有时要明说**（不明说的形态是模型顺手编一个案例填上，
   * 而编出来的案例读起来最像尽责）。
   *
   * 变异：把 interpretationDisputedTiebreak 里 ①②③ 三行删回原来那两行 → 本条红。
   */
  it('清单段写死"先检索过往相似案例、标个案不是规则、库里没有要明说"（变异：删掉①②③三行 → 红）', () => {
    const seg = renderLawyerMandatory({ ...FAKE_PACK, interpretationDisputed: REVIEW });
    // ① 先去取案例，而且告诉它到哪里取（包里那两格的名字）
    expect(seg, '没说"先检索案例"，模型读到存疑就直接跳到不下结论').toContain('先检索过往相似案例');
    expect(seg, '没点名 precedents，模型不知道库里已经收了哪几张').toContain('precedents');
    expect(seg, '没点名 searchKeywords，模型不知道下一次该拿什么词去找').toContain('searchKeywords');
    // ② 摆出来的案例必须自带解毒剂：不标这句，个案裁法会被当成这一条已有定论
    expect(seg, '没写"个案不是规则"，一个个案的判法会被当成定论答给用户').toContain('个案不是规则');
    // ③ 空手时明说，而不是含糊带过或编一个
    expect(seg, '没写"库里一条都没有时明说"，模型会顺手编一个案例填上').toContain(
      '库里暂无官方相似案例',
    );
    // 顺序也在字里：先案例、后分歧点。只列三件事不定先后的形态是模型先下判断再补案例
    expect(seg).toContain('先案例、后分歧点');
    expect(seg.indexOf('先检索过往相似案例')).toBeLessThan(seg.indexOf('分歧点落在哪里'));
  });
  ...
  it('没有这一节的领域一个字都不多（变异：把它改成无条件渲染 → 红）', () => {
    ...
    expect(FAKE_PACK.interpretationDisputed, '假包本来就该没有这一节，否则下面这条恒真').toBeUndefined();
    expect(seg).not.toContain('本领域另有一节');
    expect(seg).not.toContain(REVIEW.title);
  });

  it('缺省领域（同样没有这一节）的 prompt 里也不多这一句', () => {
    expect(LABOR_PACK.interpretationDisputed).toBeUndefined();
