...
/**
 * **问诊开场那一档（A）不开口。** 这是我在 manager 批准的读法之内加的**唯一一条**收窄，
 * 不静默加，理由摆在这里，要撤只改这一个常量。
 *
 * 【为什么】案件建档时 `stage` 就是「已收通知」，于是「收到裁员通知」这个位点在**用户说的第一句话**
 * 那一轮就成立——那一刻我们还什么都没帮上，先递了一句推荐。
 *
 * 【两种错法的代价不对称，这才是真正的理由】
 ...
 * ⇒ 默认往"先帮上忙再开口"偏。
 *
 * 【它不架空 D14】五个位点一个不少，四个案件节点照常推；收窄的只是**同一位点内的最早那一轮**。
 */
