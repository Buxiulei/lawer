 ...
 *
 * 【这道过滤是第二层，不是这条通路的牙】findByArticleKeys 取卡走的是 `this.get`，
 * 而 get 面自己有领域闸——本行删掉，行为一个字都不变（复审 P4-W2 二轮实测：0 条判据变红）。
 * 留着是因为它零成本地挡住"日后有人让这条通路绕开 get"的那一种改法；
 * 但**别把它当成这条通路的守卫**：真正在守的是 get 面那一道，判据也钉在那里。
 *
