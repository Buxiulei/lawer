 ...
 *
 * ⚠️ 无价格、无链接、无「预约」二字、无数字——判据会钉死这四条（referral-d14-d15 / crisis.test）。
 */
export const CRISIS_NBDPSY_LINE =
  '热线能接住此刻。但这段时间反复压着你的那些东西，需要有人陪你慢慢理清，' +
  '我们 NBDpsy 心理咨询工作室就是做这件事的，和土八鼠是同一个体系。' +
  '等你缓过这一阵、想找人好好聊聊，跟我说一声，我把怎么联系告诉你——不急，也完全由你定。';
 ...
 *
 * 【**已声明的边界**（评测官 2026-08-26 探到，WS2 定：认下这个取舍，不加常数）】
 *   整卡给过一次之后，正文里**分三处各复述一个号码**，本原语会把它们凑成"第二次"。
 *   **用户实际只看到一张完整的卡 + 三次单号码提醒。**
 *   **这不是 bug，是"完全不看行"的另一面**——同一个性质让 S4（不相邻的整卡）成立，也让这个成立。
 *
 *   为什么不修：两个候选修法（① 闭合后加距离上界 N 字；② 要求新一组里至少有一次多号码同现）
 *   **都要引入一个没有出处的常数**，而本仓今天已经在「窗口 12 字」「作用域 60 字」上
 *   各付过一次"这个数从哪来"的代价。
 *   **实测底数**：153 轮真实语料里，有卡 28 轮、判 ≥2 次 11 轮，
 *   **走这条离散路径的 0 轮**（探针有判别力：分母非空，且它是机械可判的）。
 *   ⇒ 代价是**一类可被判准解释的假红**，而不是一个没人能预测的常数。
 *   **0 轮是今天的底数，不是"永远不会"**——它若在成绩单上出现，
 *   判据侧应把 Span 原文一并印出来，让读的人一眼看出那"第二张卡"是三处散着的复述。
 ...
 *
 * 【判据 2026-08-26 改：从「数行」改为吃 `cardOccurrences` 原语】
 * 旧版：含号码的行 ≥2 就动手，且**把所有含号码的行全删**。
 * 两处毛病：① 它数的是**行**不是**卡**——整卡写在一行里它一次都不剥，
 * 三行不相邻的散号码它反而全剥；② **「有重复」的检查不该有「全部删除」的处置权**——
 * 把被指代物整个删光，正是 manager 08-25 撤回本闸时列的「悬空指代」的成因
 *（「先把号码放这儿：」后面空了，「接通了可以照这样说：」后面也空了）。
 *
 * 新版：`cardOccurrences(body).slice(1)` —— **保留第一处，只剥后续那几处所占的行**。
 * 「保留第一处」与「悬空指代」是同一个缺失（没有"第一处在哪儿"这个概念）的两个症状，
 * 一个原语一起解决。单号码的一句话重述（「随时打 12356」）不构成一次整卡，天然保留。
 *
 * ⚠️ **本闸目前仍被 `CRISIS_HOTLINE_DEDUP_ENABLED = false` 关着，本次不开。**
 * 开关是行为面改动，要有它自己的证据；这次只把它修到"可以开"的状态。
 *
...
export function stripDuplicateHotlineList(body: string, phones: string[]): string {
  const spans = cardOccurrences(body, phones);
  if (spans.length < 2) return body;

  // 第一处占用的行**一概不动**——「保留第一处」就落在这一句上。
  const lineStart: number[] = [0];
  for (let i = 0; i < body.length; i++) if (body[i] === '\n') lineStart.push(i + 1);
  const lineOf = (pos: number) => {
    let lo = 0;
    for (let i = 0; i < lineStart.length; i++) if (lineStart[i] <= pos) lo = i;
    return lo;
  };
  const protectedLines = new Set<number>();
  for (let l = lineOf(spans[0].start); l <= lineOf(spans[0].end - 1); l++) protectedLines.add(l);

  const drop = new Set<number>();
  for (const span of spans.slice(1)) {
    for (let l = lineOf(span.start); l <= lineOf(span.end - 1); l++) {
      if (!protectedLines.has(l)) drop.add(l);
    }
  }
  if (drop.size === 0) return body; // 后续那次与第一次挤在同几行里 ⇒ 剥它就会伤到第一次，宁可不剥
...
/**
 * **本闸检出了重复、但明示放弃剥除**的那种情形。
 *
 * 【为什么要把"放弃"变成一个可查询的状态，而不是静默地原样返回】
 * 目前只有一种：**后一张整卡与第一张挤在同一行**——「保留第一处」是按**行**保护的，
 * 保护范围把同行的第二张一起保住了。**这是"原语按位置、剥除按行"的粒度差残留**：
 * 计数已经从行搬到了位置，剥除的动作单位还留在行上。（评测官 2026-08-26 探到。）
 *
 * 【为什么不修那个粒度差】改成按字符区间剥，会在行中间留下半句
 *（「…010-82951332 —— 再说一遍 」），**而"把被指代物删掉、留下指代句"正是
 * manager 08-25 撤回本闸的那个理由**。修它有相当概率把撤回的那件事重新做一遍。
 * **实测底数**：153 段模型段里，**body 内出现 ≥2 张整卡的有 0 段**——
 * 同行那一支是这个空集的子集。**失效方向还是"少做"而不是"多做"**，
 * 而 manager 08-25 已定性：危机轮刷屏的代价接近零。
 * ⇒ **认下这个边界，但不许它沉默。**
 *
 * 【为什么这比"声明一个例外"好】同源守卫（`cardShapeAgrees`）此前的前提是
 * 「数得出重复 ⟺ 产线会动手」。可产线有一种**正当的拒绝**：动手就会伤到第一处。
 * 把它做成第三态（一致 / 不一致 / **产线明示放弃**），**边界就落在守卫里面，
 * 而不是变成守卫的一个例外**——例外会被下一个人当成"这条守卫本来就不太准"。
 */
export function hotlineStripDeclined(body: string, phones: string[]): boolean {
  return cardOccurrences(body, phones).length >= 2 && stripDuplicateHotlineList(body, phones) === body;
}
