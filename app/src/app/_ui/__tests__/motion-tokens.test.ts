...
describe('token 定义与引用一一对应', () => {
  ...
  it('定义了的每个 token 都有人用——没人用的档位是在骗后来人「这里有个规范」', () => {
    ...
    // JS 侧不写 var()，它走 MOTION.tap / EASE.lin 这样的成员访问，一并算「有人用」
    const viaMirror = new Set<string>();
    for (const { text } of FILES) {
      for (const m of text.matchAll(/\b(?:MOTION|EASE)\.([a-zA-Z]+)/g)) {
...
describe('JS 镜像与 CSS 逐值一致', () => {
  ...
  it('EASE 每一条都能在 CSS 里找到同名同值的 --ease-*', () => {
...
describe('减弱动效的降级面', () => {
  ...
  it('程序化滚动一律过 scrollBehavior()，字面量只剩 A7 工单认领的那一处', () => {
    ...
    // 正对照：扫描器确实能找到 behavior: 的写法（包括已经改成 scrollBehavior() 的）
...
describe('动效时长不许出现现场数字', () => {
  it('页面里没有写死毫秒的 duration arbitrary 值', () => {
    const offenders = FILES.filter(({ text }) => /\bduration-\[/.test(text)).map(
      ({ rel }) => rel,
    );
