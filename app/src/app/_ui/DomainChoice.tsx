 ...
 *
 * 【为什么不能在服务端组件里算】灰度开关只在服务端读得到，而这个控件是客户端的。
 * 让它自己去读那个开关的形态是：运维把第二个领域打开了，页面上一个选项都不会多，
 * 而没有任何一处会报错——浏览器包里 `process.env` 只有 NEXT_PUBLIC_*。
 * 所以走一条 force-dynamic 的端点现问（见 app/api/v1/domains/route.ts）。
 *
 * 【2026-09-07 更正】/login 本身已改成 force-dynamic（协议生效旗要在服务端读定，
 * 见 app/login/page.tsx 抬头），所以"那一页是静态预渲染页"这条原因已经不成立了；
 * 领域清单**照旧**走端点，不顺手改成 prop——那是另一件事，且这条路现在没坏。
 *
