...
const NEGATED = [
  ...
  /不自称律师/g,
  /*
   * 【2026-09-07 补的两条：说的是"法律规定这件事得由律师做"，正好是自称的反面】
   * 《用户服务协议》第二条第 3 款要写清「哪些事我们做不了」：
   *「只有**法律规定必须由律师办理**的事项（例如**由律师担任诉讼代理人**、
   *  以律师事务所名义出具律师函），我们会明确告诉你这一步法律上需要律师」。
   * 这两句被 /由律师/ 那条名词式规则拦下来过一次——而它们是这份产品面上
   * **最不可能是冒用**的两句：它们的作用就是把那几件事划到我们的能力之外。
   *
   * 【为什么摘的是整句，而不是给 /由律师/ 加否定前瞻】前瞻写法会把
   *「由律师起草」这类真自称在某些上下文里一并放过（前瞻只看紧邻的几个字）。
   * 摘整句则只对这两个**逐字固定**的说法生效，其余一个字都不放松——
   * 下面「由律师起草仍然红」那条判据就是这条的哨兵。
   */
  /法律规定必须由律师办理/g,
  /由律师担任诉讼代理人/g,
];
...
describe('产品面不得以律师名义自称（律师法 §13）', () => {
  ...
  /**
   * 【协议第二条第 3 款那两句】它们说的是"法律规定这件事得由律师做，所以我们不做"，
   * 是自称的**反面**。而下面第二条是它们的哨兵：摘除只对那两个逐字固定的说法生效，
   * 「由律师起草」这类真自称一个字都没放松。
   */
  it('「法律规定必须由律师办理」「由律师担任诉讼代理人」放行（协议第二条第 3 款）', () => {
    expect(
      selfClaimsIn(
        '只有法律规定必须由律师办理的事项（例如由律师担任诉讼代理人、以律师事务所名义出具律师函），' +
          '我们会明确告诉你这一步法律上需要律师并说明原因。',
      ),
    ).toEqual([]);
  });

  it('放行只对那两句生效：「由律师起草」「由律师审核」照旧红（变异：把摘除写成 /由律师/ → 红）', () => {
    expect(selfClaimsIn('这份通知由律师起草。')).toEqual(['由律师', '律师起草']);
    expect(selfClaimsIn('每一份文书都由律师把关。')).toEqual(['由律师']);
  });
