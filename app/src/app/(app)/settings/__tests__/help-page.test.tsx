...
// 这两张是 P5-C1/C2 合入后新挂上去的（隐私与同意 / 数据权利），同样是客户端件：
// DataRightsCard 里的注销那一段调 useRouter，静态渲染时会抛「app router not mounted」。
vi.mock('@/app/(app)/settings/_components/PrivacyCard', () => ({ PrivacyCard: () => null }));
vi.mock('@/app/(app)/settings/_components/DataRightsCard', () => ({ DataRightsCard: () => null }));
