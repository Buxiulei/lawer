...
export function Workbench({ caseId }: { caseId: string }) {
  ...
  /* 【这一句为什么是中性的，不按领域取】这一屏只在**没登录**时出现（上一行的 !signedIn）。
     没登录就问不出这个案子属于哪个领域——`GET /cases/{id}` 要凭据，问了必吃 401，
     那一趟请求换回来的只会是缺省领域。所以这里不摆一个"按领域取、而永远只有一种结果"的
     假接线，改成**一句哪个行当都成立的话**：不点某个行当特有的字眼，只说要讲清哪几类事实。
     写死某个行当那一版的形态是：第二个领域的用户第一次打开工作区，
     被要求交代的是另一个行当的事，而页面照常渲染、按钮照常能点。
     那几个字眼由 app/__tests__/page-domain-guard.test.ts 的词表拦着写不回来。 */
  if (!seeded && !signedIn) {
    return (
      <div className="pt-8">
        <EmptyState
          title="这个案件还没有对话记录"
          description="先做一次首诊：把事情的经过、时间和涉及的金额讲清楚。首诊结束后这里会有时间线、诉求初算金额，以及接下来 7 天要做的事。"
          action={
            <Button asChild>
