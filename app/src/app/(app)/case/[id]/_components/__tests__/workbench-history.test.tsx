...
describe('挂载即取历史', () => {
  ...
  /**
   * 🔴 **这一屏那句话不许点某个行当特有的字**（2026-09-07 复审点名）。
   *
   * 它此前写着「把被裁的经过、工资和司龄讲清楚」——第二个领域的用户第一次打开工作区，
   * 被要求交代的是另一个行当的事，而页面照常渲染、按钮照常能点。
   *
   * 【为什么这一句是中性的，而不像别的共用页那样按领域取】它只在**没登录**时出现，
   * 那时问不出这个案子属于哪个领域（`GET /cases/{id}` 要凭据，问了必吃 401），
   * 一个"按领域取而永远只有一种结果"的接线比一句中性的话更糟：它看起来像已经处理过了。
   * 词表那一侧另有 app/__tests__/page-domain-guard.test.ts 拦着这几个字写不回来，
   * 这里再从**渲染产物**上钉一次（变异：把那几个字写回 Workbench 的 description → 红）。
   */
  it('🔴 未登录空态那句话里没有某一个行当特有的字眼', async () => {
    auth.token = null;
    const { text } = probe(await settled(CASE));
    // 先自检：这一屏真的画出来了，否则下面全是在验空集
    expect(text).toContain('先做一次首诊');
    for (const word of ['被裁', '工资', '司龄', '劳动', '仲裁', '用人单位']) {
      expect(text, `未登录空态里出现了只对某一个行当成立的字：「${word}」`).not.toContain(word);
    }
  });
});
