...
| 2026-08-28 | 土八鼠徽章**矢量稿 v4 定案入库**为品牌主资产 `app/public/brand/tubashu-logo.svg`（470KB / gzip 179KB，423 path，无外部引用）。经用户红框反馈两轮迭代：外缘碎屑→平滑、书皮「劳动法」文字保真；manager 已 2 倍放大做原图/v2/v3/v4 三方对照验收（对比页链接待补） | 与「描摹版退役」不冲突——退的是**未经用户认可**的旧描摹，这版是他逐轮验收钦点的。**≥24px 标记位不切它**：实测 24/28/32px 下与现有 raster 裁切肉眼无差（6× 放大仅眼镜边缘略清），而它 gzip 179KB vs raster 6.9KB——**侧栏每页都在，26 倍代价换不到可见收益**。favicon/PWA 多尺寸 raster 照旧不动 |
