...
### 1.x 标志性案例：赢了的案子烂在最后一公里（2026-08-25 实测）

用户原话：**「裁决下来 20 天了公司一分钱没给，电话也不接，是不是就是废纸一张」**

系统检索给出的前 6 份材料：保密协议、公司注销、证据目录、起诉状——**「申请强制执行」的操作指引一次都没进**。

> **用户已经赢了，只差"申请强制执行"这一步。系统因为他不会说那四个字，给了他公司注销和保密协议。**
> **赢了的案子烂在最后一公里。**

**量化事实**（2026-08-25 实测，18 条 query / 217 张卡）：用户原话只够得着 **75/217 张卡（34.6%）**，而模型自己写的检索词够到 **154 张**。**差的 79 张不是库里没有，是要会说法言法语才拿得到。**

**这条案例同时定义了产品的价值与失败模式**：我们服务的人**赢了官司也可能拿不到钱**，而拿不到钱的原因**不是法律不站在他这边，是他不知道下一步该说哪四个字**——而"知道下一步该做什么"正是本产品承诺的东西。

**由此确立两条产品原则**：
1. **「口语 ↔ 法言法语」映射是本产品的核心资产**，不是检索的补丁。**若系统要求用户说对词才能找到法条，它就只服务于已经懂法的人——那就背叛了立项初衷。**
2. **该资产的建设顺序按"接不上会怎样"排，不按能补多少条排**：程序型词汇（强制执行/申请执行期限/仲裁时效/管辖/一裁终局/撤裁/立案材料）优先——**程序型接不上的后果不可逆**（过期、错过执行、找错机关），情绪型接不上"只是"少一段陪伴话术。**先救"少走一年"的那类，再救"体验更好"的那类。**
...
| **D14** | **品牌定位与推荐策略（2026-08-25 用户拍板，覆盖 D9 的推荐边界）**：law.nbdpsy.com 是 **NBDpsy 体系的分支**，**主动为主线心理咨询导流**。档位=**节点推荐**：①全站品牌关联常驻（页脚/关于页/邮件签名注明「NBDpsy 体系」）；②在**情绪场景**与**案件关键节点**（收到裁员通知、立案后、开庭前、拿到结果后）主动推荐心理咨询；③**法律问答过程中不插入推销**；④**频控：同一节点只推一次，用户拒绝后不再推**。**红线改写**：原「推销 NBDpsy = L1 违规」作废，改为下方 D15 的单一禁区 |
| **D15** | **危机轮付费禁令（2026-08-25 用户拍板，L1 红线）**：识别到自杀念头/严重心理危机的轮次，**只给免费公益热线，不得出现任何付费服务入口、价格、预约链接**；情绪稳定后的轮次方可推荐。理由：危机时刻的人需要的是立刻能接通的救命通道，此时推付费服务既延误救助，也有真实的法律与舆论风险 |
| **D16** | **品牌命名与视觉方向 v3（2026-08-27 用户拍板）**：产品命名「**土拨鼠劳动仲裁**」，隐喻=土拨鼠后肢站立为群体守望；logo 原创 SVG（调研未找到可核实许可且姿势匹配的开源素材）。色板定死勃艮第红/淡金/米白。前端重构三决定：底色组可动 / 壳层留、内容区改按 GOV.UK + vercel/chatbot（覆盖 D13 主骨架）/ 全站改名。分四批见 DESIGN.md「视觉方向 v3」 |
| **D17** | **低调模式下危机热线块豁免打码（2026-08-27 用户拍板）**：低调模式立的是「不知情者」标准，**但危机轮那一刻优先级是接通不是隐藏**——热线块（`InstantReplyCard`）是全站**唯一**豁免打码的正文，页面其余照糊，三个号码渲染为 `tel:` 按钮直接可拨。触发原因：代码带 `data-veil` 而注释写「必须一眼可读」，两者对不上；且糊层的按住揭开会吞掉点击，`tel:` 按钮会变成「看不见却拨得出」。不选「照糊、揭开后才拨」：用户刚说出自伤念头时不该再被要求先按住 150ms |
| **D18** | **同主体判例引用（2026-08-28 用户拍板）**：案件判断不只引法条——必须结合**公司所在地**的劳动仲裁/司法判例；目标形态是把**所在公司主体及其关联主体**名下全部劳动争议裁判文书采集入档，作为该案件的相关判例引用。采集受 D10 约束（政务源不可无人值守代查）：裁判文书网走人机接力（外勤已有登录通路；08-21 宜信系全文命中 19,737 篇即试点语料）；关联主体图谱靠用户自查回传 + 爱企查等免登录源。落地三步：①试点=用户 1 号案（宜信系）收窄「劳动争议·近5年」并结构化（判决要素：案由/结果/金额/仲裁委·法院/年份）；②数据模型=案件关联判例库（后台技术，契约先过 manager）；③引用面=agent 检索优先级 同主体 > 同地域同案由 > 一般判例（评测判据另裁）。**判例引用必须逐字带案号，禁止编造**——零编造红线（G1）覆盖本能力 |
...
## 2c. 通知设计原则（2026-08-21 用户应激事件后确立，产品文案层铁律）

**事件**：用户（本项目 1 号用户，正在经历真实裁员）反馈收到的系统邮件让他应激——发件人叫"裁员应对"、标题带"【紧急】告警"，而其中一条还是虚报。**一个以情绪安抚为核心的产品，其通知本身成了新的压力源。**

**铁律**（notify 文案层与所有告警实现必须遵守）：
1. **默认姿态是安静**——只在用户**真需要行动**时出声。内部健康度信号（心跳、拉取失败、管道失明）**一律不发用户**，只进内部日志。
2. **中性措辞**——发件人显示名与主题禁用"裁员/仲裁/维权/紧急/警告/告警"等字样（发件人用「事务提醒」类，主题用「您关注的事项有更新」类）；具体内容放正文。**低调模式下更严**（防他人瞥见泄露案情）。
3. **讲清两件事**：发生了什么 + 你要做什么。**不用紧迫感修辞代替信息**。
4. **分级瘦身**：仅"当天需行动"类（简易注销公示、清算备案等公示期极短的）即时通知；其余降为**每周汇总一封**。
5. **虚报零容忍**——告警机制自身故障（如心跳通道未接通）造成的误报，**在修好前先静音**：宁可暂时失去一个兜底，也不让用户反复收到假警报。
6. **止损速度是开关设计的第一指标**——凡"出事要立刻关"的通知开关，其**关闭路径必须比开启路径更简单、更快、更不依赖上下文**（本项目实现：文件式开关，`rm` 一条命令即刻关闭，无需改配置/等周期/重载）；且**执行者有权在首次虚报时直接关闭，不必等审批**。
7. **不发测试信**——通知链路的验收应等**真实条件自然触发**时取证，而非人为发一封测试信：测试信绕过了触发条件、证明不了真实链路，且对用户仍是一次打扰。

**通知文案范例**（监测暂停，2026-08-23 验收通过）：
> 您关注的事项监测暂停——系统检测到公司事项的自动监测已暂停一段时间，通常是运行设备未开机或网络中断。请检查设备是否开机联网；恢复后监测会自动继续，无需其他操作。

范例好在四点：讲清**发生了什么**、给出**可能原因**、说明**你要做什么**、并以"**恢复后自动继续、无需其他操作**"收尾——最后一句消解了"我是不是还得做点别的"这层焦虑。全文无"紧急/警告/失明"等刺激词。
