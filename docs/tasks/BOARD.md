...
- **窄的一条留着（哨兵）**：状态字段记的是**最后一次写入那一刻**的答案，不是停下来那一刻的答案，**两者的分歧随停更时长单调增大——这个字段最不可信的时候，恰好是它最要紧的时候。** 危险方向（写下闲态词→被派活→干活中死掉→按 4h 宽档处理）**今天无实例，不假装有。**
...
| 2 品牌+首页 | **拆两半**：2a 全站改名（**已派**，不依赖 logo）；2b logo 落位 + 首页首屏（等用户挑字母） | 前端页面 |
| 3 内容区重制 | 排队 | — |
...
**两条从这次验证里出来的通则（已验，进）**：
- **任何推导表都该内置至少一行答案已知的对照行。没有已知行的表，无法自证失效。** 哨兵做名字→session 映射时，第一版方法在它唯一能验的那行（自己的 id）上答错了，整张表作废——**要是没有那行，它会以"映射基本不可用"的样子被交出去。**
...
**"pushed" 是过程信号，不是结果信号（08-27，manager 自踩、哨兵点破）。**
一次 Python 文档编辑因引号语法错失败，`git commit` 无改动，`git push` 照样打出 `pushed <sha>`——**推的是已经合并的东西**。看到 pushed 就以为写进去了。
**别问"推送成功了吗"，问"这行字现在在文件里吗"。** 普查方法：对每条报过"已推"的条目 grep 关键词看它在不在当前文件里——**验的是结果不是过程**。08-27 普查 23 条：23/23 在（其中 1 条第一次报"缺"是我 grep 的关键词写错，**检查工具的 key 错了也会产生一条看起来像发现的"缺失"**）。
