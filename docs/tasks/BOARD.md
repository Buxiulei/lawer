...
**🚨 生产模型余额告警（并入 A4 处置）**：评测实测 DeepSeek 402 耗尽/OpenAI 无额度/DashScope 有余额——entry 档主力挂、靠降级链 qwen 撑。已发紧急邮件（messageId d0f0d4b9）+通报佰亿助理核实账户共享。根治=中转上线（有余额）或充 DeepSeek。**这就是 relay 紧迫性的实证**。
