...
**① 某些状态会剥夺当事人报告该状态的能力。**
「撞限暂停」是白名单里**唯一一个当事人无法自行写入**的状态——等它成立那一刻，会话已经拿不到回合了。探针永远只看到撞限**前**那个词。同一原因、同一时段、同一台机器：
```
manager  撞限时字段停在「在岗」(工作态) → 45min 严档 → 13:25 告警
哨兵     撞限时字段停在「待命」(白名单) → 4h  宽档 → 118min 未越线,零告警
```
**差别只在撞限那一秒字段里恰好是哪个词。** 弹窗夺走回合、撞限夺走配额，结果一样：**自报型判据在这里必然沉默，外部观察是唯一信息源。**
**推论**：白名单里放一个当事人写不出来的词，那个词永远不会被实时用到——只能预写或补写。
...
| 1 手机工作台 | **进行中**（08-27，6–8h）。规格 `docs/design-notes/2026-08-27-批1-手机工作台重制.md`。中途拍了 **D17**：低调模式下热线块豁免打码 | 前端页面 |
