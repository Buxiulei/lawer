...
- **2026-09-07 · 用户服务协议 v0.1→v0.2（两位 Fable 评审 41 条）与经理裁决**：评审查出八处「协议说的与产品做的不一致」：境外模型是中配/高配默认路径且脱敏不含姓名（个保法 §39 单独同意缺位）、劳动分享页不脱敏、NBDpsy 实名互认自动采用无同意、敏感信息无单独同意界面、无案件删除/注销/整案导出、转介无撤回、AI 标识出口漏两处、心理咨询领域角色写反。**裁决**：① 协议按产品实情如实写，不写不存在的能力；② 法律上必须的能力列为产品同步清单 12 项（附一），协议在其上线后生效；③ 境外模型**默认关闭**，备案前路由全部境内（待主理人确认）；④ 不设责任上限；送达按到达；管辖被告住所地加粗提示；⑤ 文书归用户自由处置；协议变更时未用套餐按比例退。v0.2 已推 ws/terms-draft 开 PR。派 P5 合规工作流：C1 同意闸（注册勾选+成年自述+实名采用同意+情绪记录同意+境外模型开关默认关+评测授权）、C2 数据生命周期（案件删除、整案导出、账号注销、撤回同意、30 日删除任务、转介删除请求）、C3 入口与页面（帮助与投诉页、/terms 协议页、/terms/overseas、/terms/processors、ai-labeling 措辞、劳动分享页未脱敏提示）。
