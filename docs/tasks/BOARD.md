...
- **2026-09-05 11:30 · 主理人口径：土八鼠与 NBDpsy 心理咨询工作室是一家公司、服务体系一环**（法律纠纷中的情绪由 NBDpsy 承接；心理来访的法律诉求由土八鼠协助）。处置：①危机单执行者改口吻——新句与 REFERRAL_TAIL 都用「我们/一家公司」自家指称，删「这个平台属于 NBDpsy 体系」的外人口吻，判据加一条；②设计稿 §13 加「服务体系互接」：P3 `referral_create`（用户同意后生成转介单，只带授权摘要）、内部 `POST /handoff/inbound`（NBDpsy 侧把有法律诉求的来访交给土八鼠：合并账号、建案、预填首诊、按诉求定 domain）、`users.linked_nbdpsy_id`、文案纪律禁「平台属于/引流/合作方」。
- **2026-09-05 11:45 · 主理人战略目标入账**：土八鼠终局 = 中国民事纠纷法律援助 agent 助手；路径 = 细分专精：①劳动纠纷（现）→ ②心理咨询纠纷（**服务对象是咨询师**，保护咨询师）→ ③婚姻恋情家庭纠纷 → 泛民事。设计稿 §13 加三条硬要求：角色不写死（DomainPack.parties：self/counterparts/multiParty；company_* 在非企业对方降级为主体查询）、同一账号可跨土八鼠（案件按 domain 分、工具面相同）、知识库按领域独立成包共用机制。P1 R1 的 DomainPack 骨架先不改（已在跑），parties 字段在 P3 领域包完善时补。
- **2026-09-05 12:40 · NBDpsy 盘点回来 → 设计稿 §14**：对方用户以微信为主、identities 为去重真源、实名三条链路（人级终态 users.auth_status=approved；姓名证件 AES 加密密钥只在其 Rust 服务）、来访落 leads、内部接口惯例 /api/internal + HMAC、库无只读账号。**四条决定**：不直连库不共享密钥（经对方新增两条内部接口，跨仓由 NBDpsy 工程实现，P3 开工时派单）；匹配键=已验证手机号（linked_nbdpsy_customer_code 记关联不合并）；实名互认=人级 approved（写 realname_verifications provider=nbdpsy，证件号只存掩码）；转介数据包=身份+情绪摘要（≤200 字、无案情）+需求+阶段一句话+同意时间，落对方 leads（对方加两列），本侧 referrals 记状态，转介前逐项知情同意。盘点三条待确认已由经理裁：存证=实名复用；leads 加字段归对方；人级 approved 足够、24h 事件表不作门槛。
- **2026-09-06 · 客户端接入调研回来 → 设计稿 §15**：**ChatGPT 网页与 Claude 网页的远程 MCP 只认 OAuth 不认 Bearer**（主理人 09-05 的 GPT 实测走的是 Custom GPT Actions 的 REST + API key）；Gemini 网页 MCP 限美国，Gemini CLI 支持 Bearer；DeepSeek 网页版与豆包网页版零工具入口、无常驻系统提示位，豆包生态走扣子空间；Kimi/Cursor/Cline 支持远程 MCP。**四条路径**：A MCP+OAuth 2.1（新增授权服务器、PKCE、动态注册、token 映射 api_keys）、B MCP+Bearer（已有）、C REST+API key（新增注册表生成 /api/openapi.json 供 Actions 导入）、D 无工具模式（陪跑开场白按客户端档位裁剪 + AI 回复末尾 tubashu 结构块 + 网页粘贴回填解析确认写入 + 回填过 crisis_check）。P3b 与 P3 并行开工。
