...
| 2 品牌+首页 | **2a ✅ #66（土拨鼠劳动仲裁）→ 2a-2 ✅ #68（土八鼠，构建产物三旧名全 0 + 新名 47 反向自证）**。**2b 进行中**：tubashu-hero.svg 已定稿（c6/759KB/gzip 271KB），落位+favicon 简化版派前端 | 前端页面 |
...
| **土八鼠 S08×2 重跑** | **就绪待触发**（读数器已入库 `read-rename.py`，`SELF_NAMES` 三名同数只增不删——只数新名会漏掉旧名残留，那是改名最可能的失败形态） | 等 ws/backend 合入新名，后台技术直接 ping 评测官 |
