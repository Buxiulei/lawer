...
| **2026-08-30 09:30 后当日内** | 受控首发验收：prod job_runs 应现「期限提醒」新行且 items_ok≥1；内部例外信箱见实物信与模板逐字一致；随后测试期限置 resolved、演示账户邮箱恢复原值、当场留痕。**无新行=cron 没跑；有新行 items=0=staging 没落或不在档** | 验收通过前，「提醒已经在保护你了」这句话禁止对用户说（后台技术留下的原话条件） | manager 派子代理核对 |
