...
- **2026-09-05 00:10 · 实名闸 9a8ff78 合入 + 滚版（实名闸 + case_list）**：fragment 修法核过（已实名顶层即 grid，未实名 grid 加条件 mt-2，判据「已实名：顶层元素就是 grid grid-cols-3」+ 变异臂红）；wt-int 合入 543e626，tsc 0。本次滚版内容：证据上传实名闸（主理人裁决）、MCP case_list 与「先认领案件」话术、台账。看门狗与 intake_submit 下一班。**主理人问首诊入口**：按钮只在案件为空时渲染，其案件已有 26 条时间线故无入口；直接 /intake 可用（写进名下最新案件）；已提醒「经过」栏别重抄已有时间线；backlog：基本盘为空时驾驶舱常驻「补齐基本盘」入口。
- **2026-09-05 00:30 · 主理人指令：MCP 工具面重设计（架构级）**：原话「mcp 工具需要你进行重新的设计，需要思考到 agent 在于客户进行法律咨询的交互中，所有需要用到的东西。在网页上沟通时，所有需要用到的东西，都需要具备……先做设计，写 pr，尽可能的不重不漏，再做 mcp、api、后台系统、表格、配置、skill 的完善」。**做法**：① Sonnet 全量盘点九块（站内 agent 工具 / MCP / REST / 表 / 知识库 / sidecar / 网页功能面 / skill / 既有约束）落 rd-mcp-design/inventory.md；② 经理据此写设计稿 docs/design/mcp-agent-surface.md：按用户旅程（风闻裁员→约谈→协商→被迫解除/被裁→仲裁→执行/应诉）与诉求面（档案/事实/证据/法律依据/计算/文书/期限/行动/公司情报/情绪危机/计费与身份）列工具目录，每个工具：名、用途、入参出参、scope、幂等键、前置（实名/余额/阶段）、计费、对应 REST/表；再列表结构变更、配置、后台、skill 三份改法与分期计划；③ 开分支提 PR 供主理人审；④ 批后拆票按分级派单。已在飞的 intake_submit/写入幂等、个案报告设计并入该设计稿而不重复做。
- **2026-09-05 00:45 · 看门狗 stop 裁决落实 29ba6bc + 真机复核派出**：stop = abort + 半截本地落定（stopped 标记，小字「已停止接收；服务端会答完这一轮，刷新可见完整回答」）+ 解锁；liveTurn ref 由 run 收尾与 stop 各查一次谁先落定谁置空，绝不落两条；空半截只解锁不落空条；三判据（半截保留无错误卡 / 上游跑完不落第二条 / 未开口就停不落空条）+ 变异臂红；439 过、tsc 0。执行者自报「phase 离开 streaming」在 node 台架不可观测，靠 reset dispatch 保证——真机复核补这一条。复核官（L 级）：standalone + 假上游（慢/永挂两模式）+ **冻结代理**（meta 帧后停止向浏览器转发、服务端照常）复现静默死流，五臂 A–E：45s 自动补答案、visibilitychange 立即对账、永挂两次不到 STALLED 带 retry_of、健康流停止保半截且刷新只一条回答、对照回归；变异四臂亲跑。ec33ad8（实名闸 + case_list）CI 绿，生产构建中。
- **2026-09-05 01:05 · 滚版 ec33ad8 上产并真机核对**：CI 33879359337 绿 → 备份 → ff 1b38087→ec33ad8 → BUILD OK → 双服务重启，/ 200、sidecar 200、journal 无 error。核对（用网页会话路径给 uid=1 现签一把 key，用完即吊销，吊销后 401）：tools/list 十个含 case_list；case_list 回 smoke 案件一条；SKILL.md 线上含 case_list、旧「问用户 case_id」句已无；上传闸：未实名 uid=1 POST /evidence → 403 REALNAME_REQUIRED 三段式原文，evidence 零新增。**发现**：/data/lawer/deploy/smoke-fixture.py 写的 api key 已对不上 09-03 之后的密钥格式（tools/list 401「需要有效的 api key」），smoke.env 那把作废——backlog：fixture 改走 POST /keys 正规签发路径或按新哈希格式写。
- **2026-09-05 01:40 · MCP 工具面重设计 v1 成稿，PR #96 待主理人评审**：docs/design/mcp-agent-surface.md（分支 ws/design-mcp-surface）——七原则、旅程×诉求矩阵（8×11）、约 45 工具按 11 族列规格、能力注册表单一真源生成站内/MCP/REST/manifest/skill、四横切机制（client_ref 幂等+自然键去重、报价→确认→扣费、个案报告 stale 机制、危机 crisis_check 与服务端硬拦边界）、表变更（case_reports/agent_writes/service_quotes/crisis_hits + 四表加列 + scope 加 spend）、pricing_config 七键、后台四视图、skill 生成区、六项待拍板（出证收费/OCR-ASR 单价/spend 权限/报告按案/危机强制度/来文解读本期）、盘点十缺口逐条落点、四期拆票。**两执行者撞额度**：review-chat-watchdog 停在真机 seed、fix-mcp-intake 停在代码改完未 tsc（12 文件未提交，含新 db/dedup.ts）——额度恢复后原 agent 续跑（保上下文），令 intake 先提交半成品。
- **2026-09-05 02:00 · 建档与写入去重单回报 0bc5591（ws/mcp-intake-submit）**：intake_submit（工具层元→分，assertOwned）、case_update 扩基本盘、timeline_add client_ref（先查后写 + 部分唯一索引）+ 近重复守卫（lib/cases 入口，三入口共用）、行动卡同题待办不双建（insertActionItem 回 {id,created}，站内 action_card 按 created 计数）、manifest 补两条、skill 四份同步、migrate 加列加索引（exec 48→49）；四变异臂红；1388 过、tsc 0。执行者三点说明合理：首诊批量写不走近重复守卫（原子批量）、变体文档顺手同步（同源副本要求）、双保险幂等。派 Sonnet 复核（迁移可重入、站内 action_card 行为、守卫误伤面、跨用户、manifest/skill 一致）。**上产注意**：含迁移，重启后按惯例手动触发 getDb 迁移并核 timeline_events 有 client_ref 列与索引。
- **2026-09-05 02:20 · 建档与写入去重复核 PASS（Sonnet）→ 合入 wt-int**：18 文件全在范围；4549 过、tsc 0；五点全过（迁移 addColumnIfMissing + IF NOT EXISTS 可重入、部分唯一索引排除 NULL；站内 action_card created=false 不计数不 emit；守卫误伤面有测试；跨用户 CASE_NOT_FOUND 零写入、校验失败字段级不 500；manifest/skill 逐字一致）；四臂亲跑红，其中第四臂证明库层唯一索引真实拦截。推 main 跑 CI；看门狗真机复核若及时 PASS 同班上产。
- **2026-09-05 02:35 · 滚版 d4c1976 上产并真机核对**：CI 33932127084 绿 → 备份 → ff ec33ad8→d4c1976 → BUILD OK → 双服务重启 → / 200、sidecar 200、journal 无 error；迁移由首个请求的 getDb 自动落地（timeline_events.client_ref 列 + uq_timeline_client_ref 索引均在，我的 tsx 手动触发一行因引号错误未跑成，但结果已由库核实）。MCP 用现签临时 key（用完吊销）核：tools/list 十一个含 intake_submit；timeline_add 同 client_ref 两次 ⇒ 同一 id 66、库里一行；case_update 单改 position 落库。**现在 GPT 可在对话里建档**（intake_submit）并补基本盘（case_update），写回不再双写。
- **2026-09-05 02:50 · 问它看门狗真机复核 PASS → 合入滚版**：standalone + 假上游 + 冻结代理（meta 后停转保持连接）+ Chrome 141：A 慢上游冻结 60.2s 自动补上完整回答且发送键复活、库一问一答；B 切后台回来 2.6s 触发对账、18s 补上；C 永挂两次不到 75.2s STALLED 卡、重试带 retry_of=本轮 message_id；D 健康流停止保半截、刷新只一条回答；E 对照无新文案。变异四臂红；4548 过、tsc 0；服务端零改动。**nit 1 已修**（d59d03c，经理亲改一行）：watchdog.ts 死导入 HEARTBEAT_INTERVAL_MS 把服务端 events.ts 拉进 client 包；**nit 2 留观**：可见性翻转恰遇 abort 吐出缓冲帧时 run 收尾与对账并发落半截，settleRecovered 按 id 去重兜底，真机三路未复现。复核官为夺端口杀了几个孤儿 next-server（以后只按端口杀）。
- **2026-09-05 03:10 · 滚版 52e4784 上产（问它看门狗）**：CI 33933290638 绿 → 备份 → ff d4c1976→52e4784 → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200、journal 无 error；客户端包含看门狗文案（静态包 grep 命中）。今日（09-04→05）共五次滚版：662f59a 余额闸跟进 / 1b38087 后台操作列+签章主体 / ec33ad8 实名闸+case_list / d4c1976 建档+写入去重 / 52e4784 看门狗。
- **2026-09-05 03:30 · 主理人问「发我邮件了吗」→ 补发日报邮件**：今日五批上线可感知变化八条 + PR #96 六项拍板（附建议）+ 两件顺手可做，发 163 与 gmail 两址，SMTP 回执 accepted 两址（messageId b79d3bff…@nbdpsy.com）。**坑**：发信脚本放 /tmp 跑时 nodemailer 解析失败（Node 从脚本所在目录找模块），要把脚本临时放进 app 目录再跑、跑完删。规矩：有决策要拍或一天多批上线，当日发一封汇总邮件，不等主理人问。
- **2026-09-05 10:20 · 主理人对 PR #96 六项拍板 + 两条新要求 → 设计稿 v1.1（074c5b1）**：①出证不收费 ②单价按初值 ③不细分 spend ④报告按案件 ⑤危机方案主理人未看懂——已用人话重述（服务端能做：词表工具+事实卡首行标记+禁号永不出现+对外文书必带后果；做不到：强迫外部 agent 开口），按建议执行，主理人若要「握手」硬闸回「硬」 ⑥内容提取（OCR/录音/**视频**）与**每件证据简报**并入本期（P2 紧接 P1）⑦**多领域架构**：劳动纠纷之后紧接心理咨询纠纷土八鼠——新增 §13：工具面/表/协议跨领域不变，领域包（阶段/首诊表/事实卡分节/算钱器/知识库/模板/期限种类/对方主体类型/危机词表/说明书/文案）按 `domains/<key>/` 打包，`cases.domain` 落库，注册表条目声明适用领域，共用层出现「劳动」硬编码即测试红。分期重排：P1 骨架与对齐（注册表含 domain、cases.domain、8 项写能力、knowledge 十类、生成器）→ P2 取证核验（sidecar /video、extraction_jobs、evidence_extract 三模式、证据简报、doc/transcript）→ P3 记忆与守则 → P4 第二领域包。开工 P1。
- **2026-09-05 10:40 · P1 开工（workflow wf_64690e4d-211，基线 52e4784）**：R1（Opus）能力注册表 lib/capabilities（families 分族、listCapabilities/getCapability、现有十一工具原样迁入且 tools/list 顺序不变）+ agent_writes 表与 withClientRef 助手 + cases.domain 列与 DomainPack 骨架（labor，stage 校验真消费）+ 共用层禁「劳动/仲裁」字面量守卫 → Fable 复核 → PASS 后四线并行各自 Sonnet 复核：W1 claim_calc/claims_upsert/claims_list/deadline_set/deadline_resolve/action_create（算钱器抽到 lib/cases/claims.ts 站内同函数）、W2 draft_list/get/write（对外缺后果服务端拒收、同题新版本）/company_profile_upsert/emotion_log/timeline_list/timeline_milestone、W3 知识十类唯一常量 + full_text + knowledge_get、R2 manifest 运行时生成（rest-index 49 路由、tools_version）+ gen-agent-docs 生成区与守卫 → 集成官合四支、重生成文档、三统计、standalone MCP 冒烟（每个新工具各调一次、重放 deduped、他人案件零写入）。结果落 rd-p1/。上产由经理走 CI+bundle 流程。
- **2026-09-05 11:10 · 主理人产品规则：情绪危机时同时推 NBDpsy 心理咨询与免费热线**：原话「危机干预热线的效果通常不好，只能应急，要解决心理问题，得找我们 NBDpsy 心理咨询工作室」。**这条推翻 D9/D15 的「危机轮不得出现付费入口」**（CRISIS_DIRECTIVE 末条「任何推销感」、prompt.ts nbdpsyEligible=false 硬禁、referral D15 L1 判据）。落法（M 级，ws/crisis-nbdpsy）：危机资源卡（确定性首段）= 热线（应急，号码逐字）+ 一句 NBDpsy 工作室（解决持续问题；同一体系；无价格无预约链接；「你说一声我把联系方式告诉你」）；危机卡的 NBDpsy 句**不消耗** emotion_log 的一案一次转介标记（两标记继续分开）；24h 冷却随卡；模型仍不许自行添加推销句；D15 判据改为「危机轮必含热线 + NBDpsy 句，且无价格/链接」；陪跑指南 §三 同步（MCP crisis_check 在 P3 复用同一段）。关切口径不变：先接住人、先热线、再说工作室。
- **2026-09-05 12:00 · 主理人要求：土八鼠与 NBDpsy 账号打通**：①NBDpsy 来访客户可能是土八鼠用户，账号要打通；②在 NBDpsy 做过实名的，土八鼠直接复用实名结果用存证；③我可读 NBDpsy 用户信息表（同机 PostgreSQL，代码库本地 /home/roots/NBDpsy）；④土八鼠转介用户去 NBDpsy 时并入背景：实名信息、当前内心情感状态（土八鼠据对话总结）、可能的需求等。处置：派 Sonnet 只读盘点 NBDpsy 的用户/实名/来访背景/内部 API/库访问/通知/合规约束（落 rd-mcp-design/nbdpsy-identity.md，不写任何凭据与真实数据），据此写设计稿 §14「账号与实名互认、转介数据包」：身份匹配键（验证过的手机号优先）、实名复用（NBDpsy 实名记录快照写入 realname_verifications provider=nbdpsy，不复制证件号明文）、转介数据包（实名摘要 + 情绪状态摘要 + 需求 + 同意时间，经服务间接口写入 NBDpsy 来访档，不直写对方库）、用户知情同意（转介前明确告知会传什么）。
- **2026-09-05 12:15 · 第二个土八鼠需求种子入账（只记忆，不开做）**：首个用户 = NBDpsy 机构负责人；立场 = 机构/咨询师方：机构如何应对来访、咨询师如何不犯法律错误、来访危机如何处理与干预、是否突破保密条例、是否报警。开做时调研：伦理守则与保密例外的法律边界、精神卫生法、报警/通知监护人的义务与免责、机构合规（知情同意、记录保存、投诉处理）、投诉/退费/名誉纠纷 SOP、危机干预流程与模板。已写入长期记忆。
- **2026-09-05 13:00 · 主理人重申分工「fable 审查/设计/验收/派单，opus 执行，sonnet 调研，haiku 监控」**：收回 09-04 降 token 新规里「S/M 级复核交 Sonnet」这一条——**复核一律 Fable**，省 token 靠缩范围与输出卫生。在飞的 P1 workflow 脚本已把 W/R2 复核的 model 改回默认（Fable），不打断当前运行；从下一单起全部按此。
- **2026-09-05 13:10 · 危机轮 NBDpsy 转介单回报 a93790a（ws/crisis-nbdpsy）**：新句 CRISIS_NBDPSY_LINE 随危机卡首段给（24h 随卡冷却，不碰 referred_nbdpsy），走 buildCrisisOpener 同一条路；NBDPSY_PITCH 闸与 D15 闸在危机轮只判模型段不误剥系统句；CRISIS_DIRECTIVE 末条改「那句由系统随卡给出，模型不得再自行添加」；scripts/eval/assertions.ts 的 D15 活判据同口径改为只判模型段（否则跑批每个合规危机轮都报假 L1——执行者判定属判据改写范围，经理认可）；928/928、eval 487/487、全量 4590、tsc 0；三变异臂红。**经理退回一处措辞**：「和土八鼠是同一个体系」→「一家公司」（主理人原话），REFERRAL_TAIL 同改，改后送 Fable 复核。
- **2026-09-05 13:30 · 危机轮 NBDpsy 单 6398f83（自家口吻）→ Fable 复核**：新句「……这一块也是我们自己在接：土八鼠和 NBDpsy 心理咨询工作室本就是一家公司……跟我说一声，我把怎么联系告诉你——不急，也完全由你定」；REFERRAL_TAIL 同口径删「这个平台属于 NBDpsy 体系」；判据加自家指称断言、变异臂红；1417 绿、tsc 0。spec/历史成绩单旧口径按「不重写历史」不动。复核项：窗内首次/24h、热线在前且逐字、无价格链接预约、不碰 referred_nbdpsy、出口闸只剥模型段、非危机硬禁原样、eval 判据同口径、三文档一致；四臂亲跑。
- **2026-09-05 13:50 · 危机轮 NBDpsy 单复核 PASS（Fable）→ 合入滚版**：a–g 全过（窗外首次给、热线在前逐字、无价格链接预约且自家指称、不碰 referred_nbdpsy、出口两闸只剥模型段、非危机硬禁原样、eval 与产线同一 splitCrisisOpener）；4592 过、tsc 0；四臂亲跑红。nit：执行者计数口径与复核不一致（实质全绿）；range-diff 对密集 CJK 渲染有误差，实体以 blob 为准。合入 wt-int（316c976）推 main。**自伤第 N 笔**：又一次在 wt-int/app 里写 BOARD 把链条打断——BOARD 段永远单独一条命令、以 cd 主仓开头。
- **2026-09-05 14:20 · 滚版 fa0b358 上产（危机轮同时给热线与 NBDpsy）**：CI 33942156807 绿 → 首次 ssh 被对端关闭（瞬时）重试即通 → 备份 → ff 52e4784→fa0b358 → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200、journal 无 error；线上陪跑指南含「一家公司」，服务端包含危机句。
- **2026-09-05 14:50 · P1 workflow 完成（11 agent，56 分钟）→ 经理验收合入**：R1 注册表/领域骨架 da9028c Fable 复核 PASS；W1 c115262 / W2 1db8ad5 / W3 20bcb28 / R2 ecbeae4 各 PASS；集成官合四支为 ws/p1-integration 0789075（冲突 3 文件 5 处全保留）、重生成文档、4708 过 / tsc 0 / standalone 构建过、真机冒烟 25 工具逐一可调、写工具 client_ref 重放 deduped、他人案 CASE_NOT_FOUND、manifest 与 tools/list 一致、rest 62 路由（任务书写 49 是陈旧数，以扫描真值为准）。**集成官四条观察，经理处置**：① deadline_resolve 已了结重放无 deduped 字段 → 经理亲改一行补 deduped:true（同形）；② W2 六条未进 route 守卫 case_id 必填名单 → 经理补名单（draft_get 按 draft_id 归属判，除外）；③ standalone 下 /skill 路由依赖 LAWER_SKILL_DIR → 生产 env 已设且目录存在、线上 200，无风险；④ gen-agent-docs FAMILY_LABELS 缺四族标题（守卫按设计报出）集成官已补。合入 wt-int 推 main，含迁移（agent_writes 表、cases.domain 列）。
- **2026-09-05 15:30 · 滚版 3cda07f 上产（P1 MCP 对齐）并真机核对**：CI 33942600179 绿 → 备份 → ff fa0b358→3cda07f → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200、journal 无 error。迁移由首个请求落地（agent_writes 表、cases.domain 列，现有 5 案全 labor）——**教训**：重启后紧接着查库会看到迁移未跑（getDb 惰性），核对要先打一个请求再查。真机（临时 key 用完吊销）：tools/list 25、manifest tools 25 / rest 62 / tools_version 5f9193fd；case_list 带 domain；action_create 同 client_ref 两次 ⇒ 第二次 deduped:true、agent_writes 1 行、行动卡 1 张；他人案 claims_list ⇒ CASE_NOT_FOUND；knowledge_get 回法条卡；线上接入说明含新工具名。第一次冒烟我用错 action_create 入参（items 数组，非 title/detail）得 NO_ITEMS——是我手误不是缺陷。**P1 收口。今日第七次滚版。**
- **2026-09-05 16:00 · P2 取证核验前置盘点回来 → 开工**：盘点要点：/ocr（Qwen-VL，秒级，无体积闸）与 /asr（Paraformer-v2，同步阻塞、无超时、有说话人分离与时间轴、仅单声道）已部署零调用；生产无 ffmpeg、sidecar 无音视频库（纯绿地）；文件 AES 加密只有 app 能解，沿用「app 解密→字节 HTTP 传 sidecar」（/pades 先例）；上传闸实测 25MB；三张解读表零写入路径，review-rules 只有 knowledge_get 单点消费；计费可抄 dossier-billing 三段式泛化成 service_quotes；无任务队列（唯一先例 cron 轮询状态列）；case-facts EVIDENCE_DISCLAIMER 硬编码「系统不做文本提取」接线后必改。**处置**：生产装 ffmpeg（仅新增包）；P2 拆五票：T0 基建（extraction_jobs 表 + 进程内 worker（租约/心跳/重启回收）+ service_quotes 泛化 + pricing 键 + sidecar-client 加 /ocr /asr 带超时 + evidence 加列）、T1 sidecar /video（ffmpeg 抽 16k 单声道音轨 + ≤12 关键帧）、T2 evidence_extract 三模式 + 证据简报（LLM 固定 schema）+ evidence_get/brief 工具 + 事实卡免责声明改写 + 网页证据详情、T3 来文解读（review-rules + LLM → 三张表 + 文件解读页真数据）+ 录音转写建议时间线、T4 evidence_upload_url/register + 音视频上传闸 200MB（Caddy 反代限额同改）+ evidence_attest 免费入 MCP + 出证自动简报。T0/T1 并行 → T2/T3/T4 → 集成官 → Fable 复核各票 → 经理滚版（含 Caddy reload）。
- **2026-09-05 16:20 · P2 workflow 开工（wf_f01e245c-446）+ 生产根分区提醒**：五票按 T0/T1 并行（基建 extraction_jobs+worker+service_quotes+sidecar 客户端加列 / sidecar /video）→ Fable 复核 → 合 ws/p2-base → T2/T3/T4 并行（提取三模式+简报+事实卡+网页 / 来文解读+转写建议 / 上传地址+闸+出证入 MCP）→ 各复核 → 集成官三媒介真机。生产已装 ffmpeg 6.1.1（仅新增包）。**根分区 / 92%→90%（5.8G 空）**：/home/ubuntu/NBDpsy 26G、journal 3.3G、/root 2.2G；只做了 apt clean 与 14 天 vacuum（释放 0）；已知会 NBDpsy 工程会话；lawer 数据在 /data（92G 空）不受影响，但 build 与 apt 用根分区，低于 3G 会开始失败——纳入监控项。
- **2026-09-05 17:40 · P2 workflow 首轮：T0/T1/T2 PASS，T3/T4 MUST_FIX（复核抓到真金白银的口子）**：T3 来文解读——M1 确认步不校验报价 service（5 公道值的 ocr 报价买 20 的解读）、M2 退款后同 quote_id 重试免费再解读、M3 案件不符在扣费后返回不退款、M4 详情页 404 判定用错字段永远走不到；T4 上传——T4-1 size 未报且无 Content-Length 时内存预算退化为 25MB 档（注释与代码相反）。**裁决**：transcript_submit 保持只读不计费不做 asr，但必须要求转写已存在否则 EXTRACTION_REQUIRED 指路 evidence_extract asr；writeOnce 失败纳入退款；音视频上传档暂定 100MB（MemoryMax 1280M 下 200MB×6 贴上限，边收边写盘另开单后抬 500MB），Caddy 同改；畸形 token 回 404；网页 multipart 25MB 不动记 backlog。脚本加一轮修复+复审后 resume。
- **2026-09-05 18:05 · P2 修复轮一：T4 PASS（cd… eb6d241 之后修 T4-1 与 404 与 100MB 档），T3 仍 MUST_FIX 两条**：MF-3 transcript_submit 说明写「不收费」经 gen:docs 落到接入说明触发既有 J3 计费文案守卫（凡「不收/免费」须带「在你自己的 agent 上」）——执行者只跑子集未见全量红；MF-4 submitDoc 不看 confirmService 的 deduped 位，退款后同 quote_id 重试或换 client_ref 再确认可 charged=0 白拿解读（复核官探针 R1/R2 实跑）。**裁决**：措辞改「只读不写、不走报价、不做转写」重出文档；deduped ⇒ 409 QUOTE_ALREADY_USED 指路重报价，两条负例；writeOnce try/catch 进 refundAnd；回报必须全量 vitest。脚本修复轮改为最多两轮后 resume。**教训**：任务书要求全量 vitest，执行者跑子集漏了跨模块守卫——复审必跑全量这条已在复核提示里，靠它兜住了。
- **2026-09-05 19:10 · P2 workflow 完成：T3 第二轮 PASS（afb91d8），集成 ws/p2-integration 04fb252**：61 文件 +7669；tsc 0；全量 4833 过；standalone 构建过；MCP 37 工具。三媒介真机（standalone + 独立库 + 假上游：本机无 DashScope/LLM key，/ocr /asr /video 与简报/审查/转写模型全由 fake-upstream 按契约复刻）a–h 全 PASS：图片 ocr 报价 5 不动钱→确认扣 5→worker 4s done→简报 v1；音频 asr 报价 8→转写带时间轴说话人；视频 video 报价 12→音轨三句 + 四帧；doc_submit 报价 20→findings 两条 must 取规则库；transcript_submit 零写时间线；upload_url→PUT→register→attest 免费→自动简报；QUOTE_EXPIRED / GONGDAO_EXHAUSTED / 他人 EVIDENCE_NOT_FOUND；case_facts 新免责句 + 简报摘要。**集成官两条未通过项（经理派小修）**：① 事实卡统计行「已提取 3 条其中 4 条有简报」算术不自洽（出证自动简报让未提取条目也有简报）→ 三数独立表述；② doc_submit 报价回包驼峰 vs evidence_extract 下划线 → 统一下划线。**经理追加**：视频档 T4 修复轮未按裁决收到 100MB（模板仍 200MB）→ 本次一并改 100MB（guard/upload-routes/Caddy 模板/守卫）。**部署项**：生产 Caddy /api/v1/evidence* 拆独立档 100MB（脚本已备，validate 走 sudo -u caddy，失败自动回滚）；LAWER_KNOWLEDGE_DIR 已在 app.env；sidecar venv 有 pypdf 6.17.0；DASHSCOPE key 在 sidecar env——上产后用 smoke 账号跑一次真 OCR + 真简报作为真上游验收。
- **2026-09-05 19:40 · P2 收尾三项完成（ws/p2-integration d925396）→ 经理验收合入**：①事实卡统计行三数独立（「其中」改「含」，判据零容忍词表）；②doc_submit 报价回包统一下划线 DocQuoteView；③视频档 100MB（guard/upload-routes/Caddy 模板/守卫；413 文案与工具描述同步「视频 100 MB」；token-flow 判据改字面 101MB/100MB 才有牙——执行者自报「用常量±1 的判据改回 200 也绿=没牙」，好教训）；全量 4835 过、tsc 0。sidecar video.py 的 200MB 上限保留（提取输入的高天花板，无害）。合入 wt-int 推 main；上产步骤：bundle → build → 重启 → 迁移核（extraction_jobs/service_quotes/evidence 新列）→ Caddy 脚本（validate 走 sudo -u caddy，失败回滚）→ worker 启动核 → smoke 账号真 OCR + 真简报（真上游验收）。
- **2026-09-05 20:10 · a7b19ed CI 红 → ea19434**：app 作业两条报价判据红「本机没有 ffprobe」（音视频报价按 ffprobe 量时长）——runner 缺 ffmpeg，生产已装。CI app 作业加 apt 装 ffmpeg（插在 npm ci 之后、tsc 之前，yaml.safe_load 校验 6 步），推 ea19434 重跑，bundle 重新暂存。sidecar 作业绿。
- **2026-09-05 21:00 · 滚版 ea19434 上产（P2 取证核验）并真上游验收**：CI 33949977141 绿 → 备份 → ff 3cda07f→ea19434 → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200；迁移落地（extraction_jobs、service_quotes、evidence 三新列）；manifest 37 工具；Caddy /api/v1/evidence* 独立档 100MB（validate 走 sudo -u caddy、reload ok）；sidecar /video 直调 3s 合成视频 → duration 3、帧 1、probe h264、有音轨 ✓。**真上游冒烟（smoke uid=1 临时置已实名，临时 key 用完吊销）**：evidence_upload_url → PUT 201 → evidence_register（首次我填错 category「公司文书」，允许值为 合同/工资/社保/考勤/沟通记录/公司文件/录音/其他）→ 报价 5 不动钱 → 确认扣 5 → **worker 15s 内领取**（进程内 worker 已证实在生产运行）→ sidecar /ocr 503「DashScope 拒绝访问模型 qwen-vl-ocr-2025-11-20：该 API key 未开通此模型」→ 三次 attempts 后 failed。**两条结论**：① **待主理人**：在百炼控制台给该 key 开通 qwen-vl-ocr-2025-11-20（项目锁 dated 版本按定价核定）；② **缺陷**：提取最终失败不退款（余额 4324→4319，无退款流水）→ 已派 ws/extract-refund（worker 终态 failed 按 order_ref 幂等退款 + entitlement 归还 + refunded_at）。**ASR 未验实**：2 秒合成正弦波直调 /asr 回 502「task_status=FAILED」，sidecar 日志无细节——可能是无语音内容或模型未开通，待主理人开通模型后用真录音再测。
- **2026-09-05 21:40 · 提取失败退款单 3eff72c 复核 PASS（Fable）→ 合入滚版**：同一事务（退款抛错整体回滚、作业留 running 待回收再退，钱不丢）；幂等两道（refunded_at 抢占位为纵深冗余，真兜底是 refund-<order_ref> 的 ref 幂等，臂 2 单删抢占位为绿、臂 2b 破 ref 才红）；金额/feature/ref 与扣费一致；成功零退、未确认无退款路径；券路径归还可再消费无 ledger 行；migrate 可重入；4839 过、tsc 0。**现状语义记账**：视频逐帧 OCR 失败被逐帧吞掉 ⇒ 作业成功不退；只有整体 handler 抛错才整单退——无部分退款，产品侧留意。
- **2026-09-05 22:10 · 滚版 b590ea1 上产（提取失败退款）**：CI 33951505619 绿 → 备份 → ff ea19434→b590ea1 → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200、journal 无 error；extraction_jobs.refunded_at 列落地。今日第九次滚版。smoke 账号那笔失败 OCR 的 5 公道值按旧路径未退，属测试残留不追。
- **2026-09-06 · 主理人「开工」+ 两条新要求**：①OCR 参照 NBDpsy 发票 OCR 的实现（很可能用的是这把 key 已开通的模型）——派 Sonnet 读 /home/roots/NBDpsy 找模型与请求形态，并在生产用同一把 key 逐个试 qwen-vl-ocr-2025-11-20 / qwen-vl-ocr / qwen-vl-plus / qwen-vl-max；②除 GPT 外要支持 **DeepSeek 网页版、豆包网页版、Gemini**——派 Sonnet 用 WebSearch 查证各家网页产品 2026-09 的远程 MCP/工具/文件上传支持；不支持的走「粘贴档案快照 + 粘贴回填」路线（设计后派单）。**P3 workflow 开工**（wf 见 /workflows）：R1 个案报告（case_reports、bootstrap、乐观锁、markReportStale 唯一入口、facts 首行 buildFactsStatusLine、/case/[id]/report 页）→ Fable 复核（最多两轮修复）→ 五线并行：crisis_check+crisis_hits+me_get+quote_list / citation_check（四步核验落代码）/ 公司情报六工具走既有 dossier-billing 包一层 + DomainPack.parties / share_create/revoke + draft_export（sidecar /draft-pdf）/ 转介本侧（referrals 表、HMAC 客户端、referral_create 同意与摘要过滤、实名互认读侧 provider=nbdpsy 掩码快照）→ 各复核 → 集成官真机。**跨仓派单**：已向 NBDpsy 工程会话发两条内部接口需求（identity/status、leads/referral）与反向 handoff/inbound。
- **2026-09-06 · OCR 拦路石定案（Sonnet 读 NBDpsy + 生产实测）**：DashScope key 与 NBDpsy 同一把（sha256 前 8 位同）；该 key 对 qwen-vl-ocr-2025-11-20 / qwen-vl-ocr / qwen-vl-plus / **qwen3-vl-plus-2025-12-19** 全 403，对浮动别名 **qwen3-vl-plus** 与 qwen-vl-max（2026-10-10 下线）200——百炼开通粒度是精确字符串，开通裸别名 ≠ 开通它指向的 dated 快照。NBDpsy 发票 OCR 走 OpenAI 兼容 chat/completions + base64 data URI + prompt 约束，无 ocr_options。成本：qwen3-vl-plus ≈ 1.15 公道值/页，ocr.per_page=5 不必调价（毛利 ~77%）。**派单**：sidecar/ocr.py 切 qwen3-vl-plus 同形态、去 ocr_options、契约不变、判据含「默认模型名」TODO 判据；**待主理人**：百炼开通 qwen3-vl-plus-2025-12-19 后改回 dated 锁版。
- **2026-09-06 · OCR 切 qwen3-vl-plus 单 59d13aa 经理复核通过 → 合入滚版**：仅 sidecar/ 改动；OpenAI 兼容端点 + base64 data URI + 无 ocr_options + prompt 约束；默认 env OCR_MODEL 或 qwen3-vl-plus；403/429 不重试自述、5xx/超时重试两次；OCR_MAX_TOKENS=4096 常量（裁：够用不加 env）；契约 {text,model,request_id} 不变；pytest 81 过；三变异臂红。浮动别名的「指向变更悄悄抬价」风险为过渡代价，主理人开通 dated 名后改回。
- **2026-09-06 · OCR 上产真上游 PASS + 退款路径首次真实生效 + REST 九缺口开工**：7f9f199 上产后首跑仍 403——sidecar.env 的 OCR_MODEL=qwen-vl-ocr-2025-11-20 覆盖了新默认（env 备份后改 qwen3-vl-plus，只重启 sidecar）；改后真图 5s 提取正确「解除劳动合同通知书（冒烟测试）…经济补偿按N+1计算…」，扣 5。**退款首证**：前一次失败作业余额 4314→4319、refund 行 1、refunded_at 有值。**新观察**：提取成功后简报未生成（brief_version 0）、无日志——brief.ts 静默 return null，纳入 B 票。**援助律师 09-06 实测九条 REST 缺口**（主理人误发我的那条其实就是此事）：①PATCH 不收基本盘 ②18 工具无 REST ③realname/status 对 key 拒 ④无证据作废 ⑤时间回显裸 UTC ⑥multipart 上传未入说明 ⑦列表无分页 ⑧claim_calc 逐个报缺参 ⑨缺《劳动合同法》35 条与解释一 43 条法条卡。**裁法**：A 票注册表驱动通用桥 POST /api/v1/tools/{name}（同前置闸同 handler）+ PATCH 走 case_update + 实名状态只读 + 分页；B 票 evidence_void + 时间 +08:00 序列化同源 + claim_calc 一次列全 + 简报失败可见与 regenerate；C 票法条卡官方原文核实。workflow rest-parity 已起，Fable 复核。已回复援助律师。
- **2026-09-06 · P3b（客户端接入）workflow 完成：ws/p3b-integration 3111f61（三票 oauth 2552d51 / openapi 1e75d77 / paste 04fa4ed 各 Fable PASS）**：4975 过、tsc 0、standalone 76 路由。真机：OAuth 全流程（两份 well-known、动态注册、同意页、code→token、refresh 旋转、复用即整链吊销 + 401 挑战头、设置页显示「来自 X 的授权」并可吊销）；OpenAPI 3.1 校验 0 问题、?profile=actions 9 操作；矩阵十档片段可复制且地址全为公网基址；opener 三档裁剪保「先读这四条」与「回填约定」；paste-back 预览零写库、confirm 写 4、同批重放 deduped 4、危机词触发。集成官顺手把 openapi 支「OAuth 未上线」四档文案翻真并改双向判据。**两缺陷派修**：F1 跨批重贴时间线双写（预览按自然键、写入只认 client_ref）→ 裁 addTimelineEvent 入口 client_ref 未命中也过自然键并登记 ref；F2 整链吊销未同步 api_keys.enabled → 裁吊销函数同步置 0。**部署项**：LAWER_PUBLIC_URL 成 OAuth issuer 硬依赖（滚更前核）；next.config 三条 /.well-known rewrite，需核 Caddy 不截 /.well-known/oauth-*；新表 oauth_clients/codes/tokens + api_keys 两列。
- **2026-09-06 · P3b 两缺陷修毕（ws/p3b-integration a192bd3）**：F1 addTimelineEvent 入口 client_ref 未命中也过自然键（跨批重贴 written=0/deduped=1，真不同事件仍两条）；F2 revokeChain 连带 disableApiKey（code/refresh 复用 ⇒ api_keys.enabled=0、设置页标已失效；正常续期不动）。4981 过、tsc 0、两臂红。偏离采纳：timeline 不落 agent_writes（既有刻意设计，自然键兜住重放）。等 P3 主线与 REST 补齐线到齐后三线合并、一班上产。
- **2026-09-06 · NBDpsy 技术侧（baiyi）契约 v1.2 到**（/home/roots/NBDpsy/docs/secretary/state/T259-契约v1-20260906.md）：POST /api/internal/identity/v1/status（body phone+nonce；found/verified/customer_code/real_name/id_type/id_number 全号/verified_at）、POST /api/internal/leads/v1/referral（201 lead_id / 200 duplicate；幂等 sha256(source_case_hash:consent_at)）；鉴权头 X-Source: tubashu / X-Timestamp(±300s) / X-Signature: sha256=hex(HMAC(secret, ts+\n+raw_body))；仅 127.0.0.1:8083；限流 60/60s；密钥在生产 /root/t259-tubashu.secret（600）。**与 P3 R6 工单口径差异**：签名串（我方写的是只签 body）、路径带 v1、nonce、错误体形态——P3 合并后加一张适配票对齐（客户端 lib/nbdpsy/client.ts）。**答复四问**：手机号我侧恒 11 位归一（非大陆号注册不进）；护照接受、我侧只存掩码；X-Source tubashu 同机；证件类型规则接受。联调 20:00–22:00，请对方造测试客户 17700000001（已实名护照）/ 17700000002（未实名）；上产时把密钥读入 app.env（NBDPSY_INTERNAL_BASE/SECRET），冒烟账号手机号设 17700000001。
- **2026-09-06 · REST 补齐 workflow 完成：ws/rp-integration c723dc2（bridge aee2551 / void 89cdecc / statute 2f86da5 各 Fable PASS）**：5030 过、tsc 0、standalone 过；真机 81 PASS：通用桥 39 能力全可达（读 200、写 client_ref 重放 deduped、只读 key 写 403、他人 404）；PATCH 基本盘；realname/status 三态只回状态；分页 total/next_offset 封顶；evidence_void 各视图（过滤先于分页、出证拒、事实卡不含）；+08 跨日；claim_calc 一次列全缺参且 MCP/桥逐字同；简报失败 brief_status/brief_error + regenerate；知识库 218→220 命中第 35 条与解释一 43 条。集成官修四处接缝（toolErrorResult 签名、三条新路由与 paging 壳走 apiJson 保时区、invokeCapability 透传 extra）。**两记项待随后小修**：claim_calc 日期格式错不进 invalid 且不一次列全；REST 专用 GET evidence 不含派生 brief_status。**部署项**：LAWER_KNOWLEDGE_DIR 已在生产 app.env。待 P3 主线到齐后三线（p3 / p3b a192bd3 / rp c723dc2）合并、一班上产。
- **2026-09-06 · P3 主线 workflow 完成：ws/p3-integration 31310b9（R1 report aa8cdd3 / crisis 9ad3e67 / cite 1aa2f7a / company 941f7a3 / share 19320fb / referral b30fc8c 各 Fable PASS）**：54 工具；5007 过、tsc 0、sidecar 78 过、standalone 过；真机 a–g：报告初稿→按节改→写入即 stale 且事实卡首行提示；crisis_check 首段三热线 + NBDpsy 句、crisis_hits 落行、首行「近 72 小时危机信号」；引用核验三种法名归一、错条号 found:false；主体情报报价不扣、确认幂等；分享未实名拒、到期 410、导出 PDF 中文可读；转介 consent 缺拒、摘要去公司名、假对方验签通过、实名互认只落掩码；tools/list=manifest=接入说明 54。**集成官两条高中项 + 经理裁决**：①危机计数两处真源打架（MCP 写 crisis_hits、转介包数 timeline 标记 ⇒ 来自外部 agent 的危机紧迫度恒 0）→ 裁 crisis_hits 唯一真源，站内也写；②导出 PDF 与分享页带出【发出前必读】尾注 → 裁渲染前剥（唯一函数），写库不动；③migrate 计数写死判据合并即失效 → 改推算；④NBDpsy 客户端按契约 v1.3 适配（X-Source/X-Timestamp/签名串 ts+body/nonce/v1 路径/id_number_masked）。四项一票派出（ws/p3-integration 上），随后三线合并。
- **2026-09-06 · P3 四项收尾落地（ws/p3-integration：5ca2b06 危机真源 crisis_hits / 0752b4a 导出分享剥尾注 / ba28525 迁移计数改推算 / df20d17 NBDpsy 客户端 v1.3）**：5018 过、tsc 0、各臂红。裁：撤 exec 计数注释与守卫采纳；转介请求体须按契约 §2 扁平 shape（对方 deny_unknown_fields），补 packet→v1.3 映射一提交后进三线合并。
- **2026-09-06 · NBDpsy T259 就绪（14:16 上其生产；测试号 17700000001 已实名护照 / 17700000002 未实名；nonce 一次性重放 409；首发转介会给对方管理员发短信，知情）+ 三线合并开工**：P3 转介体拍平 v1.3 白名单（76b10aa，deny_unknown_fields 假对方通过、多发一字段 400 红）。派集成官合 ws/p3-integration 76b10aa + ws/p3b-integration a192bd3 + ws/rp-integration c723dc2 → ws/release-0906：注册表按 p3→p3b→rp 追加、mcp 路由以 rp 单一前置闸叠 p3b OAuth 解析、rp 的 apiJson 出口覆盖新路由、迁移计数推算、生成物重出、OpenAPI/manifest 快照重生成；三统计 + 八臂冒烟。上产后跑 prep-t259.sh（密钥入 app.env、冒烟账号手机号设 17700000001）即可联调，不必等 20:00。
- **2026-09-06 · 三线合并完成 ws/release-0906 96515b1（p3 a1e62fe → p3b 0508a8b → rp 94df95c → 判据补齐）**：冲突逐文件按规则落（注册表并集 56 条、mcp 路由 rp 单闸 + p3b OAuth 路径、apiJson +08 出口覆盖 9 条新路由、迁移三支新表全留计数推算、生成物三样重出）；tsc 0、5380 过、sidecar 86 过、standalone 过；八臂冒烟全 PASS（tools/list=manifest=56、桥五能力带 +08、OAuth 全流程、跨批重贴零双写、报告 stale 与危机标记同现、转介白名单字段集 201、导出无尾注、作废不进事实卡）。**部署项**：新 env NBDPSY_INTERNAL_BASE/SECRET；新表 7；sidecar venv pypdf（build.sh 装 requirements 已含）。**裁决一处口径分叉**：MCP/桥前置闸只认本地实名、REST 证据路由认 NBDpsy 互认 → 改前置闸为 async 调同一 requireRealname（含互认），派小票后上产。
- **2026-09-06 · 前置闸互认统一 b38b167 → release-0906 合入滚版**：checkPreconditions 改 async 调 realnameVerifiedOrLinked（与 requireRealname 同入口）；MCP 与桥各 4 判据（互认放行落掩码快照、found=false 403、不可达 403 不崩、本地已实名不查对方）；两臂红；5388 过、tsc 0。合入 wt-int 推 main，CI 绿后上产 + prep-t259（密钥入 env、冒烟账号手机号 17700000001）+ 联调。
- **2026-09-06 · 滚版 0fb6f55 上产（release-0906：P3 + P3b + REST 补齐 + 互认闸）并联调**：CI 34019233211 绿 → 备份 → ff 7f9f199→0fb6f55 → BUILD OK → 双服务重启 → / 200、sidecar 200、公网 200；七张新表落地；manifest 56 工具；/.well-known/oauth-authorization-server 200；/api/openapi.json 200；app.env 注入 NBDPSY_INTERNAL_BASE/SECRET（值从 /root/t259-tubashu.secret 读，未经聊天）；冒烟账号手机号设 17700000001、auth_status 置回未认证。**联调**：① 实名互认 PASS——未实名账号调 evidence_upload_url → 经 NBDpsy identity/v1/status 回 verified → 放行、realname_verifications provider=nbdpsy 掩码快照、users.auth_status=已实名、linked=C20260906003；② 转介 referral_create → 本侧 pending → worker 两次 POST 均 400 invalid_body（契约 §2 未给 urgency/realname_status 类型；本侧发整数与中文枚举）→ 已请 baiyi 按审计表指出字段。09-06 第 2 次滚版。
- **2026-09-06 · 0fb6f55 新面生产冒烟全过**：case_report_get 首次生成初稿（基本盘/阶段/辖区/起算日/月薪基数齐）；crisis_check 首段三热线 + 自家 NBDpsy 句；citation_check《劳动合同法》第三十五条命中新法条卡逐字原文；通用桥 POST /api/v1/tools/case_facts 200；opener?tier=short 200（11.4KB，含回填约定）。journal 无 error。
- **2026-09-06 · 转介 400 定案 = urgency 类型（对方 JSON string，我方发整数）→ 契约 v1.4（十标量键全 string、needs 数组、恰 11 键）→ 改一行 + 判据**：db87396 改映射与新增类型判据；**自伤一笔**：我的一条命令链没按 vitest 退出码闸门就 commit+push，把一条红判据（旧断言 toBe(2)）推上了 main（fc9a9c7，CI 必红）——立即修判据再推覆盖。规矩：任何链式命令里 push 之前必须 `[ $VRC -eq 0 ]` 闸门，不能只看 tsc。
- **2026-09-06 · 滚版 7e8157a 上产（urgency 字符串）→ T259 联调全过**：CI 34019714947 绿 → 备份 → ff 0fb6f55→7e8157a → BUILD OK → 重启；referral_create 二发 → worker → NBDpsy leads/v1/referral **201 lead_id=79**，本侧 referrals #2 sent（#1 旧记录按不可重试 failed attempts=5）；加上 identity/v1/status 互认放行，两条内部接口在生产上双向验证完成。已回 baiyi 结果码；测试客户由对方一周内清理；反向 handoff/inbound 下期。09-06 第 3 次滚版。
- **2026-09-06 · 日报已发（三批上线、八处变化、三件等主理人）；REST 两记项票撞额度在全量测试前，额度恢复后续跑；P4 心理咨询纠纷领域前置调研派出（Sonnet，官方源优先：精神卫生法/民法典/消保法/个保法/中国心理学会伦理守则，保密例外与报警义务、危机干预流程、纠纷类型 SOP、文书模板、阶段与角色、危机词表与热线、与劳动包共用差异表、风险项）**。NBDpsy 确认 T259 收口（leads 79、审计 201、短信已发；测试数据 9/13 清理）。
- **2026-09-06 · REST 两记项票 3b831f9 经理复核通过 → 合入滚版**：claim_calc 日期格式错进 invalid 一次列全（借 calc.parseDate 同一把尺，MCP/桥同回包）；REST GET evidence 补派生 brief_status（与 MCP 同一符号 briefStatusOf）；5392 过、tsc 0；两臂红。
- **2026-09-06 · P4 心理咨询纠纷领域调研回来 → 设计稿 §16**：法律地图（精神卫生法 23/28/30/31、个保法 28–31、民法典 1032/1033/933/188、消保法 26/55、伦理守则第二版 3.2/1.8/1.10/8.2/8.3）；**最高风险**：强制报告制度官方文件未明列心理咨询机构（决定「该不该报警」口径，立项前须律师书面确认）；咨询合同定性影响退费公式；伦理守则官方 PDF 无法机器抽取（入卡前人工核对）；民法典四条为二手转载交叉印证（入卡前官方原文核对）；热线只认 12356（国卫医政函〔2024〕259 号），希望 24 热线非官方。§16 定：parties（self 咨询师/机构，counterparts 来访/监护人/平台/协会/监管/媒体）、非线性 stages + 并行轨「危机事件处置」、首诊 schema（来访化名/编号，最小化）、分节、算钱器（退费/违约金/三倍赔偿测算）、期限（3 年时效等）、九类文书、咨询师侧危机词表与处置骨架首段、敏感级脱敏、知识包清单、四条待律师核固定条目、P4 四票 + 灰度开关。
- **2026-09-06 · 滚版 4098805 上产（REST 两记项）**：CI 34031828194 绿 → 备份 → ff 7e8157a→4098805 → BUILD OK → 重启 → / 200、sidecar 200、公网 200、journal 无 error。09-06 第 4 次滚版。**自伤一笔**：推送链里 `git push | tail` 掩盖了 TLS 抖动的失败退出码、zsh 无 PIPESTATUS（是 pipestatus）、`&` 后的 echo 永远打印——三处一起让「pushed/watch re-armed」成了假话；已核 origin/main 实际推上并用 Monitor 直接盯 CI。规矩进 memory。
- **2026-09-06 · P4 派单（wf_07eaba01-598）**：四票四独立 worktree（wt-p4-w1..w4，基线 4098805）。W1 多领域管线（DomainPack 全字段 + 按 domain 取包 + 知识索引 domain 字段 + LAWER_DOMAINS_ENABLED 灰度 + labor 零变化快照守卫）∥ W2 counseling 知识包内容（官方原文 curl 逐字核实才标「原文核实」；伦理守则一律待核实；四条待律师复核各成风险卡）→ W3 counseling 领域包代码（敏感级脱敏三处、转介包无来访者信息、退费三口径判据）∥ W4 网页入口（flag 关时 HTML 逐字节不变）→ 集成员双领域旅程冒烟（labor 基线零差异 + counseling 全程）。Opus 执行 / Fable 对抗复审（≤2 修补轮）/ 不 push。
- **2026-09-06 · 生产复查「OCR 后简报不产」→ 病因坐实并修（edf07f0）**：重跑 p2 真上游冒烟：OCR 5s 完成，brief_version=0，brief_error=「调用简报模型失败（上游/网络）：This operation was aborted」——可见性修法（0fb6f55）生效，原因是 brief.ts 调 chatJSON **没传 timeoutMs**，落到 openai-compat 的 8s 缺省（那是给对话内分类小调用定的）；简报是后台六字段 JSON、入门档慢模型，8s 必超时 ⇒ 生产上从来没写出过一份简报。修：BriefLlm.chatJSON 增 opts、显式 60s；判据捕获 opts，删传参即红（变异已验）。tsc 0 / 该文件 10 绿。已推 main，CI 盯中，bundle 已上传。**教训**：「失败可见」只是第一层，可见之后要当天读那条 error 原文——8s 缺省这类静默配置错，日志不点名就永远像网络抖动。
- **2026-09-06 · 滚版 edf07f0 上产（简报 60s 超时）+ 首份生产简报**：CI 34032621362 绿 → 备份 → ff 4098805→edf07f0 → BUILD OK → 重启 → /、sidecar、公网 200。真上游冒烟：OCR 5s 完成，简报在 25–70s 之间落库（brief_version=1、brief_error 清空、六字段齐）——生产**第一份**简报；实测简报耗时远超旧 8s 缺省。09-06 第 5 次滚版。**另一处自伤**：lawer-app 的 StandardOutput=append 到 /data/lawer/logs/app.log，**journalctl 里从来没有应用日志**——此前多次「journal 无 error」的收尾读数读的是空集，等于没看。以后收尾看 app.log/sidecar.log。
- **2026-09-06 · app.log 复盘两条**：① 22 条「Failed to find Server Action "x"/"y"/"0"」全是外网扫描器对 `/` 发的伪造 Next-Action 头（3.66.27.41、45.148.10.119、206.189.19.19 等多源，ID 非哈希），Next 按设计拒绝，不影响用户；backlog：Caddy 层对非 40 位十六进制 Next-Action 直接 4xx（可选）。② 唯一一条「[extraction] 任务 3 的摘要卡没写成…aborted」在第 367 行、最后一次 Ready 在第 387 行——是修前旧账，修后那次成功不打 warn。**app.log 没有时间戳**：按时间过滤是假过滤；滚版前记 `wc -l` 当游标，收尾只看游标之后的行。
- **2026-09-06 · 新任务：法律智能体通用设计（主理人）**：「用 fable 设计如何构建一个好的法律智能体，从设计选型起；思想应通用，且要包含现有劳动纠纷部分怎么优化」。首版请求（文书网判决搜集+法律汇总+知识文档）被主理人中途改为本任务。派单 wf_d9fed869-76b：Sonnet 三路调研（外部现状与幻觉证据 / 中国法律数据源与合法获取 / 土八鼠现状架构与 12+ 条教训）→ 五路 Fable 独立方案（接地、法律方法、用户旅程、安全评测、平台多领域；同答 Q1–Q10）→ 三位 Fable 评审（严谨性/可落地/用户与合规）→ 主会话综合成 docs/design/legal-agent-architecture.md（分支 ws/design-legal-agent）。
- **2026-09-06 · 法律智能体通用设计稿出稿（PR：ws/design-legal-agent）**：wf_d9fed869-76b 11 agent 全回（1.6M tokens）。评审排名：接地 81/80/70、法律方法 79/79/64、安全评测 76/71/74、旅程 72/56/65、平台 67/68/64。综合裁定：接地方案为引用链骨架（法源登记簿封顶——实测 103 张判例卡 68 张首源 wikisource/sohu、57 张却标「原文核实」；StatuteGuard；answer_verify 回贴核验），法律方法方案为推理骨架（procedureNodes 期限程序化、来源四档、要件三态类型、facts_token）；嫁接安全评测的运行时边界闸/对方写入过闸/judge 家族守卫/危机 72h 不催办、旅程的首卡 ≤24h 与服务端催办、平台的跨域污染与 outOfScope。补裁八条空白（律师法 §13 等三项原文核实责任人、案卷包定义、法条版本输出侧、数据生命周期、NBDpsy 推介判据、对手方冒充、闸开火出路、现状核对表机制）。**流程教训**：五份方案有 3–4 份在修已修好的东西（六条原文已有、daysUntil 已统一、send_consequences 已拒收）——设计不得继承过期现状；路线图 S0 就是「按 file:line 核过的现状表」，以后派设计单先发经实测的基线。
- **2026-09-07 · P4 半程：W1/W2 过审，W3/W4/集成因额度中断，续跑**：W1（aeb5e9e）一轮修补后过审：灰度开关判据用「假包挂进注册表」才有牙（三处变异红）、危机管线按域接全（splitCrisisOpener/D15 兜底/杠杆闸回落/prompt/tools/MCP crisis_check/paste 七处）、生成器领域集合与注册表两向严格相等；5495 绿。W2（012924b）一轮修补后过审：检索按域过滤（缺省域 labor）、热线 hours 守卫盯 facts 字段。**裁决（W1 openQuestions）**：① 读路径回落缺省领域、写/选领域拒绝——采纳，作为政策写在 domainPackOrDefault 注释；② knowledge_search 无 case_id → 缺省领域（W2 已落），跨域须显式 domain；③ prompt.ts 危机窗贴附指令的热线字面量 → 搬进 DomainPack.crisis，加进 W3 任务；④ task-class 词表、leverageSubject 评测通路、intake persist 的 kind/role 字面量、families 直接 import LABOR_CAPABILITY_COPY——递延 backlog「共用层领域化第二批」，不阻塞 P4。W1 复审两条 minor（labor-zero-change ⑧ 用了不存在的能力名 claim_register/draft_create 永远绿；assertDomainPack 判据覆盖）转给集成员修。续跑：W3/W4 worktree 保留前任的提交与未提交改动，任务书加「续跑说明」。
- **2026-09-07 · 主理人对设计稿 §8 五项的裁决**：① 法条原件由我用动态工作流从中国官方法律网站下载入库（国家法律法规数据库 flk.npc.gov.cn、gov.cn、court.gov.cn、北京市政府/人社局），律师法 §13 等三项原文核实也由此完成；② 没有外勤，判例卡换源与核实全部由工作流做；③ **不设律师签字**——定位是低成本法律援助、尽可能用 AI：要件卡等内容改为「AI 多路独立推导 + 官方原文机械比对 + 对抗复审」，设计稿中「checked_by 律师」「待律师核」一律改口径；④ 留出集**已授权**（脱敏真实案件季度回放，永不进夹具、不入 git）；⑤ **知识库里不允许「二手转述」「待核实」**：每条二手信息必须追到一手信源并核实；追不到的卡从可用索引隔离（knowledge/quarantine/，带原因），不得以「待核实」状态留在库里。派单：grounding 工作流（S1 法源登记簿 + 原件下载 + 220 张卡逐张核实/换源/隔离 + 复审）。
- **2026-09-07 · 派单：知识库接地（wf_6d788104-1ae，worktree wt-ground 基线 aeb5e9e=ws/p4-w1）**：Sonnet 清点全库所引法律文件 ∥ Opus 做 S1（knowledge/sources.json 登记簿、scripts/fetch-source.py 非 .gov.cn 拒收、scripts/verify-quotes.py 引文⊆原件机械比对、gen-knowledge-index.py strict 守卫：隔离目录排除 + 白名单 + 引文核验 + 非「原文核实」即红）→ Sonnet 按批从 flk.npc.gov.cn/gov.cn/court.gov.cn/北京市政府与人社局下载原件（含律师法/生成式 AI 办法/标识办法逐字节选）→ Sonnet 11 批核实 220 张卡（法条挂原件核引文、判例卡换官方源逐项核对、数据卡数值逐字、无外部断言卡按 README 口径；追不到一手源整张隔离到 knowledge/quarantine/）→ Opus 收口（strict 重建索引、闭卷记录、compliance-originals.md、门禁、提交）→ Fable 抽 20 张亲自 curl 比对 + 10 张隔离卡反向找源 + 三处变异。counseling 39 张卡在 P4 集成后做第二遍。
- **2026-09-07 · 裁决（P4-W4 来问，不回消息以免 resume 分身）**：① W4 不合 ws/p4-w3；页面代码一律按注册表取包不认领域名；旅程判据用按 §16 形状造的第二领域夹具包跑「建案→首诊→事实卡」，并对 DOMAINS 里每个已注册包各跑一遍——采纳（W4 自报默认走此路）。② W1/W2 合并时的行为分叉「index.json 有 domain 但注册表没这个包」：W1 loadIndex 拒绝启动（全站 500）vs W2 跳过——**裁：生成器（构建期）保持两向严格相等即红，加载器（运行期）对未注册 domain 的卡排除并 console.error 点名、不拒绝启动**；两道闸互补不冲突（构建挡在提交前，运行期兜底不炸站）。W4 把 W1 三条判据改钉此行为、不删测试——采纳。集成员验收时核：strict 生成器变异红 + 加载器排除路径有判据。
- **2026-09-07 · 两条工作流额度中断后续跑（第二次）**：P4：W1 982d518 过审（两轮修补；自纠上一轮「parse.ts 顺带修真缺陷」叙事为伪问题——基线 calculatorKinds 本就是 claims DDL 值集，改名后必须跟改）；W2 f18f2bd 过审；W3 9efdb10 过审但留 1 major（分享/导出 redactNotice 承诺替换化名/编号而 maskContacts 只脱 PII）→ 裁进集成修；W4 9a3b1d7 复审 2 major（守卫白名单漏共用页、登录流领域选择无判据）修补中断于 b6362ee → 续跑。接地：66 份法律文件 14 批下载，失败 5（全总法援办法官网被拦、劳部发 1995-236 未定位、工资支付暂行规定与劳部发 1994-479 已定位但脚本抓取失败、北京工资支付规定重复登记）；前 3 批 + SOP 第 5 批 80 张核实完（换源 24、改引文 15、隔离 0）；收口员中断前已按 host 规则把 72 张 wikisource 源判例卡移入隔离区（记了试过 wenshu/flk/bjcourt）——**不算完成**：判例批次续跑时逐张去官方典型案例页再找，找到搬回。清点员发现：44 张判例卡唯一信源 wikisource 却标「原文核实」；「司法解释二」裸引与公司法解释二混淆风险；7 张判例卡 title 是坏 YAML。裁决补记：加载器对隔离区路径/未注册 domain 一律排除并点名、不拒绝启动（与 W4 那条同源）。
- **2026-09-07 · 知识库接地一轮收口（ws/grounding f11b346，29 agent）与验收裁决**：结果：220 张 → packs 159（原文核实 156 + 无外部断言 3）+ 隔离 61（全是判例卡，103→42）；登记 96 份官方原件（22 个 host）；279 条 statute_quotes 机械比对全部一致；改引文 86 处（含北京工资支付规定第十三条「工资支付记录表」→原件「工资支付表」这类真错）；换源 55；mohrss EdgeOne JS 挑战被脚本算 cookie 破解；加载器按裁决改为排除+点名。Fable 复审 pass 但 1 major + 5 minor，**验收裁定合并前必修**：① 个税法登记条目 raw 是 flk SPA 空壳、text 是 chinatax 粘贴，raw/text/sha 不同源——加 scripts/audit-sources.py（从 raw 重抽文本比对 text.txt、重算 sha）进 CI，全库过一遍；② 案例九（qingjia-shouxu-maodun-kuanggong-2025）court.gov.cn 465001 明文有要旨却被隔离——搬回；③ 非 gov host 借「行业规范」过闸（北大六院科普、危机中心简介、公证处收费）——新增 kind「机构官网」：只许用于该机构自身信息（热线/地址/收费），登记须写理由，不得作法律/判例依据；④ 隔离卡 52 张仍标「原文核实」——隔离区一律改「待核实」并加守卫；⑤ 判例卡 case_facts 是摘写、机器核不了——新增 facts.case_quotes（官方页逐字节选 + source_id），verify-quotes 扩到它，strict 要求 packs 里每张判例卡 ≥1 条核得过的 case_quotes；⑥ packs 里 8 处说明文字提到 wikisource/sohu 改写为「非官方转载站」。派单 grounding-fix 工作流。
- **2026-09-07 · 第三次额度中断后续跑**：P4：W1 3d0859f / W2 267c98e / W4 2aeb59c 过审；W3 336d073 两轮修补后仍 2 major（resolveCompanyRole「不点名 role 沿用同名行角色」在 labor 上是真实行为变更；「缺省领域逐字不变」判据自比）→ 另开 p4-finish：W3 第三轮修补 + 复审（≤1 轮再修）→ 集成员在 wt-int 392c499（已合 w1/w2/w4 + 三笔修补）上合 w3、全门禁、双领域冒烟。接地二轮：代码票已提交三笔（912c040 个税法原件换回 / 474c581 案例九搬回与机构官网 / e708f2e case_quotes 与 CI）但未返回 → 续跑补缺，随后 42 张判例卡逐字节选、收口、复审。
- **2026-09-07 · 归因更正 + backlog（来自援助律师会话）**：case 2 的 29 件未编号证据（id 27–55，09-06 11:13–11:42）由主理人**另一个助手会话**用同一把 key（#2 lawer-aid）经 MCP 直连上传，不是律师会话（它 12:00 后才首次调用，只走 REST 传了 19 件编号出证）；对用户而言仍是本人上传，作废与否由主理人定。**Backlog（采纳）**：REST 直写路径不落 agent_writes 而 MCP 落——同一把 key 两条路审计不平权；按设计稿 §4.4 让 REST 写也经注册表 effect 入口落 agent_writes（key_id + endpoint + target）。列入 S5「自带 agent 保护」切片。
- **2026-09-07 · 登录失效（403）中断后续跑（第四次）+ 裁决**：P4-W3 第三轮 64095d2 过审，留 1 major（tools.ts company_profile_upsert 对不合法 role 从基线「静默落签约主体并保存」变成「整条拒收」）+ 1 minor（upsertCompany 对 domain 写坏的旧案回 500）→ **裁：缺省领域回到基线宽松语义并钉判据；已存在案件的写路径按读路径口径回落缺省领域，UNKNOWN_DOMAIN 只给新建/选领域**，交集成员修后合入。接地二轮代码票 bbe3c35 完成六条并补两处（未复算桶钉子、报告分隔符），查出两处旧账（材料包 zip 文本截断 30720 字；社平封顶 xls 文本头三行人写）→ **裁：本轮机械重抽（python-docx/antiword、xlrd/libreoffice），未复算名单只留真无法抽取的**。42 张判例卡逐字节选与收口、复审续跑。
- **2026-09-07 · P4 验收通过并推 main（986fafe = 集成 466f1ea + 台账）**：集成员合入 w3 64095d2、修两条裁决（company_profile_upsert 宽松语义；upsertCompany 回落缺省领域），门禁 genDocs/tsc/vitest/pytest/genIndex/build 全 0；我本机复核 tsc 0、vitest 342 文件 6030 绿。双领域冒烟：counseling 旅程 20 步全绿（化名首诊、并行轨、十节事实卡 + 四条「法律解释存疑」、退费三口径、危机四步骨架含 12356、分享/导出/转介包三处脱敏，labor 对照臂不脱敏自证判据有牙）；labor 零差异：19 步 17 步逐字同，2 步各多一个可空键（cases.track: null、share.redact_notice: null）——**裁：加可空键不改值不算行为变化，通过**；未启用领域的自述错误只在服务端日志（注册不阻断登录是既定设计）——**裁：接受，backlog 给注册回包加 onboarding_skipped_reason**。上产口径：LAWER_DOMAINS_ENABLED 不设（只开 labor），counseling 待知识包第二遍核实后再开。CI 盯中，bundle 与 deploy-986fafe.sh 已备。
- **2026-09-07 · 滚版 986fafe 上产（P4 多领域管线 + counseling 包，灰度关）**：CI 34081761872 绿 → 备份 → ff edf07f0→986fafe（脚本因 git 给 8 位短 SHA 误报 HEAD_MISMATCH 在 build 前退出，核完整 SHA 后手动起 build；模板改为完整 SHA 前缀核对）→ BUILD OK → 重启 → /、sidecar、公网 200；惰性迁移已跑（cases.track / cases.domain 在）；GET /api/v1/domains 只列 labor；index.json 220 labor + 39 counseling（counseling 已注册但未开启，不进检索）；app.log 游标后零 error。09-07 第 1 次滚版。
- **2026-09-07 · 三项合规原文已取回（docs/design/legal-agent-inputs/compliance-originals.md，官方原件 sha 登记）与经理读法（无律师，按主理人裁决由 AI 给读法，标「解释存疑」）**：① 律师法 §13 两句：不以律师名义从事法律服务、不从事诉讼代理/辩护——产品面任何自称「律师/律师意见」的文案必须清零（首页 page.tsx:83 现有「律师意见」字样），工具面无递交/代理动作（已结构化）；② 生成式 AI 办法 §17 只覆盖「具有舆论属性或社会动员能力」的服务，一对一法律陪跑与私密档案不提供公共表达渠道——读法：不落入，不做算法备案，但保留安全评估材料准备；③ 标识办法 §4 引用深度合成规定 §17 第一款，「智能对话、智能写作」在列——读法：**适用**：对话界面显式标识、文书/导出 PDF 显式标识且元数据带隐式标识（§5）、用户协议写明标识规范（§8）。派单 ai-labeling（Opus 实现 + Fable 复审，worktree wt-compliance 基线 986fafe）。
- **2026-09-07 · 知识库接地二轮收口（ws/grounding 105bfa4）与验收**：packs 161（原文核实 157 + 无外部断言 4，待核实 0，二手 0）、隔离 60；判例卡 43 张 case_quotes 覆盖 100%（44 条）；全库 323 条引文机械比对一致；登记簿 96 条复算一致 94、未复算仅剩 2 份 pdf（pypdf 抽取随版本漂，不进量程）；材料包 .zip 自解 [MS-DOC] 重抽找回 4.4 万字（**更正**：30720 字不是截断，是巧合——往严重方向猜错也是猜错）；年鉴 .xls 用 xlrd 逐格重抽。Fable 复审 pass + 1 major：4 张判例卡引文把原件全角引号写成半角、verify 的 normalize 折引号所以绿——**裁：逐字＝只归一空白，不折引号字形；量尺改同源后以原件为准改卡**。派 grounding-integrate：合入 main（与 P4 在生成器/加载器/README/CI 冲突，口径：两边守卫都保留都生效），counseling 39 张卡以 GROUNDING_PENDING 显式豁免到 2026-09-14（第二遍核实另派），Fable 复审后上产。
- **2026-09-07 · AI 标识合规票（ws/ai-labeling 9357194）实现回来 + 裁决**：勘查：全站此前零 AI 标识；「律师」在产品面只有否定式免责句（page.tsx:83 是「不构成律师意见」，不是自称——台账上一条点名有误，更正）；仓里没有任何用户协议页。落地：法定半句「以下内容由人工智能生成合成」共用 + 行当半句进 DomainPack.copy（labor 不构成律师意见/不形成委托代理；counseling 不构成诊断治疗督导意见）；对话、文书、分享页、导出 PDF 四出口显式标识；PDF 元数据 Subject/Producer/Keywords 三要素（Producer=北京天开艾洛迪心理咨询有限公司（土八鼠），不给不兜）；新增 /terms/ai-labeling；律师自称守卫先摘否定式再拦八种自称。32 条判据、25 例变异全红。**裁决 openQuestions**：① 深度合成规定 §17① 原件未登记——合入时用 fetch-source 登记并在协议页写出适用链条；② 守卫放行否定式——采纳；③ 两截措辞——采纳；④ 存证证明 PDF 无 AI 文本不加标识——采纳（夸大也是错）；⑤ 单起协议页、不造同意条款——采纳，backlog「用户服务协议」；⑥ 低调模式只糊行当半句——采纳；⑦ 个案报告页与文件解读页也是生成内容——**合入时补**（现成组件一行）；⑧ §9 无标识导出未实现——采纳（无此功能）。
- **2026-09-07 · AI 标识票 Fable 复审 pass + 1 major 4 minor → 合入时修**：major：协议页写「消息列上方持续可见、不随对话滚走、不可关闭」但实现只是普通块级元素——**裁：做成真的持续可见（sticky 于消息列顶部或钉在输入框上方），协议页措辞与实现一致**；minor：DomainPack.copy.pages.aiLabelDisclaimer 改为必填键（registry 全字段守卫点名）；演示案件 DraftEditor 也加标识；律师自称守卫词表补「律师起草/在线律师/律师咨询/律师级」；协议页自称「协议的一节」→ 改为「标识规范说明（用户服务协议起草后并入）」。合入顺序：等 grounding-integrate 落地 wt-int 后，再合 ws/ai-labeling 并做上述修补 + openQ ①⑦（登记深度合成规定原件写适用链条；个案报告页与文件解读页加标识），Fable 复审，推 main 上产。
- **2026-09-07 · 知识库接地合入集成尖（wt-int dad6329）验收通过**：九处冲突逐处裁（两边守卫都在：生成器 domain 两向严格 + 八道扎根守卫；加载器一份排除循环同时管未注册 domain 与隔离区路径；README §2.2 D 类/§2.3 领域；CI 五步齐）；引号量尺改严后实红 11 张 26 条（不止复审报的 4 张判例卡，还有 7 张法条/SOP，534 号解答 13 条最多），全部以原件为准逐位改；**据实报备**：量尺仍保留 markdown 记号剥除（facts 引文自带加粗记号，不剥则百余条锚不上）与 gen 数值面的千分位归一（490 处，实测有牙）；counseling 39 张卡 GROUNDING_PENDING 豁免到 2026-09-14，**到期即失效**（生成器与 verify 判红并印路径）；TS 侧同一入口读豁免。门禁九项全 0；vitest 6044 绿；build 通过（worktree 软链 node_modules 换 cp -al 实体目录）。Fable 复审 pass + 3 minor（verify 对坏 YAML 静默跳过、到期判定 TS 用 UTC 日、verify 过期不无条件退出）→ 交 AI 标识合入一并修。派 labeling-integrate。
- **2026-09-07 · AI 标识合入集成尖（wt-int 154d63b）验收通过**：合并零冲突；对话页标识改 sticky（Playwright 实测滚到 5000px 仍 top=56 可点中）；aiLabelDisclaimer 必填键；演示文书、个案报告页、文件解读页三处补标识；律师自称守卫补五条服务式自称（**从严**：对「建议请律师起草」这类第三方句同样拦，判据钉死，可翻）；深度合成规定原件登记（第一次抓到的是新闻稿页、text 里找不到第十七条，纠正为全文页——量具先审的实例）；说明页 11 段引文逐字取自两份原件并配判据；接地三条 minor 修（坏 YAML 计错、到期按上海日历日、verify 过期无条件红）。门禁九项全 0，vitest 6109 绿，build 通过。Fable 复审 pass + 2 minor（说明页多说了「报告的分享页」这一出口；--card 单查也受全库坏卡影响）→ backlog 小修。推 main 上产。
- **2026-09-07 · 滚版 2613856 上产（知识库接地 + AI 标识 + 台账）**：CI 34088212587 绿 → 备份 → ff 986fafe→2613856（模板按完整 SHA 前缀核对通过）→ BUILD OK → 重启 → /、sidecar、公网 200；/terms/ai-labeling 200 且含「生成合成内容标识」；首页有说明页链接；domains 仍只列 labor；index 161 labor + 39 counseling（豁免）；隔离 60；原件 97；app.log 游标后零 error。09-07 第 2 次滚版。汇总邮件已发主理人（四件待拍板：咨询包何时开灰度、用户服务协议、律师守卫从严口径、百炼与 ASR 两旧项）。
- **2026-09-07 · 派单：counseling 知识包第二遍核实（wf_6199d2e3-db6，worktree wt-counsel 基线 2613856）**：三批 Sonnet 逐张核实（法条挂登记簿 source_id；伦理守则只认学会官网发布页、PDF 抽不出文本则改条号指引不逐字引用；判例只认官方页，冬妮案未终审隔离；数据卡核官方出处；无外部断言卡去引文）→ Opus 收口删除 GROUNDING_PENDING、strict 全库绿、闭卷第六章 → Fable 复审（抽 10 张 curl 原页、隔离卡反向找源、变异）。目标：豁免到期（09-14）前 counseling 包零待核实零二手，可随时开灰度。
- **2026-09-07 · 主理人对邮件四件的裁决与执行**：① 「先开」→ 生产 app.env 加 LAWER_DOMAINS_ENABLED=labor,counseling，重启后 /api/v1/domains 列出心理咨询纠纷（39 张咨询卡仍在豁免期，第二遍核实进行中）；② 用户服务协议：授权我起草，开两个 Fable 评审，我决策 → 分支 ws/terms-draft（docs/legal/用户服务协议-草案.md）；③ **律师转介口径（裁决）**：不轻易建议找律师，能由智能体完成的都由我们完成；只有法律上必须律师签字/背书、绕不过去的事项（诉讼代理与辩护、以律所名义出具律师函等）才提示，且不向用户推卸责任——守卫从严口径**维持**，charter/文案里「到仲裁自然分流律师」改为「只提示法律上必须律师的事项」（backlog 票，Opus）；设计稿 §7.2 案卷包措辞同改；④ OCR：NBDpsy 发票 OCR 用的也是裸别名 qwen3-vl-plus；生产 OCR_MODEL=qwen3-vl-plus 实测 200 可用；qwen3-vl-plus-2025-12-19 与 qwen-vl-ocr 均 403 未开通——**带日期快照不是必须**，只是为了防模型静默换代；结论：不必开，维持裸别名并记 served model。
- **2026-09-07 · OCR 模型口径（主理人：不用带日期版本；qwen-vl-ocr-latest 已开通）**：生产 OCR_MODEL 本就是 qwen3-vl-plus（与 NBDpsy 同）；sidecar/ocr.py 文件头「改回 dated」的 TODO 作废，下次滚版删掉。实测 qwen-vl-ocr-latest 200 可用：0.5s vs 1.5s，tokens 相同，但冒烟图上把「解除劳动」输出成「解除劳」（少一字，qwen3-vl-plus 完整）——**量具先审**：派 Sonnet 在生产用 case 2 真实材料（PDF/eml/图片 10 份）做 A/B（相似度、漏字、耗时、tokens），据读数决定是否切换或按类型分流。
- **2026-09-07 · 主理人裁决 OCR 选型标准**：「用效果好的、稳定的，这个成本省不了」——A/B 只按识别准确率与稳定性（漏字/多字/换行保真、重复运行一致性）定，速度与价格不作为切换理由；两者打平则维持现状 qwen3-vl-plus（已端到端验证到简报）。
- **2026-09-07 · OCR A/B 结论（case 2 真实材料 10 份：4 PDF + 3 eml + 3 图片，生产机内解密处理，材料未出生产机）**：按生产同一调用形态（不传 ocr_options），qwen-vl-ocr-latest **10/10 拿不到可用文字**（9 份输出版面坐标串、1 份 2.9MB 大图空响应），qwen3-vl-plus 10/10 产出准确连贯文字（有真值的 4 份相似度 0.53–1.00，低分为表格排版差异；「解除劳动合同」完整，冒烟图截断未复现）；latest 的 prompt token 在大图上反而多 1.7–3.2 倍。**裁定：维持 qwen3-vl-plus，不切换**（符合主理人「效果好、稳定，成本省不了」）。开放项：若将来配 ocr_options 走 ocr-latest 专用模式可再测一次，不排期。定价未取到官方原始页，不给数字。
- **2026-09-07 · counseling 知识包第二遍核实收口（ws/counseling-grounding 0fbeba9）验收通过，合入 wt-int**：39 张 → 可用 37（原文核实 36 + 无外部断言 1），隔离 2（顾歌案唯一来源为代理律所宣传文、冬妮案未终审）；伦理守则第二版以中国心理学会期刊《心理学报》网页版登记为行业规范一手源，三张伦理卡引文逐字核过；四张风险卡改为「一手源在手、法律结论存疑」：confidence 原文核实 + 每卡一条机械核过的引文 + 输出闸；民诉法答辩期条号从旧版 125 条更正为现行 128 条；GROUNDING_PENDING 删除，全库零豁免：198 张卡、365 条引文一致、登记簿 101 条。Fable 复审 pass + 2 minor（两张卡陈旧措辞指向已核实的伦理卡/已删的待核实节）→ 交下一集成修。**待改口径**：风险卡与 counseling.ts 里「待律师复核/未经律师书面确认」措辞须按设计稿 §9 改为「法律解释存疑：给依据原文与分歧点，不下结论」——归 lawyer-referral-copy 票（正在跑）统一处理。执行者观察：三批核实员共用同一 worktree（我派单时只给了一个），未出事故但有串台风险，下次每批独立目录。
- **2026-09-07 · 滚版 bfcda2e 上产（counseling 知识包第二遍核实，零豁免）**：CI 34093363263 绿 → ff 2613856→bfcda2e → BUILD OK → 重启 → /、sidecar、公网 200；domains 列两领域；index 161 labor + 37 counseling；GROUNDING_PENDING 不存在；counseling 隔离 2；app.log 游标后零 error。09-07 第 3 次滚版。
- **2026-09-07 · 第五次额度中断续跑 + 裁决**：律师口径票修补轮 ff1fd11 完成（守卫改按六种构造判、待复核节与闭合清单先后写进 prompt、四份对外文档件数跟 lawyerMandatory 走），复审二死于额度 → 续跑。**裁决其 openQuestion**：法援/工会/监察/热线属公共免费资源卡，不是「推给律师」——**保留**（执行者已按此恢复 charter §1），但只能作为资源给出、不得用来收尾。P5 三票各有 31–54 个未提交文件时中断 → 任务书加续跑说明；C1 境外模型改为「注册页单独勾选、默认未勾、未勾全境内」后续跑。
- **2026-09-07 · 律师口径票（ws/lawyer-referral-copy ff1fd11）复审 pass + 4 minor → 派 lawyer-integrate 合入 wt-int**：全仓「律师」362 行普查四类归位，b 类兜底句产品面/charter/skill/知识卡清零；lawyerMandatory 两条（诉讼代理与辩护、以律所名义出律师函，锚律师法 §13 原文）；守卫按六种句法构造判定；四份对外文档件数跟清单走；G2 评测机械断言绿。四条 minor（S4 书名号漏洞、why 漏「基层法律服务工作者」民诉法 §61、guide.ts 未列举、labor-zero-change 不覆盖 prompt）与「待律师书面确认」→「**法律解释存疑**」口径改名、两处陈旧措辞、ocr.py dated TODO 删除，一并在合入时修，Fable 复审后推 main。
- **2026-09-07 · 主理人贴出中转账单（令牌「土八鼠」）复盘**：09-04 下午~75 次调用为评测跑批与冒烟（sonnet-5/opus-5），非用户流量；09-05~07 无用户 Claude 流量（入门档走境内 DeepSeek 直连）；09-06 20:14 两条是我的模型探针。**两个发现**：① 每次请求「缓存写」3 万–27 万 token、几乎零「缓存读」——系统提示前缀每轮在变，提示缓存从未命中；中转按 0.1 倍计缓存写所以暂不贵，走官方价会放大 → backlog「提示前缀稳定化（charter 在前、事实卡等动态段在后）」；② 分组列显示流量经「Claude-Code-2 / Kiro-Claude-1 / Vertex-Claude-1」等池——即中转把请求分发到共享的订阅池而非官方 API 密钥：稳定性差（「上游负载饱和」、首字 3–33 s）、中转方可读提示词、与协议「境外接收方 Anthropic」表述有出入（实际接收方先是中转商）。建议：中配/高配关键步骤改走官方渠道（Vertex/Bedrock 正规合约）或至少把令牌分组固定到 Vertex 组；协议 /terms/overseas 页须如实写明中转服务商为接收方之一。待主理人定。
- **2026-09-07 · 律师口径合入 wt-int（a53814a）验收通过 + 3 minor 派小修**：零冲突合并；口径改名全仓一致（lawyerReview → interpretationDisputed，四张风险卡/纪律句/标记词/TODO 闭卷/退费算钱器 flag/四处 SOP 指针统一为「现行法律解释存疑：以下是依据原文与分歧点，土八鼠不下结论」——执行者把范围扩到算钱器与 SOP，**裁：采纳**，两种口径并存对用户可见即为错）；S4 按位置判、§61 补基层法律服务工作者、guide.ts 列举两件、labor-prompt-snapshot 新基线、两处陈旧措辞、ocr.py dated TODO 删除。门禁六项 0（vitest 满并发下 filesGc 因机器负载间歇超时，--maxWorkers=2 全绿）。Fable 复审 pass + 3 minor：why 句前半「只能是执业律师」与民诉法 §61 自相矛盾（**必修**，进 prompt 的用户可见文本）、guide.ts 枚举未被判据钉住、ocr.py:135 悬空注释 → 派 Opus 小修后推 main。
- **2026-09-07 · 律师口径三条小修（da84c71）→ 推 main 8027f32**：why 句按民诉法 §61 列全可委托代理的几类并说明我们不属其中、本人出庭为默认路径；labor-prompt-snapshot 基线换新（注释记为预期改动）；§61 判据加负样本「只能是执业律师」；guide.ts 两件枚举被判据钉住（变异证实件数判据原有盲点）；ocr.py:135 注释改正。本机门禁 tsc/vitest 6150/gen strict/index diff/verify 全 0。CI 盯中，bundle 与 deploy-8027f32.sh 已备。执行者提的 commit 署名冲突（任务书 Opus vs harness Fable）：**裁：子代理提交按任务书署 Opus，经理提交按 harness 署名，两者并存不改**。
- **2026-09-07 · 主理人两条裁决**：① **官方出的案例可以采信**（仲裁委/法院/人社局官方发布的典型案例 = 一手源，与现行「.gov.cn 原文核实」口径一致；主理人贴的「产假工资保卫战」案例正在查官方出处）；② **现行法律解释存疑时，应找过往相似判例**——「解释存疑」不能只摆分歧点，要检索官方发布的相似案例，呈现实际怎么裁的（标明是个案不是规则），库里没有就明说并登记缺口。派单 disputed-precedents：Sonnet 按四项存疑点（强制报告主体、咨询合同定性、记录保存年限、机构许可备案）到官方站点找相似案例并按四步法建判例卡；Opus 给 interpretationDisputed 加 precedents/检索键并改 tiebreak 指令（存疑→先给判例→再给分歧点→不下结论）；Fable 复审。
- **2026-09-07 · 滚版 8027f32 上产（律师口径 + 解释存疑改名 + 小修）**：CI 34104185847 绿 → ff bfcda2e→8027f32 → BUILD OK → 重启 → /、sidecar、公网 200；knowledge/packs 零「待律师」；app.log 游标后零 error。09-07 第 4 次滚版。
- **2026-09-07 · 「产假工资保卫战」出处查实**：真实来源是**天津市红桥区人民法院**「五一特辑丨以法护劳 守护荣光——红桥法院发布 5 起劳动争议典型案例」（tjhqfy.tjcourt.gov.cn/article/detail/2026/05/id/9301098.shtml，官方澎湃号 2025-05-02 同文），法院判决而非仲裁裁决；案情要素（朱某、3000 元、2021 年 7–11 月、一万余元、产假工资=劳动报酬适用特殊仲裁时效）逐字吻合。主理人文档上的「朝阳区劳动人事争议仲裁委员会」水印**与来源不符**——朝阳仲裁委站点无此案例痕迹，疑为转载合集套用水印。**裁**：内容可采信（官方法院发布），但只能以「天津红桥法院典型案例·外地类比」建卡，不得标朝阳先例；待主理人说明文档来源后决定是否追同一合集另外四案（戎某工资福利、平台用工「创客」劳动关系、女职工管理岗/操作岗退休年龄、韩某未签合同二倍工资）。建卡并入 disputed-precedents 之后的知识滚版。
- **2026-09-07 · 第六次额度中断续跑 + P5/存疑裁决**：P5：C1 同意闸 ccb3ae2 过审（+49 判据、七组变异；4 minor）；C3 入口与页面 97654f0 过审但 1 major（/terms 第五条第 5 款把「注册时单独勾选」改写成「设置中开启前征得同意」——与主理人口径不符，集成时改回）；C2 数据生命周期两轮修补至 ca04317（首轮 blocker：清理任务把护照实名材料当孤儿文件删；次轮 blocker：retention-worker 每小时跑 gcOrphanFiles 同类风险；major：软删过滤未覆盖期限提醒与守望计费 JOIN），第三轮复审续跑。**裁决 C1 四问**：不勾≠撤回（登录只往开写）采纳；N-2 不可达前置守卫接受；境外文案以 C3 页面为正本共源；其余三处 getProvider 显式 overseasAllowed=false。C1 不单独发版，三票一起合。存疑判例：调研只找到四川高院退费案（已在库），其余三项官方源不可达（samr 403 / nhc 412 / gsxt 521 / 最高法站内检索零命中）→ precedents 只挂一条，其余登记「判例缺口」；impl 续跑。
- **2026-09-07 · 存疑判例机制（ws/disputed-precedents 0aee35e）复审 pass + 1 major 3 minor → 派 Opus 小修**：DomainPack.interpretationDisputed.items 改为 {text, precedents, searchKeywords}（precedents 可空、searchKeywords 不可空——「查过没有」与「没人查」不能同形）；模型指令第 4 条改三步（先官方案例实际裁法并标个案非规则→再依据原文与分歧点→仍不下结论不推律师）；事实卡逐条印过往相似案例；四张风险卡加「过往相似案例」节；TODO 清单新增「解释存疑·判例缺口」含信源可用性总表。八条变异红。复审 major：事实卡空臂断言恒真（纪律句本身含同一短语）→ 修；minor：纪律末句「不是把事情交出去的理由」无判据、「不下结论」两处冗余、四川案卡内 https 链接取不到（须 http+UA）。**裁**：小修后待 P5 集成完再合入 wt-int；三项缺口的 Playwright 调研另排；报告节也列案例；卡内检索词改引用包内 searchKeywords 不再重复。
- **2026-09-07 · 存疑判例小修完成（ws/disputed-precedents fb604d4）**：空臂句提成常量 NO_PRECEDENT_LINE 并先断言纪律句不含它（守卫的守卫）；precedentLine 提到 lib/knowledge/precedent-line.ts 供事实卡与报告共用（避免 cases↔agent 循环依赖）；报告节两臂；风险卡词表改为指回包内 searchKeywords 且守卫禁卡内再出现词表形状；四川案卡 https→http（实测 http+浏览器 UA 200/22200B）。六条变异红；tsc 0、vitest 6161 绿、gen strict 0、verify 0。**裁**：① 「卡内 URL scheme 与登记簿一致」守卫——现存 13 处不一致（多为卡 http/登记 https），先补一票修完再立守卫，进 backlog；② 卡内「本轮试过」的历史检索串不算第二份词表，边界采纳；③ meta.json fetch_method/fetch_url 矛盾归同一 backlog 票。合入顺序：等 P5 集成落地 wt-int 后再合。
- **2026-09-07 · P5 合规集成落地（wt-int 4077de0）验收 + 追认**：三票合入（C1 同意闸 ccb3ae2、C2 生命周期 f2aa765 三轮修补过审、C3 页面 97654f0），集成员按裁决修四组遗留；门禁六项 0；本机冒烟 19/19 + 页面 6/6 + 四套守卫 76 例；变异四处红。**追认集成员四项自裁**：① 路由统一用 C1 的 overseasAllowed（按型号判），撤下 C2 的 domesticOnly（按 provider 判会在 RELAY_ROUTE_DOMESTIC 下误伤境内型号）——追认；② consents 合成一张表 + revoked_at，撤回可逆，overseasModelsAllowed = 开关 ∧ 有效同意——追认；③ lawyer-referral-guard 加三条固定否定搭配、MANDATORY 放宽到「必须由（执业）律师」——追认；④ 设置页境外同意改用 C3 的两钮 Dialog——知会。**上线口径**：/terms 与 /terms/overseas 仍有三处 Pending（个保法 §38 出境路径责任人、中转商名称与所在地、境外接收方联系方式）+ 协议附二五项业务事实未填——**裁：P5 代码可上产但「协议生效」与「境外勾选」须挂旗默认关（LAWER_TERMS_LIVE），注册页在旗关时不强制勾选、不出境外框、协议页带「预览」横幅；情绪记录同意、实名采用同意、数据生命周期、帮助与投诉入口照常上线**；待主理人补齐事实后填 Pending、翻旗。存疑判例分支 fb604d4 已合入 wt-int（0b8b55f，gen/verify/tsc 0）。派 terms-flag 小票。
- **2026-09-07 · 协议生效旗 LAWER_TERMS_LIVE（wt-int 1bfa5d0）验收通过**：唯一读取口 termsLive()（缺省关、只认 1/true）；关臂：注册页不渲染三个勾选框、发码闸恒开、不落 terms/adult/overseas 行、境外恒 false、POST /me/preferences 开境外回 TERMS_NOT_LIVE、三张条款页「预览版」横幅；开臂 P5 行为逐字不变；情绪/实名采用/生命周期不受控。12 条变异红；双预置全量 6610 绿；build 路由表 /login 与三条款页动态渲染（协议发布后也不改回静态）。Fable pass + 2 minor（开臂测试不清理 env；rest-index 未写 TERMS_NOT_LIVE）→ backlog。**裁**：ai-labeling 页「无横幅」前瞻守卫留着不计牙；OPS.md「占位清零后改判据不改占位」口径确认；worktree node_modules 改硬链接供给（记 memory）。推 main 上产。
- **2026-09-07 · 滚版 c90a73d 上产（P5 合规 + 存疑判例 + 协议生效旗关）**：CI 34140528484 绿 → ff 8027f32→c90a73d → BUILD OK → 重启 → /、sidecar、公网 200；/terms（预览横幅在、零「草案」）、/terms/overseas、/terms/processors、/terms/ai-labeling、/settings/help、/login（旗关无勾选框）全 200；LAWER_TERMS_LIVE 未设；迁移：consents/complaints 表在、cases.deleted_at 在；app.log 游标后零 error。09-07 第 5 次滚版。翻旗前置：主理人八项事实（§38 出境路径责任人、中转商名称与所在地、Anthropic 联系方式、公道值换算与赠送、退款与守望欠费、服务邮箱、短信与实名核验服务商、产假案例文档来源）。
- **2026-09-08 · 晨检 + 派卫生票**：生产 c90a73d app/公网 200、双服务 active、上产后 app.log 零 error。派 hygiene-0908（worktree wt-hygiene，node_modules 改硬链接供给）：卡内 URL 与登记簿 scheme 一致 13 处修 + 守卫 (i)；meta.json fetch_method 与实际 scheme 一致 + audit 新关；consent 三组测试 afterAll 清理 env；rest-index 补 TERMS_NOT_LIVE 说明。Fable 复审。
- **2026-09-08 · 主理人：「卡在我这里的我都给不了答案，暂时放过」**：八项事实（§38 出境路径、中转商名称所在地、Anthropic 联系方式、公道值换算与赠送、退款与守望欠费、服务邮箱、短信与实名核验服务商、产假案例文档来源）与中转换官方渠道**全部递延**，无日期。协议生效旗与境外选项维持关。**自查条件**：2026-09-21 复查日再问一次；期间不做协议定稿与境外相关工作。不依赖主理人的活继续：卫生票（在跑）→ 天津红桥法院五案建卡（官方页可采信，外地类比）→ 设计稿 S2 条号/数值运行时闸 → S4 来源四档与要件三态 → 提示缓存前缀 → REST 写路径落 agent_writes。
- **2026-09-08 · 卫生票（ws/hygiene-0908 4daf3f4）验收通过，合入 wt-int（ff）**：13 处卡内 URL 只改 scheme 与登记簿对齐 + 守卫 (i)（扫整卡含正文来源行，同 host+path 异 scheme 即红）；fetch-source 按实际 scheme 记 fetch_method（存量 16 条同型一并改齐）+ audit 第⑥关（缺 meta 亦红）；三组同意测试 afterAll 清旗 + 结构守卫；rest-index 补 TERMS_NOT_LIVE（按既有守卫不写环境变量字面量）。pytest 165、vitest 6613 双预置绿、gen/verify/audit 0。Fable pass + 1 minor（http 单档失去三次重试）。**裁**：五条 openQuestions 全部采纳（16 条一并改、缺 meta 判红、守卫口径优先、隔离区 5 处反向分叉另开票并扩 (i) 量程、保留 arm-cleanup 守卫）；http 重试改为显式重试进 backlog。等天津五案建卡过审后一起滚版。
- **2026-09-08 · 派 S2 闸链补齐（wt-s2 基线 c90a73d，wf_d6e8ce0f-8e4）**：StatuteGuard（条号放行集 = 本轮 statute_quotes ∪ 核心条映射命中；文号/条款项负样本）、ValueGuard（不在 claim_calc 出参与 facts.values 的带单位数字标【数值无来源】，⑧ 补进原文免检，日期只认 deadline_list）、lib/agent/gate-chain.ts 顺序常量 + 相邻闸交互用例、gate_report 替换率/漏网率两列 + 2% 预算、登记簿 status≠现行锚点替换。L 级验证 + 变异矩阵；Fable 复审 ≤2 轮修补。
...
- **2026-09-08 · S2 两轮修补后仍 fail（wt-s2 2ec5624，3 major）→ 裁决 + 第三轮**：已落地：StatuteGuard（流上、放行集 = 本轮 statute_quotes ∪ 核心条、多行原文与交叉引用兼容、异类嵌套引号）、ValueGuard（calc 出参/档案事实/用户原话/facts.values 四路放行、约写与 ±0.5% 容差分账、倍数记号按 calc kind）、GATE_CHAIN 顺序常量 + 9 对相邻闸交互用例、gate_report 替换率/漏网率两列、真库 318 条引文全链替换率 0%。复审剩三条真问题：① ⑨ 的 exact 放行集不带类别（calc 出参里的 12 个月/3 倍/7.5 年被当无单位数字，模型编「12 倍」「7.5 倍」照放行）；② 裸「第 N 条」把合同/手册/制度条款当法条误标（裁员场景高频）；③ 「放行集含核心条映射命中」无仓内判据。**裁**：第三轮只修这三条（exact 按类别分集、裸条号前 6 字含合同|协议|手册|制度|章程|规定|通知|裁决|判决 即非法条、核心条探针进 gate-chain 第五节）；**上线口径**：⑥ 按替换模式；⑨ 按**观察模式**（只记 notice 与 gate_report、不改正文）上线，待有 key 的真模型跑批量出误标率 <2% 再切替换（常量开关 + 判据）；用户面出路话术（frames.ts 置 null 的 WS2 口径）另开票；commit 署名冲突维持「子代理按任务书」。
- **2026-09-08 · S2 第三轮（wt-s2 939394f）复审 pass + 1 major 2 minor → 派小修后合入**：三条 major 已闭（⑨ 按金额/倍数/百分比分格放行；裸条号前 6 字含合同|手册|制度等即非法条计 ambiguous；核心条注入进放行集有真编排探针）；观察模式常量 VALUE_GUARD_MODE='observe'（记 notice 与 gate_report、不改正文）；9 条变异红；全量 6831 绿、eval 快集 527、六套守卫 108。复审 major：calc 出参 formula 里「× 150%/200%/300%」字面量落金额格，加班费法定倍率被标——**合入前修**（% 字面量直接收进百分比格）；minor：不带《》的「劳动合同法第四十七条」被载体排除放过（窗口末为法规后缀即仍按法条）、ambiguous 拆载体/序数两列。**裁 openQuestions**：观察期用户面零告知接受（notice 只进运维与归档）；切换判据的取数口径（归档 notice.value_marked 逐条人工复核算误标率）归评测官 backlog——待办绑可自查条件；算钱器结构化带单位出参立为独立小票；百分比字段命名约定由「% 字面量直接归类」替代。
- **2026-09-10 · S2 小修（689cb35）合入 wt-int → 推 main 9e7c7d9c → CI 34223628126 绿 → 上产中**：% 字面量直接收进百分比格（加班费 150/200/300% 放行、350% 照标）；不带《》的法名末尾为法规后缀仍按法条判（「公司规定第二十条」形态为已知缺口，需法规名录）；ambiguous 拆载体/序数两列贯通 gate_report/notice/评测报表。合并 tsc/vitest 6840/eval 527/gen/verify/gen:docs 全 0。上产前核 3a3a306 以来 app.log 两条 error 均为扫描器伪造 Server Action ID，噪音。
- **2026-09-10 · 滚版 9e7c7d9c 上产（S2 闸链：StatuteGuard 替换模式 + ValueGuard 观察模式 + 闸链顺序表 + 替换率/漏网率报表）**：BUILD OK → 重启 → /、sidecar、公网 200；docs/design-notes/gate-chain-order.md 在；VALUE_GUARD_MODE 观察；app.log 游标后零 error。09-10 第 1 次滚版。**待办绑自查**：观察期误标率取数口径（归档 notice.value_marked 人工复核）归评测官；复查日 2026-09-24 看有无真模型跑批读数，有则裁是否切替换模式。
- **2026-09-10 · 派 S4 来源四档与要件三态（wt-s4 基线 9e7c7d9c）**：source-tier 单一枚举 + asserted_by 迁移回填；事实卡档位由数据推出；elements.ts 三态/burden 类型与 buildElementSheet 纯函数（内容归 S6）；facts_token 高危写前置（case_update/claims_upsert/deadline_set/draft_write，缺/过期 FACTS_STALE 夹最新事实卡；站内自动携带）。L 级验证；Fable 复审 ≤2 轮。
- **2026-09-10 · S4（wt-s4 b59c97e）复审 pass + 3 minor → 派小修后合入**：source-tier 单一枚举与 asserted_by（外壳按身份填、不收入参）迁移回填、事实卡档位由数据推出（时间线抬头改数出来的「有书证支撑 N 条」）、取证闸（阶段 ∈ DomainPack.evidenceGate.stages 且关键事实无书证 → 首行点名 + 强制取证卡）、elements.ts 三态/burden 类型与纯函数（内容归 S6）、facts_token（四高危写前置、FACTS_STALE 夹最新事实卡与新令牌、client_ref 重放豁免、站内自动携带、事实卡渲染唯一入口）；+116 判据、14 变异红、6956 绿；复审用 9e7c7d9c 原件建真旧库核迁移。**裁 openQuestions**：取证闸暂按事实组计数，S6 落地后改按 P0 要件；网页端改阶段时由服务端替 jwt 会话签令牌（将来）；两处取数窗口不同偏向报警，接受；claims_upsert 覆盖 vs company_profile_upsert 补充语义写进接入说明；站内令牌为构造性保证，S5 再议真核验。minor：首诊直落 stage 绕过取证卡种子；case_intake/欢迎事件 asserted_by 未按身份填；注释行当名词——一并小修。
- **2026-09-10 · S4 小修（c8ad654 + ea09aee）合入 → 推 main 7f0ff47e**：首诊过取证闸种子卡、MCP/REST 首诊与欢迎事件按身份填 asserted_by（system）、注释中立化并把 source-tier.ts/elements.ts 纳入 registry-guard 共用层名单、接入说明写清覆盖/补充语义、REST 时间线端点补 assertedBy 与 source_tier 透传（说明书宣告过的入参，坏值 400 INVALID_SOURCE_TIER 不静默折档——**裁：保留**）。labor-baseline 动一格（claims_upsert.source_tier 描述，预期）。合并 tsc/vitest 6970/eval/gen/gen:docs 全 0。CI 盯中，bundle 已备。
- **2026-09-10 · 派提示缓存前缀稳定化（wt-cache 基线 7f0ff47e）**：system prompt 分静态（charter+纪律+闭合清单，逐字节恒定）/半静态（检索 packs 按 id 排序）/动态（事实卡、本轮指令）三层，静态段最前；稳定性判据（两案件/两轮 diff、静态段无日期/id）；anthropic 直连加 cache_control 断点 ≤4，中转形态不变；usage 的 cached/cache_creation 读数入账单与评测报表。charter 文本一字不改（labor-prompt-snapshot 必绿）。Fable 复审。
- **2026-09-10 · 滚版 7f0ff47e 上产（S4 来源四档 + 要件三态类型 + facts_token）**：CI 34388914684 绿 → BUILD OK → 重启 → /、sidecar、公网 200；迁移列在、存量回填「自述/user」；app.log 游标后零 error。09-10 第 2 次滚版。
- **2026-09-10 · 提示缓存前缀（wt-cache d0f10e0）复审 pass + 2 minor → 派小修后合入**：system prompt 三层（静态 charter+纪律+闭合清单 5790 字逐字节恒定、半静态 packs 按 id 排序、动态事实卡等）；同案两轮/两案静态段相同率 100%（原先只有 charter 2905 字稳定，其后紧跟每轮变的时刻行）；anthropic 直连 cache_control 断点 ≤4，中转形态不变；PROMPT_CACHE notice + 评测报表「缓存命中率」列（未回报印「未回报」）；billing 两桶两臂。**裁决**：① 危机/空包指令改回首要位（紧接静态段之后、packs 之前）——无真模型对照前不拿依从性换缓存；② 「六套守卫」成文 = index-guard / lawyer-referral-guard / migrate-idempotency-guard / registry-guard / ledger-entrance-guard / labor-prompt-snapshot（108 条）；③ token_usage 可空列 + reconcile 口径改动立独立小票；④ 危机窗 repeatCardNote 留在 packs 段不动。minor：hit_rate 读桶未回报 → null 无判据；断点偏移在 PII 脱敏前算（counseling 两张卡含长 URL id 会被银行卡规则吞、第二断点偏 10–20 字）——小修中。**自查条件**：上产后首批流量 PROMPT_CACHE cached_read>0 的轮次占比，复查日 2026-09-24（恒 0 则分「中转不给字段」与「上游不做隐式缓存」两路查）。
- **2026-09-10 · 提示缓存小修（34eede2）合入 → 推 main 9f13e2af**：危机/空包指令回首要位（段序 静态→首要位→packs→动态，第一断点不动、危机轮只作废 packs 块）；hit_rate 空桶 null 判据；断点按 PII 脱敏后重算（withPiiRedaction 内重映射，system 轮 tool_calls 也纳入分块脱敏）；「静态段与本域全部卡脱敏前后逐字相同」判据：labor 零变化，counseling 两张含长 URL id 的卡被银行卡规则吞（KNOWN_SWALLOWED 白名单，新增此类卡会红——刻意）。复测：同案两轮/两案共同前缀 6217/6211 字均越过第二断点。合并 tsc/vitest 7013/eval/gen/gen:docs 全 0。CI 盯中，bundle 已备。**裁**：KNOWN_SWALLOWED 形态保留；后续 counseling 卡把长数字 URL 移进 frontmatter sources 与 labor 一致（写进 README，backlog）。
- **2026-09-10 · 派 REST 写路径审计平权（wt-audit 基线 9f13e2af）**：lib/audit/agent-writes.ts 单一入口；api_key 身份的全部 REST 写路由落 agent_writes（key_id + endpoint + method + target），jwt 不落；结构守卫扫写路由必须引用入口（白名单 auth/oauth/keys/tools/admin/consents）；agent_writes 增 endpoint/method 列（MCP 行 mcp:<tool>）；并入 token_usage 可空两桶小票（未回报 ≠ 0，reconcile 口径）。Fable 复审。
- **2026-09-10 · 滚版 9f13e2af 上产（提示缓存前缀稳定化）**：CI 34395241375 绿 → BUILD OK → 重启 → /、sidecar、公网 200；app.log 游标后零 error。09-10 第 3 次滚版。自查：复查日 2026-09-24 看 PROMPT_CACHE 读数（cached_read>0 轮次占比）。
- **2026-09-10 · 派 S6 要件卡与争点表（wt-s6 基线 9f13e2af）**：labor 四诉求（2N/N/欠薪/双倍工资）要件卡内容，basis 必须解析到登记簿原件核过的 statute_quotes 锚点，举证责任条号核不到只能 unverified；事实卡「要件表」分区（〔未记录〕只许「需补」）；issue-table.ts 争点派生 + 报告两节渲染派生集合；element_fill / element_sheet_get / issue_list 三能力；claim_calc 风险区间（书证最低值～自述最高值，差额对应缺证要件）；要件级断言、争点→行动卡链接率、预算实测。Fable 复审 ≤2 轮。
