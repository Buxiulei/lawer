...
| **2026-08-30 09:30 后当日内** | 受控首发验收：prod job_runs 应现「期限提醒」新行且 items_ok=1；hubaiyipku@163.com 见实物信与模板逐字一致；随后将测试期限行（deadlines：case_id=2/kind=自定义/due=08-31，manager 08-29 解锁的唯一 INSERT）置 resolved_at **不 DELETE**（行留库存证，与 job_runs/notify_log 互证）。**无新行=cron 没跑；有新行 items=0=staging 不在档** | 验收通过前，「提醒已经在保护你了」这句话禁止对用户说（后台技术留下的原话条件）；用户对该测试行有至 09:30 的否决窗口 | manager 派子代理核对 |
