...
| 2 品牌+首页 | **2a ✅ 合入** #66（改名，1510/9 复核）：出站邮件只用短名「土拨鼠」（全称含两个敏感词）、PWA `short_name` 留「陪跑」、charter 自称独立 commit。**2b**（logo 落位 + 首页首屏）等用户挑字母 | 前端页面 |
| 3 内容区排版 | **派出**（08-27，2a 合入后）规格 `docs/design-notes/2026-08-27-批3-内容区排版.md` | 前端页面 |
