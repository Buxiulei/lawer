...
**📐 SSE 内存实测收官（2026-08-30 00:47，探针零事故收工：真库零写、余额精确复原 365、临时 key 删净带 401 证明、MainPID 未变）**：**0.54MB/并发连接**（cgroup 实测，N=12 干净点+物理核对互证）→ 1000 路 ≈625-660MB = MemoryHigh 的 61-64%，**进程内存正式降级为非第一墙**（memory.events 全程六项全零）；尾部风险留案：长 tool 循环连接 1.5-2MB/条，若 1000 路中占比高会击穿 MemoryMax——与 P0-cost 的 MAX_TOOL_ROUNDS 经济学同根。空载连接地板 7.6-207KB/条随规模递减，1000 路 established 无墙。12 并发零 429。**墙重排定稿：上游延迟>内存**——首字延迟中位 3.5s/尾部 12.3s（服务端 TTFB 仅 0.02-0.09s，全在上游）。
