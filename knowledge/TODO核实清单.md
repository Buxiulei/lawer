...
#### 第二轮（2026-09-10，真实浏览器）

> 上面「一～三」是**第一轮**（2026-09-07，curl）。本轮换真实浏览器重跑同样三项，
> 目的与上一轮是同一个：把「查过没有」与「没人查过」分开。
> **三项判定全部不变**（仍是缺口）；变的是信源那张表——上一轮判「不可用」「翻不到」的
> 几个站，本轮在浏览器里可达或摸清了形状，下一个人不必再从零试一遍。
> 全程真实浏览器交互，未 curl、未登录、未绕验证码。

##### 一、一页结论

| pack id | 官方案例 | 擦边 | 线索（未证实） | 判定 |
|---|---:|---:|---:|---|
| risk-qiangzhi-baogao-zhuti | 0 | 0 | 1 | 仍是缺口 |
| risk-jilu-baocun-nianxian | 0 | 0 | 0 | 结构性缺口，复核不变 |
| risk-difang-xuke-beian | 0 | 1 | 0 | 仍是缺口；那条擦边与本领域无关，不建议挂卡 |

三项都没有找到官方案例，所以这三条的 `precedents` 本轮**仍是空数组**——
空的意思仍然是"查过，库里没有"，不是"没人查过"。

##### 二、信源可用性（真实浏览器实测，与第一轮同一批站）

| 信源 | 本轮结果 | 判定 |
|---|---|---|
| rmfyalk.court.gov.cn（人民法院案例库） | 检索即跳 account.court.gov.cn 强制登录（手机／支付宝／钉钉），无匿名入口 | **确认需登录**（第一轮是沿用旧结论，本轮是实锤） |
| www.spp.gov.cn 站内全文搜索 | URL 模式 `guestweb/s?siteCode=N000005434&searchWord=<词>`，服务端直出，支持精确／模糊与时间范围；「强制报告」1613 条，「强制报告 典型案例」11 条直接命中 2020-05-29 首批 | **本轮新增可用**；但语料是新闻／理论文章，**不是裁判文书或处罚决定库** |
| spp.gov.cn/xwfbh/dxal/index.shtml（典型案例栏目） | 静态分页 `index_2.shtml`… 确实存在，只是翻回 2020—2022 成本高 | 可用——**部分推翻第一轮的「只滚动近期条目、翻不到」** |
| www.12309.gov.cn（中国检察网） | 抓到页面自身的请求 `cmsApi/cms/manuscript/searchPage`、`cmsApi/open-api/manuscript/page`，均 **405**（页面预加载、未带参数） | 接口定位到了，未打通（第一轮是"定位不到"） |
| samr.gov.cn | 首页／行政处罚决定栏目／站内搜索均 200 | **本轮新增可用（浏览器）**；第一轮 curl 是 403 |
| creditchina.gov.cn | **400**（第一轮 412） | 不可用 |
| nhc.gov.cn | http／https 均 **400**（第一轮 412） | 不可用 |
| gsxt.gov.cn | 521 | 不可用 |
| bjgy.bjcourt.gov.cn | 可达，新闻／公告栏目；「信息查询」未展开 | 未摸清 |

##### 三、逐项：试过什么、判定是什么

**① risk-qiangzhi-baogao-zhuti——0 案例，1 条线索（未证实，本轮未登记）**

- 逐字核对后**排除**两份：`spp.gov.cn/xwfbh/dxal/202005/t20200529_463532.shtml`
  《侵害未成年人案件强制报告典型案例》五起——「心理咨询」在其中**均为事后救助手段**
  （委托咨询师做辅导／测评），**无一起的义务主体是咨询机构**，也无瞒报漏报情形；
  `spp.gov.cn/zdgz/202211/t20221109_592326.shtml`《强制报告！多地织密未成年人保护网》——
  「心理咨询」**0 命中**。
- **线索一条（未证实）**：`spp.gov.cn` 理论研究栏目 2026-01-21 刊文
  《优化路径完善侵害未成年人案件强制报告制度》
  （https://www.spp.gov.cn/llyj/202601/t20260121_716539.shtml ），
  承认强制报告主体范围界定存在模糊，并列举了未被明确纳入的新兴业态主体。
  **它是一篇署名理论文章，不是规范性文件，也不是案例**；它列举的新兴主体里
  **不含心理咨询机构**。所以它只能证明"分歧点是官方认领的问题"，
  **不能证明心理咨询机构落在哪一边**。
- **本轮没有把它登记进 `knowledge/sources.json`**：登记簿的 `kind` 受控词表
  （法律／行政法规／司法解释／部门规章／地方规章／规范性文件／官方案例／官方数据／
  行业规范／机构官网）里**没有一档装得下"官方站点刊载的署名理论文章"**，
  硬塞进任何一档都会让登记簿替这篇文章声称一个它没有的效力位阶。
  卡住的**不是取回**——该 URL 已实测可取（`fetch-source.py` 的同一条下载路径，
  HTTP 200、36611 字节、sha256 `8b24ae832c03`…，抽出正文 7570 字，
  报告里那几句摘句在抽出的正文里逐字 grep 得到）。等经理裁定 kind 口径后即可登记。
- **判定：仍是缺口。** 风险卡①的定性不受影响，`precedents` 保持空。

**② risk-jilu-baocun-nianxian——无候选、无线索**

本轮没有产生任何候选或线索。`nhc.gov.cn` 真实浏览器 http／https 均 400
（第一轮 curl 是 412），规范性文件发布页仍取不到。
第一轮的判定（**规则本身空白**，不是案例没搜到；复活条件是行业标准或明文规定出台，
而不是再检索一遍）**本轮复核不变**。

**③ risk-difang-xuke-beian——0 案例，1 条擦边（不建议挂卡）**

- 唯一擦边：广东省市场监管局网络不正当竞争典型案例第九例
  （`samr.gov.cn/jjj/fbzdjz/art/2023/art_adb4c9b25cde4731bc529315f60ed6f3.html`，
  惠州市光亮之家健康管理有限公司虚假宣传案），处罚的是把从业人员宣传成
  "心理咨询师"等头衔却拿不出资格证书，依《反不正当竞争法》第八条第一款
  认定虚假宣传、罚款 20 万元。
- **不建议挂卡**：主体是视力调理中心，**不是心理咨询机构**；处罚依据是虚假宣传，
  **与机构的行政许可／备案无关**。挂上去的形态是——卡上出现一条"官方案例"，
  而它回答的根本不是这张卡的问题。
- **判定：仍是缺口。** 但第一轮记的"samr.gov.cn 403 不可达"本轮被推翻：
  **浏览器可达**，所以这条路是"翻过了没有"，不再是"进不去"。

##### 四、下一个人接这一节时

1. **12309**：在页面检索框里真实输入并提交，抓那一次请求的 body 再照着构造——
   本轮抓到的两个接口是页面预加载发的，没带参数所以 405。**不需要账号。**
2. **spp 站内全文搜索**：`fetch-source.py` 大概率能直接取。
   **先用一个已知答案的查询自检**，确认返回的是真实结果而不是诱饵页
   （第一轮 Bing 那一课，见上面的 ⚠️ 第 2 条）。摘要不全就回 `xwfbh/dxal` 原路径取完整版。
3. **samr 惠州案例**：真实浏览器 UA 可访问，**curl 未测**。
4. **bjgy 的「信息查询」**、各区法院分站（如 `cyqfy.bjcourt.gov.cn`）的案例检索：
   这两处本轮没展开，留给下一个人。

##### 五、本轮的疑点（没查实，写下来免得下一个人当成已知）

1. spp 站内搜索「强制报告」1613 条 →「强制报告 典型案例」11 条，**召回逻辑未深究**。
2. 「心理咨询机构」61 条，前 10 条全是养老／司法救助，窄主题排序不友好，**未穷举组合**。
3. samr 那个页面的 URL 路径是 `/art/2023/`，页面标注却是 2021-11-19，**未核实**哪个是对的。
4. `nhc.gov.cn` 本轮 400、第一轮 412：拦截层的状态码不稳定，或按请求特征分流，**未查实**。
