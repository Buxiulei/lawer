...
**`updated` 什么时候进位**（经理 2026-09-10 裁定）：只在这张卡的**外部事实那一面**
重新核过时进位——引文重取或改字、数值更新、`sources` 增删、`confidence` 变档。
**纯正文与记录类改动不进位**：措辞、结构、研究日志（试过哪些源、各自回了什么）、
指向别处的说明。理由是它与 `confidence` 说的是同一面——"这张卡断言的外部事实核到哪天"。
让正文改动也进位的形态是：一张卡显示昨天刚核过，而它引的条文其实是半年前取的那份原件，
**且没有任何一处会说这件事**。
（实例：`risk-qiangzhi-baogao-zhuti` 2026-09-10 改写了两段研究日志——哪个站翻得到、
下一轮往哪走——官方原件一份没重取，所以它的 `updated` 仍是 2026-09-07。）
...
#### 7.3.1 单个包的显式欠账 `GROUNDING_PENDING`

`knowledge/packs/<某个包>/GROUNDING_PENDING` 让**该目录（含子目录）**整包退出 (b)–(h)，
只做结构校验。生成器在 stderr 逐目录报出张数与到期日；`verify-quotes.py` 对这批行
**照常核、照常印**，只是不计入退出码（报告里逐行标着「豁免·<目录>，不计入退出码」）。

文件里**必须**有一行 `最迟: YYYY-MM-DD`，没有就抛错：一份不会到期的豁免等于永久放行，
而它看起来只是一句临时说明。**到期即整体失效**，那天起生成器与 `verify-quotes` 一律判红，
红里印着这个文件的路径与到期日。

三条纪律：

1. **豁免的是守卫，不是入索引**——这批卡照常进 `index.json`、照常被检索到。
   要的是"少几张卡不如把账写下来"，不是"这批卡对用户不可见"。
2. **它罩的是目录段**，`packs/counseling` 的豁免罩不到 `packs/counseling-x/`。
3. **删掉文件即恢复守卫**。这个口子的开关就是这个文件本身，不是脚本里的一份名单
   （名单删起来要改代码、要过 review，于是它会留着）。
...
比对前两边都做归一：NFKC（全角→半角，**引号族除外**）、去掉全部空白与 markdown 的
`*` `_` `>` `` ` ``。所以卡里写成加粗引用块、原件是纯文本，不影响判定。

**引号字形一律不折**（经理 2026-09-07 复审裁定）：全角 `“ ” ‘ ’` 与半角 `" '` 在这把尺子下
不是同一个字，全角 `＂` 与半角 `"` 也不是（NFKC 顺手做的宽度折叠在这一族上被挡住了）。
折了的话，一张把原件 `“…”` 写成 `"…"` 的卡照样报「一致」——而 gen 的两面一致校验不折，
同一张卡在两把尺下一绿一红，绿的那把正是我们对外说"逐字核过官方原件"时指的那把。
（这不是吹毛求疵：引号在法条里划的是定义的边界，如「不宜认定为“劳动合同确实无法继续履行的”情形」。）
千分位逗号同理**不在**这把尺子里：`1,000` 与 `1000` 判不一致；那一层只属于数值那一面。

**生成器用的是同一个函数**（`knowledge_sources.normalize_quote`），不另立一把尺：
"卡片正文 ↔ 本卡 facts" 与 "卡片 ↔ 官方原件" 判的都是"这段字一不一样"。
数值 / 号码 / 地址那一面才在它之上多一层"去千分位逗号"（`gen-knowledge-index.normalize`）。
